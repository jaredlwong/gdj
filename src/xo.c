...
F jtjope(J jt, A w, C*mode)
{
...
#if (SYS&SYS_UNIX)
	{
		C* cs = CAV(t);
		f = fopen(cs, mode);
		if(!f && errno == ENOENT && !strcmp(mode, FUPDATE)) {
			f = fopen(cs, FUPDATEC);
		}
		if(!f && errno == EACCES && strcmp(mode, FREAD  )) {
			f = fopen(cs, FREAD);
		}
	}
#else
	{
		...
		s = (US*)CAV(z);
		for(i = 0; i < (I)strlen(mode); ++i) {
			usmode[i] = (US)mode[i];
		}
		usmode[i] = 0;
#if !SY_WINCE
		f = _wfopen(s, usmode);
		if(!f && errno == ENOENT && !wcscmp(usmode, FLUPDATE)) {
			f = _wfopen(s, FLUPDATEC);
		}
		if(!f && errno == EACCES && wcscmp(usmode, FLREAD  )) {
			f = _wfopen(s, FLREAD);
		}
#else
		{
			f = _wfopen(s, usmode);
			if(!f && !wcscmp(usmode, FLUPDATE)) {
				f = _wfopen(s, FLUPDATEC);    // no errno on wince
			}
			if(!f && wcscmp(usmode, FLREAD  )) {
				f = _wfopen(s, FLREAD);
			}
		}
#endif
...
F1(jtjopen)
{
	...
	if(AR(w)) {
		R rank1ex(w, 0L, 0L, jtjopen);
	}
	RE(h = fnum(w));
	if(h) {
		RZ(z = sc(h));
		ASSERT(fname(z), EVFNUM);
		R z;
	} else {
		if(jt->fopn == AN(jt->fopf)) {
			RZ(jt->fopa = ext(1, jt->fopa));
...
B jtadd2(J jt, F f1, F f2, C*cmd)
{
	...
	GA(c, LIT, 1 + strlen(cmd), 1, 0);
	MC(CAV(c) + 1, cmd, AN(c) - 1);
	cmd = CAV(c);
	if(jt->fopn + 1 >= AN(jt->fopf)) {
		RZ(jt->fopa = ext(1, jt->fopa));
		RZ(jt->fopf = ext(1, jt->fopf));
	}
	*cmd = '<';
	RZ(*(jt->fopn + AAV(jt->fopa)  ) = ra(cstr(cmd)));
	RZ(*(jt->fopn + IAV(jt->fopf)  ) = (I)f1);
	*cmd = '>';
	RZ(*(jt->fopn + AAV(jt->fopa) + 1) = ra(cstr(cmd)));
...
F1(jtjclose)
{
	...
	if(AR(w)) {
		R rank1ex(w, 0L, 0L, jtjclose);
	}
	RE(j = i0(indexof(jt->fopf, sc(fnum(w)))));
