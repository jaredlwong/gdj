...
static I jtpad(J jt, A a, A w, C*zv)
{
	C dash, *u, *v, *wv;
	I c, d, r, *s;
	RZ(a && w);
...
static A jtcenter(J jt, A a, I j, I k, I m)
{
	A z;
	C*x;
	I n, *s, zn;
...
static F1(jttreach)
{
	R troot(scc('0'), graft(ope(every(w, 0L, jttrr))));
}
...
static F1(jttrr)
{
	...
	x = AAV(t);
	if(0 < m) {
		RZ(x[0] = fl & VGERL ? treach(fxeach(fs)) : trr(fs));
	}
	if(1 < m) {
		RZ(x[1] = fl & VGERR ? treach(fxeach(gs)) : ex ? trr(unparsem(zero, w)) : trr(gs));
	}
	if(2 < m) {
		RZ(x[2] = trr(hs));
	}
...
{
	PROLOG;
	EPILOG(tconnect(troot(mtv, trr(w))));
}
