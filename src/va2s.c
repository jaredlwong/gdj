...
static A jtvaspc(J jt, A a, A w, C id, VF ado, I cv, I t, I zt, I af, I acr, I wf, I wcr, I f, I r)
{
	A q;
	I*as, *v, *ws;
	as = AS(a);
...
static A jtvasp0(J jt, A a, A w, VF ado, I cv, I t, I zt)
{
	A e, x, xx, y, z, ze, zx;
	B b;
	I n;
	...
	GA(ze, zt, 1, 0,    0    );
	ado(jt, 0, 1L, 1L, AV(ze), b ? AV(e) : AV(y), b ? AV(y) : AV(e));
	RE(0);
	GA(zx, zt, n, AR(x), AS(x));
	if(n) {
		ado(jt, !b, 1L, n, AV(zx), b ? AV(x) : AV(y), b ? AV(y) : AV(x));
	}
	RE(0);
	if(cv & VRI + VRD) {
		RZ(ze = cvz(cv, ze));
	...
	GA(z, STYPE(AT(zx)), 1, AR(xx), AS(xx));
	zp = PAV(z);
	SPB(zp, a, ca(SPA(p, a)));
	SPB(zp, i, ca(SPA(p, i)));
	SPB(zp, e, ze);
...
static I zcount(A ay, A wy, B ab, B wb)
{
	I c, d, i, j, m, n, *u, *v, yc;
	v = AS(ay);
	m = v[0];
	yc = v[1];
	n = *AS(wy);
...
static A jtvaspeq(J jt, A a, A w, C id, VF ado, I cv, I t, I zt, I f, I r)
{
	A ae, ax, ay, we, wx, wy, z, za, ze, zx, zy;
	B ab = 1, wb = 1;
	C*aev, *axv, *wev, *wxv, *zxv;
	...
	GA(zx, zt, d * xc, AR(ax), AS(ax));
	*AS(zx) = d;
	zxv = CAV(zx);
	zk = xc * bp(zt);
	GA(zy, INT, d * yc, 2,     AS(ay));
	*AS(zy) = d;
	zyv = AV(zy);
	i = j = d = 0;
	u = AV(ay);
	...
	if     (wb && m > i) {
		c = m - i;
		ICPY(zyv, u, c * yc);
		ado(jt, 0, 1L, c * xc, zxv, axv, wev);
	} else if(ab && n > j) {
		c = n - j;
		ICPY(zyv, v, c * yc);
		ado(jt, 1, 1L, c * xc, zxv, aev, wxv);
	}
	GA(ze, zt, 1, 0, 0);
	ado(jt, 0, 1L, 1L, AV(ze), aev, wev);
	RE(0);
	if(cv & VRI + VRD) {
		A e, x;
		RZ(e = cvz(cv, ze));
...
A jtvasp(J jt, A a, A w, C id, VF ado, I cv, I t, I zt, I af, I acr, I wf, I wcr, I f, I r)
{
	A fs, z;
	if(!AR(a) || !AR(w)) {
		R vasp0(a, w, ado, cv, t, zt);
	}
	if((SPARSE & AT(a) || SPARSE & AT(w)) && spmult(&z, a, w, id, af, acr, wf, wcr)) {
		R z;
	}
	if(af != wf) {
		RZ(fs = ds(id));
		R sprank2(a, w, 0L, acr, wcr, VAV(fs)->f2);
	}
	if(acr != wcr) {
		R vaspc(a, w, id, ado, cv, t, zt, af, acr, wf, wcr, f, r);
	}
