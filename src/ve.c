...
static I remii(I a, I b)
{
	I r;
	R!a ? b : (r = b % a, 0 < a ? r + a * (0 > r) : r + a * (0 < r));
}
...
D jtdgcd(J jt, D a, D b)
{
	...
	a1 = a;
	b1 = b;
	while(remdd(a1 / jfloor(0.5 + a1 / a), b1)) {
		t = a;
		a = remdd(a, b);
		b = t;
	}
...
static D jtdlcm(J jt, D a, D b)
{
	ASSERT(!(INF(a) || INF(b)), EVNAN);
	R a&&b ? a*(b / dgcd(a, b)) : 0;
}
...
F2(jtintdiv)
{
	A z;
	B b, flr;
	I an, ar, *as, *av, c, d, j, k, m, n, p, p1, r, *s, wn, wr, *ws, *wv, *zv;
	RZ(a && w);
	an = AN(a);
	ar = AR(a);
	as = AS(a);
	...
	if(an && wn) {
		m = prod(r, s);
		n = prod(b ? ar - r : wr - r, r + s);
	} else {
		m = n = 0;
	}
	GA(z, INT, b ? an : wn, b ? ar : wr, s);
	zv = AV(z);
	d = wn ? *wv : 0;
	p = 0 < d ? d : -d;
	p1 = d == IMIN ? p : p - 1;
	...
	if(!wr && p && !(p & p1)) {
		...
		case 0:
			...
			break;
		case 1:
			DO(n, c = *av++; *zv++ = 0 < c ? 1 + ((c - 1) >> k) : (c + p1) >> k;);
			break;
		case 2:
			DO(n, c = *av++; *zv++ = c > IMIN ? -c >> k : -(-c >> k););
			break;
		case 3:
			DO(n, c = *av++; *zv++ = 0 <= c ? -(c >> k) : 1 + (-(1 + c) >> k););
		}
	} else if(flr) {
		if(1 == n) {
			DO(m, c = *av++; GETD; *zv++ = INTDIVF(c, d);)
		} else if(b) {
			DO(m, GETD; DO(n, c = *av++; *zv++ = INTDIVF(c, d););)
		} else {
			DO(m, c = *av++; DO(n, GETD;    *zv++ = INTDIVF(c, d););)
		}
	} else {
		if(1 == n) {
			DO(m, c = *av++; GETD; *zv++ = INTDIVC(c, d);)
		} else if(b) {
			DO(m, GETD; DO(n, c = *av++; *zv++ = INTDIVC(c, d););)
		} else {
			DO(m, c = *av++; DO(n, GETD;    *zv++ = INTDIVC(c, d););)
		}
	}
...
{
	RZ(a && w);
	R df1(behead(over(AR(w) ? w : reshape(a, w), one)), bsdot(slash(ds(CSTAR))));
}
...
F1(jtbase1)
{
	...
	if(c > (SY_64 ? 63 : 31) || !(t & B01)) {
		R pdt(w, weight(sc(c), t & RAT + XNUM ? cvt(XNUM, num[2]) : num[2]));
	}
	if(c) {
		m = n / c;
	} else {
...
F1(jtabase1)
{
	...
	ASSERT(t & DENSE, EVNONCE);
	if(!n || t & B01) {
		R reshape(over(shape(w), n ? one : zero), w);
	}
	if(!(t & INT)) {
...
F2(jtabase2)
{
	...
	if(1 == ar && at & B01 + INT&&wt & B01 + INT) {
		I*av, d, r, *u, *wv, x, *zv;
		RZ(coerce2(&a, &w, INT));
		RE(zn = mult(an, wn));
		GA(z, INT, zn, 1 + wr, AS(w));
		*(wr + AS(z)) = an;
		av = an + AV(a);
		wv = wn + AV(w);
		zv = zn + AV(z);
		if(2 == an && !av[-2] && 0 < (d = av[-1])) {
			I d1, j, k;
			k = 0;
			j = 1;
			while(d > j) {
	...
	u = av;
	DO(an, d = *--u; *--zv = r = remii(d, x); x = d ? (x - r) / d : 0;);
		  );
		R z;
	} else {
		PROLOG;
