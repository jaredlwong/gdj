...
void jtwri(J jt, I type, C*p, I m, C*s)
{
	C buf[1024], *t = jt->outseq, *v = buf;
	I c, d, e, n;
	if(jt->tostdout) {
		c = strlen(p);          /* prompt      */
...
static void jtwrf(J jt, I n, C*v, F f)
{
	C*u, *x;
	I j = 0, m;
	while(n > j) {
...
A jtinpl(J jt, B b, I n, C*s)
{
	C c;
	I k = 0;
	if(n && (c = *(s + n - 1), CLF == c || CCR == c)) {
		--n;
	}
...
static C* nfeinput(J jt, C* s)
{
	...
	jt->breakignore = 0;
	if(!y) {
		breakclose(jt);    /* J input verb failed */
		exit(2);
	}
...
I jdo(J jt, C* lp)
{
	...
	A x;
	jt->jerr = 0;
	jt->etxn = 0; /* clear old errors */
	old = jt->tbase + jt->ttop;
	*jt->adbreak = 0;
	x = inpl(0, (I)strlen(lp), lp);
	while(jt->iepdo && jt->iep) {
		jt->iepdo = 0;
		immex(jt->iep);
		jt->jerr = 0;
		tpop(old);
...
void jsto(J jt, I type, C*s)
{
	...
	if(jt->nfe) {
		...
		if(jt->smoutput) {
			((outputtype)(jt->smoutput))(jt, (int)type, s);
		}
#if SY_WIN32 && !SY_WINCE
		if(type & MTYOFM) {
			oleoutput(jt, strlen(s), s);    /* save output for ole */
		}
#endif
	}
}
...
C* esub(J jt, I ec)
{
	...
	if(ec == EDCBUSY) {
		return "busy with previous input";
	}
	if(ec == EDCEXE) {
		return "not supported in EXE server";
	}
	if(ec > NEVM || ec < 0) {
