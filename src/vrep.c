...
static REPF(jtrepzdx)
{
	...
	if(SPARSE & AT(w)) {
		...
		x = jt->fill && AN(jt->fill) ? jt->fill : filler(w);
	}
...
static REPF(jtrepzsx)
{
	...
	if(equ(zero, SPA(ap, e))) {
		...
		if(c == k) {
			RZ(w = irs2(sc(1 + k), w, 0L, 0L, wcr, jttake));
		}
...
#if !SY_64 && SY_WIN32
static REPF(jtrepbdx)
{
	A z;
	B*b;
	C*wv, *zv;
	I c, i, *iv, j, k, m, p, q, r, zn;
	RZ(a && w);
	if(SPARSE & AT(w)) {
		R irs2(ifb(AN(a), BAV(a)), w, 0L, 1L, wcr, jtfrom);
	}
	m = AN(a);
	q = m / SZI;
	r = m % SZI;
	...
	*(wf + AS(z)) = p;
	...
	if(zn)switch(k = AN(w) / (c * m) * bp(AT(w)), FL & AT(w) || k != sizeof(D) ? k : 0) {
		...
		default: {
			C*u;
			I k1 = k, k2 = k * 2, k3 = k * 3, k4 = k * 4, km = k * m;
			for(i = 0; i < c; ++i) {
				u = i * km + wv;
...
static REPF(jtrepbsx)
{
	...
	b = BAV(x);
	if(!AN(SPA(ap, a))) {
		R irs2(ifb(n, b), w, 0L, 1L, wcr, jtfrom);
	}
	if(!*BAV(e)) {
		GA(q, INT, n, 1, 0);
		v = v0 = AV(q);
	...
	wp = PAV(w);
	if(DENSE & AT(w) || all0(eq(sc(wf), SPA(wp, a)))) {
		RZ(q = denseit(a));
		R irs2(ifb(AN(q), BAV(q)), w, 0L, 1L, wcr, jtfrom);
	}
	wa = SPA(wp, a);
	wy = SPA(wp, i);
	wx = SPA(wp, x);
	...
	*(wf + AS(z)) = m = *AV(q);
	...
	v = AV(ai);
	DO(n, if(!*b++)*v++ = u[i];);
	RZ(g = grade1(over(ai, repeat(q, y1))));
	gv = AV(g);
	GA(d, INT, AN(y1), 1, 0);
	dv = AV(d);
	j = 0;
	DO(AN(g), if(m > gv[i])++j; else {
		dv[gv[i] - m] = j;
		});
	RZ(zy = repeat(q, wy));
	v = AV(zy) + *AV(c);
...
static REPF(jtrep1d)
{
	...
	if(t & CMPX) {
		if(wcr) {
			R repzdx(from(apv(n, 0L, 0L), a), w,                wf, wcr);
		} else {
			R repzdx(a, irs2(apv(m, 0L, 0L), w, 0L, 1L, 0L, jtfrom), wf, 1L );
		}
	}
	if(t & B01) {
		B*x = BAV(a);
...
static B jtrep1sa(J jt, A a, I*c, I*d)
{
	A x;
	B b;
	I*v;
	...
	if(AR(a)) {
		ASSERT(equ(one, aslash(CSTARDOT, le(zero, ravel(x)))), EVDOMAIN);
		RZ(x = aslash(CPLUS, x));
	}
	if(!(INT&AT(x))) {
		RZ(x = cvt(INT, x));
...
static REPF(jtrep1s)
{
	...
	RZ(a && w);
	if(AT(a)&SCMPX) {
		R rep1d(denseit(a), w, wf, wcr);
	}
	RE(rep1sa(a, &c, &d));
	cd = c + d;
	if(DENSE & AT(w)) {
		R rep1d(d ? jdot2(sc(c), sc(d)) : sc(c), w, wf, wcr);
	}
	wr = AR(w);
	ws = AS(w);
	n = wcr ? *(wf + ws) : 1;
...
F2(jtrepeat)
{
	...
	wb = 1 && wt & DENSE;
	if(1 < acr || acr < ar) {
		R rank2ex(a, w, 0L, MIN(1, acr), wcr, jtrepeat);
	}
	ASSERT(!acr || !wcr || (m = *AS(a), m == *(wf + AS(w))), EVLENGTH);
	if(!acr || !wcr) {
		R ab&&wb ? rep1d(a, w, wf, wcr) : rep1s(a, w, wf, wcr);
	}
	if(at & CMPX + SCMPX) {
		R ab ? repzdx(a, w, wf, wcr) : repzsx(a, w, wf, wcr);
	}
	if(at & B01 + SB01 ) {
		R ab ? repbdx(a, w, wf, wcr) : repbsx(a, w, wf, wcr);
	}
