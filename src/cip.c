...
static A jtipprep(J jt, A a, A w, I zt, I*pm, I*pn, I*pp)
{
	A z = mark;
	I*as, ar, ar1, m, mn, n, p, *ws, wr, wr1;
	ar = AR(a);
	as = AS(a);
	ar1 = ar ? ar - 1 : 0;
	RE(*pm = m = prod(ar1,  as));
...
F2(jtpdt)
{
	...
	if(ar && wr && AN(a) && AN(w) && at != wt && B01 & at + wt) {
		R pdtby(a, w);
	}
	t = coerce2(&a, &w, B01);
	...
	case FL: {
		...
		NAN0;
		if(1 == n) {
			DO(m, v = wv; c = 0.0; DO(p, s = *u++; t = *v++; c += s && t ? s*t : 0;); *zv++ = c;);
		} else for(i = 0; i < m; ++i, v = wv, zv += n) {
				x = zv;
				if(c = *u++) {
					if(INF(c)) {
	...
	case CMPX: {
		...
		c = zeroZ;
		DO(p, c.re += ZRE(*u, *v); c.im += ZIM(*u, *v); ++u; ++v;);
		*zv++ = c;
		  )
		else for(i = 0; i < m; ++i, v = wv, zv += n) {
		x = zv;
		c = *u++;
		DO(n, x->re = ZRE(c, *v); x->im = ZIM(c, *v); ++x; ++v;);
...
static A jtipbx(J jt, A a, A w, C c, C d)
{
	A g = 0, x0, x1, z;
	B*av, b, e, *u, *v, *v0, *v1, *zv;
	C c0, c1;
	...
	if(!g) {
		RZ(x0 = c0 == IPBX0 ? reshape(sc(n), zero) : c0 == IPBX1 ? reshape(sc(c == CNE ? AN(w) : n), one) : c0 == IPBXW ? w : not(w));
	}
	if(!g) {
		RZ(x1 = c1 == IPBX0 ? reshape(sc(n), zero) : c1 == IPBX1 ? reshape(sc(c == CNE ? AN(w) : n), one) : c1 == IPBXW ? w : not(w));
	}
	av = BAV(a);
	zv = BAV(z);
	v0 = BAV(x0);
...
static DF2(jtdotprod)
{
	...
	if(B01 & AT(a) && AN(a) && AN(w) && CSLASH == ID(fs) && (d = vaid(gs)) &&
			(c = vaid(VAV(fs)->f), c == CSTARDOT || c == CPLUSDOT || c == CNE)) {
		R ipbx(a, w, c, d);
	}
	r = lr(gs);
...
DF1(jtdetxm)
{
	R dotprod(irs1(w, 0L, 1L, jthead), det(minors(w), self), self);
}
