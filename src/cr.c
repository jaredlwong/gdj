...
A jtrank1ex(J jt, A w, A fs, I mr, AF f1)
{
	...
	if(AN(w)) {
		...
		RZ(yw = reshape(vec(INT, wcr, ws + wf), filler(w)));
	}
#define VALENCE   1
#define TEMPLATE  0
#include "cr_t.h"
...
A jtrank2ex(J jt, A a, A w, A fs, I lr, I rr, AF f2)
{
	...
	if(AN(a)) {
		MOVEYA
			...
			RZ(ya = reshape(vec(INT, acr, as + af), filler(a)));
	}
	if(AN(w)) {
		MOVEYW
	} else {
...
A jtirs1(J jt, A w, A fs, I m, AF f1)
{
	A z;
	I*old = jt->rank, rv[2], wr;
	RZ(w);
	wr = AR(w);
	rv[1] = m = efr(wr, m);
	if(m >= wr) {
...
static void qqset(A a, AF*f1, AF*f2, I*flag)
{
	A f, g;
	C c, d, e, p, q;
	I m = 0;
	...
	if(!(m & VIRS1) && v->flag & VIRS1 && c != CQQ) {
		m += VIRS1;
	}
	if(!(m & VIRS2) && v->flag & VIRS2 && c != CQQ) {
		m += VIRS2;
	}
	if(!m) {
		p = 0;
		if(f = v->f) {
			p = VERB & AT(f) && strchr(ir2, d = ID(f));
		}
		q = 0;
		if(g = v->g) {
			q = VERB & AT(g) && strchr(ir2, e = ID(g));
		}
		switch(c) {
		case CFIT:
			if(p && d != CEXP) {
...
F2(jtqq)
{
	...
	if(VERB & AT(w)) {
		...
		d = DAV(t);
		n = r[0] = hv[0] = mr(w);
		d[0] = n <= -RMAX ? -inf : RMAX <= n ? inf : n;
		n = r[1] = hv[1] = lr(w);
		d[1] = n <= -RMAX ? -inf : RMAX <= n ? inf : n;
		n = r[2] = hv[2] = rr(w);
		d[2] = n <= -RMAX ? -inf : RMAX <= n ? inf : n;
		w = t;
	} else {
		n = AN(w);
