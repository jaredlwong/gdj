...
static B jtspsscell(J jt, A w, I wf, I wcr, A*zc, A*zt)
{
	A c, t, y;
	B b;
	I cn, *cv, j, k, m, n, p, *s, tn, *tv, *u, *u0, *v, *v0;
	P*wp;
	wp = PAV(w);
	s = AS(w);
	p = 3 + s[wf];
	...
	*(1 + AS(c)) = 2;
	...
	cn += 2;
	AN(t) =    *AS(t) = tn;
	*zt = t; /* cell divisions (row indices in y)            */
	AN(c) = cn;
	*AS(c) = cn / 2;
	*zc = c; /* item divisions (indices in t, # of elements) */
	R 1;
}    /* frame: all sparse; cell: 1 or more sparse, then dense */

static A jtgrd1spz(J jt, A w, I wf, I wcr)
{
	A z;
	I*ws, zn;
	ws = AS(w);
...
static void sp1merge0(I n, I n1, I yc, I*zv, I*xv, I*yv, I*tv)
{
	I c, d = n1 - 1, h, i, j, k, p, q, *v = zv;
	DO(n1, if(0 == xv[i]) {
	j = i;
	break;
});      /* lower limit for zero items   */
	DO(n1, if(d == xv[i]) {
	k = i;
	break;
});      /* upper limit for zero items   */
	DO(j, *v++ = yv[yc * tv[xv[i]]];);      /* items less than zero         */
	p = 1 + j;
	c = k > p ? yv[yc * tv[xv[p]]] : -1;
	q = 1;
	h =    yv[yc * tv[q    ]]   ;    /* merge indexed zero items ... */
...
static A jtgrd1spss(J jt, A w, I wf, I wcr)
{
	A c, d, t, x, y, z;
	I cn, *cv, *dv, i, n, n1, *tv, *u, *ws, wt, *xv, yc, *yv, *zv;
	P*wp;
	wp = PAV(w);
	wt = AT(w);
	ws = AS(w);
	...
	y = SPA(wp, i);
	jt->compsyv = yv = AV(y);
	jt->compsyc = yc = *(1 + AS(y));
	x = SPA(wp, x);
	jt->compsxv = CAV(x);
	jt->compsxc = aii(x) * (wt & SCMPX ? 2 : 1);
	jt->compw = w;
	jt->compswf = wf;
	jt->comp = wt & SB01 ? compspssB : wt & SINT ? compspssI : wt & SFL ? compspssD : compspssZ;
	...
	for(i = 0; i < cn; i += 2) {
		jt->compstv = u = tv + cv[i];
		n1 = cv[1 + i] - 1;
		while(ICMP(dv, yv + yc**u, wf)) {
			DO(n, zv[i] = i;);
			zv += n;
			ADVANCE(dv);
		}
...
static A jtgrd1spsd(J jt, A w, I wf, I wcr)
{
	A d, t, y, z;
	I*dv, i, n, p, *tv, yc, *ws, *ys, *yv, *zv;
	P*wp;
	wp = PAV(w);
	ws = AS(w);
	n = wcr ? ws[wf] : 1;
	RZ(z = grd1spz(w, wf, wcr));
	zv = AV(z);
	RZ(t = irs1(SPA(wp, x), 0L, wcr, jtgr1));
...
static B jtspdscell(J jt, A w, I wf, I wcr, A*zc, A*zt)
{
	A c, t, y;
	I*cv, m, n, p, *s, tn, *tv, *v, *v0;
	P*wp;
	wp = PAV(w);
	s = AS(w);
	p = 3 + s[wf];
...
static A jtgrd1spds(J jt, A w, I wf, I wcr)
{
	A c, t, x, y, z;
	I*cv, m, n, n1, p, *tv, *ws, wt, *xv, yc, *yv, *zv;
	P*wp;
	wp = PAV(w);
	wt = AT(w);
	ws = AS(w);
	...
	x = SPA(wp, x);
	jt->compsxv = CAV(x);
	jt->compsxc = p = aii(x) * (wt & SCMPX ? 2 : 1);
	jt->compn = p / m;
	...
	cv = AV(c);
	n1 = cv[1] - 1;
	jt->compstv = tv = cv[0] + AV(t);
	GA(x, INT, MAX(n, 1 + n1), 1, 0);
	xv = AV(x); /* work area for msmerge() */
	if(cv[0]) {
		DO(m, jt->compsi = i;
	}
...
static A jtgrd1spdd(J jt, A w, I wf, I wcr)
{
	A x;
	I n, *ws;
	P*wp;
...
F1(jtgrd1sp)
{
	...
	DO(wcr, --j; if(wb[j])b = 1; else if(b) {
		c = 1;
		wb[j] = 1;
		});
	if(c) {
		RZ(w = reaxis(ifb(wr, wb), w));
	}
	switch(2 * wb[0] + wb[wf]) {
	case 0: /* dense  dense  */
		z = grd1spdd(w, wf, wcr);
...
static void sp2merge0(I n, I n1, I yc, I*zyv, I*xv, I*yv, I*tv)
{
	I c, d = n1 - 1, h, i, j, k, m = 0, p, q;
	DO(n1, if(0 == xv[i]) {
	j = i;
	break;
});  /* lower limit for zero items   */
...
static A jtgrd2spss(J jt, A w, I wf, I wcr)
{
	A c, t, x, y, z, zy;
	I cn, *cv, i, j, m, n, n1, *tv, *u, *ws, wt, *xu, *xv, yc, *yv, *zyv;
	P*wp, *zp;
	RZ(z = ca(w));
	zp = PAV(z);
	wp = PAV(w);
	...
	for(i = 0; i < cn; i += 2) {
		...
		if(u[0] < u[1])SP2RENUM(0, n1, zyv + wf, u)
				...
				sp2merge0(n, n1, yc, zyv + wf, xv, yv + wf, u);
			}
	}
	RZ(x = grade1(zy));
	SPB(zp, i, from(x, zy));
	SPB(zp, x, from(x, SPA(zp, x)));
	R z;
}    /* sparse frame, sparse cell */

static A jtgrd2spsd(J jt, A w, I wf, I wcr)
{
	A x, z;
	P*zp;
	RZ(z = ca(w));
...
F2(jtgrd2sp)
{
	...
	A z;
	B b, c, *wb;
	I acr, af, am, ar, *as, j, m, wcr, wf, wm, wr, *ws;
	P*wp;
	RZ(a && w);
	ar = AR(a);
	acr = jt->rank ? jt->rank[1] : ar;
	af = ar - acr;
	wr = AR(w);
	wcr = jt->rank ? jt->rank[1] : wr;
	wf = wr - wcr;
	...
	default:
		z = irs2(irs1(w, 0L, wcr, jt->compgt == 1 ? jtgrade1 : jtdgrade1), a, 0L, RMAX, acr, jtfrom);
		break;
	case 2: /* sparse dense  */
		z = grd2spsd(w, wf, wcr);
		break;
