...
I jtmult(J jt, I x, I y)
{
	B neg;
	I a, b, c, p, q, qs, r, s, z;
	static I m = 0x00000000ffffffff;
	if(!x || !y) {
		R 0;
	}
	neg = 0 > x != 0 > y;
...
D jfloor1(D x)
{
	...
	if(x < -4.51e15 || 4.51e15 < x) {
		R x;
	}
	y = x;
	*((I*)&y) &= 0xffffffffff000000;
	R y + floor(x - y);
...
{
	RZ(w);
	R !memchr(AV(w), C1, AN(w));
}

B all1(A w)
{
	RZ(w);
	R !memchr(AV(w), C0, AN(w));
}

I jtaii(J jt, A w)
{
	I m = IC(w);
	R m&&!(SPARSE & AT(w)) ? AN(w) / m : prod(AR(w) - 1, 1 + AS(w));
}

A jtapv(J jt, I n, I b, I m)
{
	A z;
	I j = b - m, p = b + m * (n - 1), *x;
	GA(z, INT, n, 1, 0);
...
#ifdef UNDER_CE
	default:
		R t&XD ? sizeof(DX) : t&XZ ? sizeof(ZX) : -1;
#else
	case XD:
		R sizeof(DX);
	case XZ:
...
I jtmaxtype(J jt, I s, I t)
{
	...
	u = s | t;
	if(!(u & SPARSE)) {
		R u&CMPX ? CMPX : u&FL ? FL : s < t ? t : s;
	}
	if(s) {
		s = s & SPARSE ? s : STYPE(s);
		ASSERT(s, EVDOMAIN);
	}
	if(t) {
		t = t & SPARSE ? t : STYPE(t);
		ASSERT(t, EVDOMAIN);
	}
	R s < t ? t : s;
}
...
A jtodom(J jt, I r, I n, I*s)
{
	A q, z;
	I j, k, m, *u, *zv;
	RE(m = prod(n, s));
	k = n * SZI;
	GA(z, INT, m * n, 2 == r ? 2 : n, s);
	zv = AV(z) - n;
	if(2 == r) {
		u = AS(z);
		u[0] = m;
...
F1(jtvib)
{
	...
	case XNUM:
	case RAT:
		z = cvt(INT, maximum(sc(p), minimum(sc(q), w)));
		break;
	default:
		if(!(AT(w)&FL)) {
			RZ(w = cvt(FL, w));
