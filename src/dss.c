...
static A jtssdo(J jt, A a, A w, C c)
{
	DC d, e;
	I n, *v;
	RZ(w = vs(w));
	...
	while(d && !d->dcsusp) {                   /* do until topmost suspension     */
		if(d->dctype == DCCALL) {
			*(I*)(d->dci) = -2;    /* terminate each call             */
		}
		d = d->dclnk;
	}
	ASSERT(d, EVDOMAIN);                       /* must have a suspension          */
	while(d && DCCALL != d->dctype) {
		d = d->dclnk;    /* find topmost call               */
	}
	ASSERT(d, EVDOMAIN);                       /* must have a call                */
	if(a) {
		RE(n = lnumcw(i0(a), d->dcc));    /* source line # to cw line #      */
	}
	...
	fa(jt->dbssexec);
	jt->dbssexec = AN(w) ? ra(w) : 0;
	R mtm;                                     /* 0 return to terminate call      */
}

F1(jtdbcutback  )
{
	R ssdo(0L, w, SSCUTBACK );   /* 13!:19 */
}

F1(jtdbstepover1)
{
	R ssdo(0L, w, SSSTEPOVER);   /* 13!:20 */
}
F2(jtdbstepover2)
{
	R ssdo(a, w, SSSTEPOVER);
}

F1(jtdbstepinto1)
{
	R ssdo(0L, w, SSSTEPINTO);   /* 13!:21 */
}
F2(jtdbstepinto2)
{
	R ssdo(a, w, SSSTEPINTO);
}

F1(jtdbstepout1 )
{
	R ssdo(0L, w, SSSTEPOUT );   /* 13!:22 */
}
F2(jtdbstepout2 )
{
	R ssdo(a, w, SSSTEPOUT );
}
