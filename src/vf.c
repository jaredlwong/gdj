...
static F2(jtrotsp)
{
	...
	A q, x, y, z;
	B bx, by;
	I acr, af, ar, *av, d, k, m, n, p, *qv, *s, *v, wcr, wf, wr;
	P*wp, *zp;
	RZ(a && w);
	...
	jt->rank = 0;
	if(1 < acr || af) {
		R df2(a, w, qq(qq(ds(CROT), v2(1L, RMAX)), v2(acr, wcr)));
	}
	if(!wcr && 1 < p) {
		RZ(w = reshape(over(shape(w), apv(p, 1L, 0L)), w));
		wr = wcr = p;
	}
...
static void jtrot(J jt, I m, I c, I n, I k, I p, I*av, C*u, C*v)
{
	I dk, e, j, r, x, y;
	e = c * k;
	dk = e / n;
	if(jt->fill) {
		mvc(m * e, v, k, jt->fillv);
	}
	switch((jt->fill ? 0 : 2) + (1 < p)) {
	case 0:
		r = p ? *av : 0;
		ROF(r);
		DO(m, MC(j + v, k + u, y);            u += e; v += e;);
		break;
	case 1:
		DO(m, r = av[i]; ROF(r);       MC(j + v, k + u, y);            u += e; v += e;);
		break;
	case 2:
		r = p ? *av : 0;
		ROT(r);
		DO(m, MC(v, j + u, k); MC(k + v, u, j); u += e; v += e;);
		break;
	case 3:
		DO(m, r = av[i]; ROT(r);       MC(v, j + u, k); MC(k + v, u, j); u += e; v += e;);
	}
}
...
F2(jtrotate)
{
	...
	B b;
	C*u, *v;
	I acr, af, ar, *av, k, m, n, p, *s, wcr, wf, wn, wr;
	RZ(a && w);
	if(SPARSE & AT(w)) {
		R rotsp(a, w);
	}
...
F1(jtreverse)
{
	...
	if(!(r && AN(w))) {
		RZ(z = ca(w));
		R ARELATIVE(w) ? relocate((I)w - (I)z, z) : z;
	}
	wt = AT(w);
	ws = AS(w);
	wv = CAV(w);
	...
	default:        {
		C*s = wv - k, *t;
		DO(m, t = s += nk; DO(n, memcpy(zv, t, k); zv += k; t -= k;););
	}
	break;
	case sizeof(C): {
		C*s =    wv, *t, *u =    zv;
...
static A jtreshapesp0(J jt, A a, A w, I wf, I wcr)
{
	A e, p, x, y, z;
	B*b, *pv;
	I c, d, r, *v, wr, *ws;
...
static A jtreshapesp(J jt, A a, A w, I wf, I wcr)
{
	A a1, e, t, x, y, z;
	B az, *b, wz;
	I an, *av, c, d, j, m, *u, *v, wr, *ws;
	P*wp, *zp;
	RZ(a = cvt(INT, a));
	an = AN(a);
	av = AV(a);
	...
	v = ws + wr;
	m = 0;
	DO(MIN(an, wcr - 1), if(*--u != *--v) {
	m = 1;
	break;
});
	if(m || an < wcr) {
		R reshapesp(a, irs1(w, 0L, wcr, jtravel), wf, 1L);
	}
	ASSERT(!jt->fill, EVDOMAIN);
	GA(z, AT(w), 1, wf + an, ws);
	ICPY(wf + AS(z), av, an);
	zp = PAV(z);
	SPB(zp, e, e);
	GA(t, INT, c + d * b[wf], 1, 0);
	v = AV(t);
	DO(wf, if(b[i])*v++ = i;);
	if(b[wf]) {
		DO(d, *v++ = wf + i;
	...
	j = wf;
	DO(wcr, if(b[j])*v++ = d + j; ++j;);
	SPB(zp, a, t);
	if(b[wf]) {
		I n, q, r, *v0; /* sparse */
		if(wf != *AV(a1)) {
			R rank2ex(a, w, 0L, 1L, wcr, jtreshape);
		}
		RE(m = prod(1 + d, av));
		n = IC(y);
		q = n * (m / ws[wf]);
		r = m % ws[wf];
		v = AV(y);
		DO(n, if(r <= *v)break; ++q; v += c;);
		GA(t, INT, q, 1, 0);
		u = AV(t);
		v = v0 = AV(y);
		m = j = 0;
		DO(q, u[i] = m + *v; v += c; ++j; if(j == n) {
		j = 0;
		v = v0;
		m += ws[wf];
		});
...
F2(jtreshape)
{
	...
	jt->rank = 0;
	if(1 < acr || acr < ar) {
		R rank2ex(a, w, 0L, MIN(1, acr), wcr, jtreshape);
	}
	RZ(a = vip(a));
	r = AN(a);
	u = AV(a);
	if(SPARSE & AT(w)) {
		R reshapesp(a, w, wf, wcr);
	}
	RE(m = prod(r, u));
	RE(c = prod(wf, ws));
	RE(n = c ? AN(w) / c : prod(wcr, wf + ws));
...
F2(jtreitem)
{
	...
	jt->rank = 0;
	if(1 < acr || acr < ar) {
		R rank2ex(a, w, 0L, m, wcr, jtreitem);
	}
	if(1 >= wcr) {
		y = a;
	} else {
...
F2(jtexpand)
{
	...
	B*av;
	C*wv, *wx, *zv;
	I an, *au, i, k, p, q, r, wc, wk, wn, wt, zn;
	RZ(a && w);
	if(!(B01 & AT(a))) {
		RZ(a = cvt(B01, a));
	}
