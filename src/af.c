...
F1(jtunname)
{
	...
	RZ(w);
	v = VAV(w);
	if(CTILDE == v->id && !jt->glock && !(VLOCK & v->flag)) {
		x = v->f;
		if(NAME & AT(x)) {
			R symbrd(x);
		}
...
static F2(jtfixa)
{
	A aa, f, g, h, wf, x, y, z = w;
	V*v;
	RZ(a && w);
	if(NOUN & AT(w) || VFIX & VAV(w)->flag) {
		R w;
	}
	v = VAV(w);
	f = v->f;
...
F1(jtfix)
{
	...
	RZ(w);
	jt->rank = 0;
	RZ(jt->fxpath = reshape(sc(jt->fxi = (I)255), ace));
	jt->fxpv = AAV(jt->fxpath);
	if(LIT & AT(w)) {
