...
DC jtdeba(J jt, C t, A x, A y, A fs)
{
	A q;
	DC d;
	GA(q, LIT, sizeof(DST), 1, 0);
	d = (DC)AV(q);
	...
	case DCCALL:
		...
		d->dcf = fs;
		d->dca = jt->curname;
		d->dcm = NAV(jt->curname)->m;
		d->dcn = (I)jt->cursymb;
		d->dcstop = -2;
		if(jt->dbss == SSSTEPINTO) {
			d->dcss = SSSTEPINTO;
			jt->dbssd = d;
			jt->dbss = 0;
		}
...
static DC suspset(DC d)
{
	DC e;
	while(d && DCCALL != d->dctype) {
		e = d;    /* find topmost call                 */
		d = d->dclnk;
	}
	if(!(d && DCCALL == d->dctype)) {
		R 0;    /* don't suspend if no such call     */
	}
	if(d->dcc) {
		e->dcsusp = 1;    /* if explicit, set susp on line     */
	} else {
		d->dcsusp = 1;    /* if not explicit, set susp on call */
	}
	R d;
}    /* find topmost call and set suspension flag */

static B jterrcap(J jt)
{
	A y, *yv;
...
static A jtparseas(J jt, B as, A w)
{
	A*u, *v, y, z;
	I n;
	n = AN(w);
	...
	*u++ = mark;
	...
	if(as && z) {
		ASSERT(NOUN & AT(z) && all1(eq(one, z)), EVASSERT);
	}
	R z;
}
...
A jtparsex(J jt, A w, B lk, CW*ci, DC c)
{
	A z;
	B as, s;
	DC d, t = jt->sitop;
	...
	if(0 == c) {
		z = parseas(as, w);    /* anonymous or not debug */
	} else {                   /* named and debug        */
		if(s = dbstop(c, ci->source)) {
			z = 0;
			jsignal(EVSTOP);
		} else                      {
			z = parseas(as, w);
...
DF2(jtdbunquote)
{
	...
	if(CCOLON == sv->id && t && NOUN & AT(t)) { /* explicit */
		...
		while(0 == i) {
			...
			if(b = jt->dbalpha || jt->dbomega) {
				a = jt->dbalpha;
				w = jt->dbomega;
				jt->dbalpha = jt->dbomega = 0;
			}
			++i;
		}
	}
...
F1(jtdbrr1 )
{
	R dbrr(0L, w);   /* 13!:9   re-run with arg(s) */
}
F2(jtdbrr2 )
{
	R dbrr(a, w);
