...
{
	B*v;
	I ar, *as, at, c, d, i, q, r, r1, rc, m, wr, *ws, wt;
	UC k, p, *u;
	RZ(a && w);
	ar = AR(a);
	as = AS(a);
	...
	r1 = rc ? (BW - rc) / BB : 0;
	if(at & BIT && wt & BIT) {
		R memcmp(AV(a), AV(w), m * d * SZI) ? zero : one;
	}
	if(at & BIT) {
		u = UAV(a);
		v = BAV(w);
...
static UI bitmask(I c)
{
	...
	if(rc = c % BW) {
...
#if SYS & SYS_LILENDIAN
		{
			UC c, *v = (UC*)&mask;
			I j = SZI;
			DO(SZI / 2, --j; c = v[i]; v[i] = v[j]; v[j] = c;);
		}
#endif
	}
	R mask;
...
static F1(jtbitvfypad)
{
	I c, d, i, m, rc, wn, wr, *ws;
	UI mask, *u;
	RZ(w);
...
static F1(jtbitslplus)
{
	A t, z;
	I i, m, mm, n, n1, p, q, *u, wr, *ws, *zv;
	UC c, *vc, *wv;
	UI*v, *v0;
	static UI ptab[256];
	RZ(w);
	...
	if(!ptab[1]) {
		...
		UI x;
		v = (UC*)&x;
		DO(256, c = (UC)i; x = 0; DO(8, if(c & (UC)1)v[7 - i] = 1; c >>= 1;); ptab[i] = x;);
	}
#else
	mm = 15;
	if(!ptab[1]) {
	...
	for(i = (m + mm - 1) / mm - 1; i >= 0; --i) {
		...
		vc = (UC*)v0;
		u = zv;
		DO(n, c = *vc++; *u++ += c >> 4; *u++ += c & (UC)0x0f;);
	}
	R z;
}    /* +/ bit vector */

static F1(jtbitsland)
{
	I n;
	UI ff, *v;
	RZ(w);
...
static F1(jtbitscanne)
{
	...
	if(n % BW) {
	*(n / BW + (UI*)AV(z)) &= bitmask(n);
	}
	R z;
}    /* ~:/\ bit vector */

static F1(jtbitscaneq)
{
	A z;
	I n;
	UC c, d, p, *v, x, *zv;
...
{
	A z;
	I an, ar, *as, c, i, j, m, n, q, r, rc, r1, wr, *ws;
	UC k, *v, *zv;
	RZ(a && w);
	...
	if(1 >= wr) {
		...
		for(i = 0; i < m; ++i) {
			...
			if(r) {
				k = 0;
				DO(r,  j = *u++; if(0 > j)j += n; ASSERT(0 <= j && j < n, EVINDEX); if(v[j / BB] & (UC)128 >> j % BB)k |= bit[i];);
				*zv++ = k;
			}
			DO(r1, *zv++ = 0;);
		}
...
static F1(jtbitgrade)
{
	...
	s = AS(x);
	ICPY(s, AS(w), r);
	s[r - 1] = SZI * ((s[r - 1] + BW - 1) / BW);
	AN(x) = prod(r, s);
	R grade1(x);
}    /* /: bit array */

static F2(jtbitindexof)
{
	I j, n;
	UC*u, y;
	UI*v, x;
...
static F2(jtbitrepeat)
{
	A z;
	I c, c1, m, n, p, wr, wk, wt;
	UC*av, k;
	RZ(a && w);
	ASSERT(BIT & AT(a), EVDOMAIN);
	n = AN(a);
	...
	*AS(z) = p;
	if(BIT & wt && 1 >= wr) {
		...
		zv = UAV(z);
		j = 0;
		DO((n + BB - 1) / BB, k = *av++; c = *wv++; d = 0; DO(BB, if(k & bit[i]) {
		d |= k & bit[i] & c;
		}); *zv++ = d;);
	} else {
		I*wv, *zv;
		...
		case sizeof(I)*2:
			...
			break;
		case sizeof(I)*3:
			REPDO(I, u += 3, (*v++ = *u++, *v++ = *u++, *v++ = *u++));
			break;
		case sizeof(I)*4:
			REPDO(I, u += 4, (*v++ = *u++, *v++ = *u++, *v++ = *u++, *v++ = *u++));
			break;
		default:
			REPDO(C, u += wk, (memcpy(v, u, wk), u += wk, v += wk));
		}
	}
	R z;
}    /* bit vector # array */
...
F1(jtbitadv)
{
	...
	case 0:
		R BITDEF1(jtbitcvt);
	case 1:
		R BITDEF1(jtbitvfypad);
	case CNOT:
		R BITDEF1(jtbitnot);
	case CGRADE:
		R BITDEF1(jtbitgrade);

	case CCOMMA:
		R BITDEF(jtbitravel, jtbitcat);

	case CIOTA:
		R BITDEF2(jtbitindexof);
	case CPOUND:
		R BITDEF2(jtbitrepeat);
	case CPLUSDOT:
		R BITDEF2(jtbitor);
	case CPLUSCO:
		R BITDEF2(jtbitnor);
	...
	case CSLASH:
		...
		case CEQ:
			R BITDEF1(jtbitsleq);
		case CPLUS:
			R BITDEF1(jtbitslplus);
		case CPLUSDOT:
			R BITDEF1(jtbitslor);
		case CSTARDOT:
			R BITDEF1(jtbitsland);
		case CNE:
			R BITDEF1(jtbitslne);
		}
	case CBSLASH:
		x = v->f;
		ASSERT(CSLASH == ID(x), EVDOMAIN);
		x = VAV(x)->f;
		switch(ID(x)) {
		default:
			ASSERT(0, EVDOMAIN);
		case CSTARDOT:
			R BITDEF1(jtbitscanand);
		case CNE:
			R BITDEF1(jtbitscanne);
		case CEQ:
			R BITDEF1(jtbitscaneq);
		}
	}
}
