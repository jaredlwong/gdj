...
AHDRR(bw0011insI, UI, UI)
{
	I d = c / n, k = c - d;
	DO(m, DO(d, *z++ =  *x++;); x += k;);
}
static AHDRR(bw0011insC, UC, UC)
{
	I d = c / n, k = c - d;
	DO(m, DO(d, *z++ =  *x++;); x += k;);
}

AHDRR(bw1100insI, UI, UI)
{
	I d = c / n, k = c - d;
	if(1 < n) {
		DO(m, DO(d, *z++ = ~*x++;
...
AHDRR(bw0101insI, UI, UI)
{
	I d = c / n, k = c - d;
	x += k;
	DO(m, DO(d, *z++ =  *x++;); x += k;);
}
static AHDRR(bw0101insC, UC, UC)
{
	I d = c / n, k = c - d;
	x += k;
	DO(m, DO(d, *z++ =  *x++;); x += k;);
...
DF1(jtbitwise1)
{
	R CALL2(VAV(self)->f2, zero, w, self);
}
...
DF2(jtbitwisechar)
{
	...
	A*p, x, y, z;
	B b;
	I an, ar, *as, at, d, j, m, n, wn, wr, *ws, wt, zn;
	VF f;
	RZ(a && w);
	d = SZI;
	x = a;
	...
	ws = AS(w);
	wt = AT(a);
	if(!(an && wn && at & LIT && wt & LIT)) {
		R from(df2(indexof(alp, a), indexof(alp, w), fs), alp);
	}
	b = ar <= wr;
	zn = b ? wn : an;
	m = b ? an : wn;
	...
	if(1 == n)                 {
		...
		n = (n + d - 1) / d;
		p = b ? &x : &y;
		RZ(*p = irs2(sc(d), *p, 0L, 0L, 0L, jtrepeat));
	} else {
		f = bwC[j];
	}
...
B jtbitwisecharamp(J jt, UC*t, I n, UC*wv, UC*zv)
{
	I p;
	UC c, i, j, *pv, s[256];
	VF f;
	...
	pv = (UC*)&p;
	DO(SZI, pv[i] = c;);
	f(jt, 1, 1L, 256L / SZI, s, pv, AV(alp));
	if(memcmp(s, t, 256L)) {
		R 0;
	}
	f(jt, 1, 1L, (n + SZI - 1) / SZI, zv, pv, wv);
	zv[n] = 0;
	R 1;
}
...
DF1(jtbitwiseinsertchar)
{
	A fs, z;
	I c, d = SZI, j, m, n, r, wn, wr;
	UC*u, *v, *wv, x, *zv;
	VF f;
	RZ(w && self);
	wr = AR(w);
	...
	fs = VAV(z)->f;
	if(!(wn && d < n && LIT & AT(w))) {
		R from(df1(indexof(alp, w), fs), alp);
	}
	m = wn / n;
	wv = CAV(w);
	j = i0(VAV(fs)->f) - 16;
	...
	if(1 == wr) {
		...
		switch(j) {
		case 1:
			DO(d - 1, x = BW0001(x, *v); ++v;);
			DO(r, x = BW0001(x, *u); ++u;);
			break;
		case 6:
			DO(d - 1, x = BW0110(x, *v); ++v;);
			DO(r, x = BW0110(x, *u); ++u;);
			break;
		case 7:
			DO(d - 1, x = BW0111(x, *v); ++v;);
			DO(r, x = BW0111(x, *u); ++u;);
			break;
		case 9:
			DO(d - 1, x = BW1001(x, *v); ++v;);
			DO(r, x = BW1001(x, *u); ++u;);
			break;
		}
		*(I*)zv = 0;
