...
static B jtlp(J jt, A w)
{
	...
	c = *v;
	d = *(v + n - 1);
	if(1 == n || (2 == n || 3 >= n && ' ' == c) && (d == CESC1 || d == CESC2) || vnm(n, v)) {
		R 0;
	}
	if(C9 == ctype[c]) {
		DO(n - 1, d = c; c = ctype[*++v]; if(b = !NUMV(c) || d == CS && c != C9) { break; })
	} else if(c == q) {
		DO(n - 1, c = *v++; if(c == q) { p = !p; } if(b = p ? 0 : c != q) { break; })
	} else if(c == '(') {
		DO(n - 1, c = *v++; j += c == '(' ? 1 : c == ')' ? -1 : 0; if(b = !j) { break; })
	}
	R b;
}    /* 1 iff put parens around w */

static A jtlcpa(J jt, B b, A w)
{
	A z = w;
	C*zv;
	I n;
...
static A jtlcpb(J jt, B b, A w)
{
	...
	if(!b) {
		...
		p = 0;
		if (c == CQ) {
			DO(n - 1, c = ctype[*++v]; if(c == CQ) { p = !p; } else if(p) { b = 1; break; })
		} else if(c == C9) {
			DO(n - 1, c = ctype[*++v]; if(!(c == C9   || c == CS   )) { b = 1; break; })
		} else {
			DO(n - 1, c = *++v ; if(!(c == CESC1 || c == CESC2)) { b = 1; break; })
		}
	}
	if(b && vnm(n, wv)) {
		b = 0;
...
static A jtlcpx(J jt, A w)
{
	RZ(w);
	R CALL2(jt->lcp, lp(w), w, 0);
}
...
static F1(jtlsh)
{
	R over(thorn1(shape(w)), spellout(CDOLLAR));
}
...
static F1(jtlchar)
{
	...
	if(!p) {
		k = (UC)d;
		RZ(y = indexof(alp, w));
		if(r1 && n < m && (!k || k == m - n) && equ(y, apv(n, k, 1L))) {
			R over(thorn1(sc(d ? -n : n)), cstr("{.a."));
		}
		RZ(y = lnum(y));
		R lp(y) ? over(cstr("a.{~"), y) : over(y, cstr("{a."));
	}
...
static F1(jtlbox)
{
	...
	I n, wd;
	RZ(w);
	if(equ(ace, w) && B01 == AT(AAV0(w))) {
		R cstr("a:");
	}
	n = AN(w);
	wv = AAV(w);
	wd = (I)w * ARELATIVE(w);
	DO(n, x = WVR(i); if(BOX & AT(x)) {
	b = 1;
	break;
});
	b = b || 1 == n;
...
static F1(jtlnum)
{
	...
	if(7 < n || 1 < n && 1 < AR(w)) {
		...
		if(p) {
			...
			v[4] = spellout(CIOTA);
			v[5] = thorn1(p ? shape(w) : negate(shape(w)));
			RE(y);
			R raze(y);
		}
	}
...
static F1(jtlsparse)
{
	...
	be = !(AT(w)&SFL && 0 == *DAV(e));
	if(be) {
		RZ(z = over(lnoun(e), cstr(SB01 & AT(w) ? "" : SINT & AT(w) ? "+-~2" : SFL & AT(w) ? "+-~2.1" : "+-~2j1")));
	}
	if(be || ba) {
	...
	if(AN(a)) {
		...
		RZ(t = over(lcpx(lnoun(head(x))), cstr(" a:}")));
	}
	ba = 0;
	v = AV(a);
	DO(AN(a), if(i != *v++) {
...
static F1(jtlnoun)
{
	...
	case LIT:
		R lchar(w);
	case C2T:
		R over(cstr("u: "), lnum(uco2(num[3], w)));
	case BOX:
		R lbox(w);
	case SBT:
		R over(cstr("s: "), lbox(sb2(num[5], w)));
	default:
		R lnum(w);
	}
}

static A jtlsymb(J jt, C c, A w)
{
	A t;
	C buf[20], d, *s;
	I*u;
...
static B lnn(A a, A w)
{
	C c;
	RZ(a && w);
	c = cl(a);
	R ('x' == c || C9 == ctype[c])&&C9 == ctype[cf(w)];
}
...
static F2(jtlinsert)
{
	...
	if(1 <= n) {
		...
		t = v->f;
		c = ID(t);
		ft = c == CHOOK || c == CFORK || c == CADVF || id == CFORK && NOUN & AT(t) && lp(f);
	}
	if(2 <= n) {
		g = AVR(1);
		t = v->g;
		c = ID(t);
		gt = vb    ? c == CHOOK || c == CFORK : lp(g);
	}
	if(3 <= n) {
		h = AVR(2);
		t = v->h;
		c = ID(t);
		ht = vb && !b ? c == CHOOK          : lp(h);
	}
	...
	case CFORK:
		...
		RZ(u[2] = g = CALL2(jt->lcp, gt || lnn(g, h) || b, g, 0));
		RZ(u[1] = str(' ' == cf(g) ? 0L : 1L, " "));
		RZ(u[4] = h = CALL2(jt->lcp, ht,             h, 0));
		RZ(u[3] = str(' ' == cf(h) ? 0L : 1L, " "));
		R raze(y);
	default:
...
static F1(jtlrr)
{
	...
	if(!(VXOP & fl) && hs && BOX & AT(hs) && id == CCOLON) {
		R lcolon(w);
	}
	GA(t, BOX, m, 1, 0);
	tv = AAV(t);
	...
	if(1 < m) {
		RZ(tv[1] = fl & VGERR ? CALL1(jt->ltie, fxeach(gs), 0L) : lrr(gs));
	}
	if(0 < m) {
		RZ(tv[0] = fl & VGERL ? CALL1(jt->ltie, fxeach(fs), 0L) : lrr(fs));
	}
	R linsert(t, w);
}
