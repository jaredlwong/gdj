...
F1(jtshift1)
{
	R drop(num[-1], over(one, w));
}

static A jttk0(J jt, B b, A a, A w)
{
	A z;
	I k, m = 0, n, p, r, *s, *u;
	r = AR(w);
	...
	if(!b) {
		RE(m = prod(n, u));
		ASSERT(m > IMIN, EVLIMIT);
		RE(m = mult(ABS(m), prod(r - n, n + AS(w))));
	}
	GA(z, AT(w), m, r, AS(w));
	s = AS(z);
...
static F2(jttks)
{
	...
	if(c) {
		...
		SPB(zp, i, d < n ? take(sc(d), j) : j);
		SPB(zp, x, d < n ? take(sc(d), y) : y);
	} else {
...
F2(jttake)
{
	...
	jt->rank = 0;
	if(af || 1 < acr) {
		R rank2ex(a, w, 0L, af ? acr : 1L, wcr, jttake);
	}
	n = AN(a);
...
F2(jtdrop)
{
	...
	jt->rank = 0;
	if(af || 1 < acr) {
		R rank2ex(a, w, 0L, af ? acr : 1L, wcr, jtdrop);
	}
	n = AN(a);
	u = AV(a);
	ASSERT(!wcr || n <= wcr, EVLENGTH);
	if(wcr) {
		RZ(s = shape(w));
		v = wf + AV(s);
		DO(n, d = u[i]; m = v[i]; v[i] = d < -m ? 0 : d < 0 ? d + m : d < m ? d - m : 0;);
	} else {
		GA(s, INT, wr + n, 1, 0);
		v = AV(s);
		ICPY(v, AS(w), wf);
...
static F1(jtrsh0)
{
	...
	ICPY(wf + AV(x), ws + wf + 1, wcr - 1);
	RZ(w = setfv(w, w));
	GA(y, AT(w), 1, 0, 0);
	MC(AV(y), jt->fillv, bp(AT(w)));
	R reshape(x, y);
}
