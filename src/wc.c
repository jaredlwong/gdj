...
static A jtcongotoblk(J jt, I n, CW*con)
{
	A z;
	CW*d = con;
	I i, j, k, *u, *v;
...
static I jtcongotochk(J jt, I i, I j, A x)
{
	I k, n, *v;
	n = *AS(x);
	v = AV(x);
	for(k = 0; k < n; ++k, v += 2)if(v[0] <= j && j <= v[1] && !(v[0] <= i && i <= v[1])) {
			R i;
		}
	R - 1;
...
static I jtcongoto(J jt, I n, CW*con, A*lv)
{
	A x, z;
	C*s;
	CW*d = con, *e;
...
static I conend(I i, I j, I k, CW*b, CW*c, CW*d, I p, I q, I r)
{
	I e, m, t;
	e = 1 + i;
	CWASSERT(c);
	...
	case CWCASE(CIF, CDO):
		break;
	case CWCASE(CELSEIF, CDO):
		CWASSERT(d);
		d->go = (US)e;
		break;
	case CWCASE(CWHILST, CDO):
		CWASSERT(d);
		d->go = (US)(1 + j);
	case CWCASE(CWHILE, CDO):
		CWASSERT(b && d);
		b->go = (US)(1 + k);
		m = i - k - 1; /* break. and continue. */
...
static I conendtry(I e, I top, I stack[], CW*con)
{
	CW*v;
	I c[3], d[4], i = -1, j, k = 0, m, t = 0;
	US ii;
	c[0] = c[1] = c[2] = -1;
	d[k++] = e;
	while(top && t != CTRY) {
	...
	m = k;
	DO(k - 1, --m; (d[m] + con)->go = (US)d[m - 1];); /* catchx. */
	(e + con)->go = (US)(1 + e);                    /* end.    */
	m = d[k - 1];
	if     (0 <= c[0]) {
		ii = (US)(1 + c[0]);
		v = j + con;
		DO(m - j - 1, ++v; if(SMAX == v->go && CBREAK != v->type)v->go = ii;);
	} else if(0 <= c[1]) {
		ii = (US)(1 + c[1]);
		v = j + con;
		DO(m - j - 1, ++v; if(SMAX == v->go && CBREAK != v->type)v->go = ii;);
...
static I conendsel(I i, I top, I stack[], CW*con)
{
	I c = i - 1, d = 0, j, ot = top, t;
	while(1) {
		j = stack[--top];
		t = (j + con)->type;
		if(t == CSELECT) {
		...
		if(t == CDOSEL) {
			...
			if(t == CFCASE && top < ot - 2) {
				(stack[2 + top] + con)->go = (US)(1 + stack[3 + top]);
			}
		}
	}
	(c + con)->go = (US)(1 + c);
...
static I jtconall(J jt, I n, CW*con)
{
	A y;
	CW*b = 0, *c = 0, *d = 0;
	I e, i, j, k, p = 0, q, r, *stack, tb = 0, top = 0, wb = 0;
	GA(y, INT, n, 1, 0);
	stack = AV(y);
	for(i = 0; i < n; ++i) {
		q = r = 0;
		...
		case CEND:
			...
			case CCATCHT:
				CWASSERT(1 <= top);
				top = conendtry(i, top, stack, con);
				CWASSERT(0 <= top);
				break;
			default:
				top -= 2;
				if(r == CWHILE || r == CWHILST || r == CFOR) {
					--wb;
				}
...
static I jtconword(J jt, I n, C*s)
{
	if(2 < n && '.' == *(s + n - 1))switch(*s) {
		case 'a':
			if(!strncmp(s, "assert.",  n)) {
				R CASSERT;
			}
			break;
		case 'b':
			if(!strncmp(s, "break.",   n)) {
				R CBREAK;
			}
			break;
		case 'c':
			...
			if(!strncmp(s, "continue.", n)) {
				R CCONT;
			}
			if(!strncmp(s, "catch.",   n)) {
				R CCATCH;
			}
			if(!strncmp(s, "catchd.",  n)) {
				R CCATCHD;
			}
			if(!strncmp(s, "catcht.",  n)) {
				R CCATCHT;
			}
			break;
		case 'd':
		...
		case 'f':
			...
			if(!strncmp(s, "for_",    4L)) {
				ASSERTN(vnm(n - 5, 4 + s), EVILNAME, nfs(n - 5, 4 + s));
				R CFOR;
			}
			if(!strncmp(s, "fcase.",   n)) {
				R CFCASE;
			}
			break;
		case 'g':
			if(!strncmp(s, "goto_",   5L)) {
				R CGOTO;
			}
			break;
		case 'i':
			...
			break;
		case 'l':
			if(!strncmp(s, "label_",  6L)) {
				R CLABEL;
			}
			break;
		case 'r':
			if(!strncmp(s, "return.",  n)) {
				R CRETURN;
			}
			break;
		case 's':
			if(!strncmp(s, "select.",  n)) {
				R CSELECT;
			}
			break;
		case 't':
			if(!strncmp(s, "throw.",   n)) {
				R CTHROW;
			}
			if(!strncmp(s, "try.",     n)) {
				R CTRY;
			...
			break;
		case 'w':
			if(!strncmp(s, "while.",   n)) {
				R CWHILE;
			}
			if(!strncmp(s, "whilst.",  n)) {
				R CWHILST;
			}
			break;
		}
...
static F1(jtgetsen)
{
	...
	GA(z, BOX, n / 2, 1, 0);
	z0 = zv = AAV(z); /* list of ctrls & sentences  */
	s = CAV(w);                       /* text of entire string      */
	for(i = 0; i < n; i += 2) {
		j = v[i];
		m = v[1 + i];            /* index & length of this word*/
		if(0 > k) {
			k = j;    /* index of sentence          */
		}
		if(conword(m, j + s)) {
			if(k < j) {
				RZ(*zv++ = str(j - k, k + s));    /* emit sentence in progress  */
			}
	...
	if(0 <= k) {
		RZ(*zv++ = str(j + m - k, k + s));    /* emit sentence if any       */
	}
	R vec(BOX, zv - z0, z0);
...
B jtpreparse(J jt, A w, A*zl, A*zc)
{
	PROLOG;
	A c, l, *lv, *v, w0, w1, *wv, x, y;
	B b = 0, try = 0;
	C*s;
	CW*d, *cv;
	I as = 0, i, j, k, m, n, p, q, yn;
	RZ(w);
	p = AN(w);
	wv = AAV(w);
	...
	for(i = 0; i < p; ++i) {
		...
		for(j = 0; j < yn; ++j) {
			...
			if(!k) {
				...
				x = k == CLABEL || k == CGOTO || k == CFOR && 4 < AN(w0) ? w0 : 0L;
			}
			q = k ? 1 && x : AN(x);
