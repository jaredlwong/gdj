...
static I bsize(B d, B tb, I t, I n, I r, I*s)
{
	I c, k, m, w, z;
	w = WS(d);
	z = BH(d) + w * r;
...
static B jtmvw(J jt, C*v, C*u, I n, B bv, B bu, B dv, B du)
{
	C c;
	...
	case MVCS(0, 0, 1, 1):
		...
		break;
	case MVCS(0, 1, 0, 0):
		DO(n, c = 0 > u[0] ? CFF : C0; DO(4, ASSERT(c == u[  i], EVLIMIT); v[i] = u[4 + i];); v += 4; u += 8;);
		break;
	case MVCS(0, 1, 0, 1):
		DO(n, c = 0 > u[7] ? CFF : C0; DO(4, ASSERT(c == u[7 - i], EVLIMIT); v[i] = u[3 - i];); v += 4; u += 8;);
		break;
	case MVCS(0, 1, 1, 0):
		DO(n, c = 0 > u[0] ? CFF : C0; DO(4, ASSERT(c == u[3 - i], EVLIMIT); v[i] = u[7 - i];); v += 4; u += 8;);
		break;
	case MVCS(0, 1, 1, 1):
		DO(n, c = 0 > u[7] ? CFF : C0; DO(4, ASSERT(c == u[4 + i], EVLIMIT); v[i] = u[  i];); v += 4; u += 8;);
		break;
	case MVCS(1, 0, 0, 0):
		DO(n, c = 0 > u[0] ? CFF : C0; DO(4, v[  i] = c; v[4 + i] = u[i];); v += 8; u += 4;);
		break;
	case MVCS(1, 0, 0, 1):
		DO(n, c = 0 > u[3] ? CFF : C0; DO(4, v[3 - i] = c; v[7 - i] = u[i];); v += 8; u += 4;);
		break;
	case MVCS(1, 0, 1, 0):
		DO(n, c = 0 > u[0] ? CFF : C0; DO(4, v[7 - i] = c; v[3 - i] = u[i];); v += 8; u += 4;);
		break;
	case MVCS(1, 0, 1, 1):
		DO(n, c = 0 > u[3] ? CFF : C0; DO(4, v[4 + i] = c; v[  i] = u[i];); v += 8; u += 4;);
		break;
	case MVCS(1, 1, 0, 0):
		MC(v, u, n * 8);
		break;
	case MVCS(1, 1, 0, 1):
		DO(n, DO(8, v[7 - i] = u[i];); v += 8; u += 8;);
		break;
	case MVCS(1, 1, 1, 0):
		DO(n, DO(8, v[7 - i] = u[i];); v += 8; u += 8;);
		break;
...
static C*jtbrephdr(J jt, B b, B d, A w, A y)
{
	A q;
	I f, r;
	q = (A)AV(y);
	r = AR(w);
	f = 0;
	RZ(mvw(BF(d, q), (C*)&f,    1L, b, BU, d, SY_64));
	*CAV(y) = d ? (b ? 0xe3 : 0xe2) : (b ? 0xe1 : 0xe0);
...
static A jtbreps(J jt, B b, B d, A w)
{
	A q, y, z, *zv;
	C*v;
	I c = 0, kk, m, n;
	...
	zv[0] = y;
	m = AN(y);
	RZ(zv[1] = q = brep(b, d, SPA(wp, a)));
	RZ(mvw(v +  kk, (C*)&m, 1L, b, BU, d, SY_64));
	m += AN(q);
	RZ(zv[2] = q = brep(b, d, SPA(wp, e)));
	RZ(mvw(v + 2 * kk, (C*)&m, 1L, b, BU, d, SY_64));
	m += AN(q);
	RZ(zv[3] = q = brep(b, d, SPA(wp, i)));
	RZ(mvw(v + 3 * kk, (C*)&m, 1L, b, BU, d, SY_64));
	m += AN(q);
	RZ(zv[4] = q = brep(b, d, SPA(wp, x)));
	RZ(mvw(v + 4 * kk, (C*)&m, 1L, b, BU, d, SY_64));
	R raze(z);
}    /* 3!:1 w for sparse w */

A jtbrep(J jt, B b, B d, A w)
{
	A q, *wv, y, z, *zv;
	C*u, *v;
	I e, k, kk, m, n, t, wd;
	...
	if(t & DIRECT)switch(t) {
		case SBT:
		case INT:
			RZ(mvw(v, u, n,  b, BU, d, SY_64));
			R y;
		case FL:
			RZ(mvw(v, u, n,  b, BU, 1, 1    ));
			R y;
		case CMPX:
			RZ(mvw(v, u, n + n, b, BU, 1, 1    ));
			R y;
		default:
			if(n) {
				int*u = (int*)v + (n * k - 1) / sizeof(int);
				*u++ = 0;
				*u = 0;
			}
			MC(v, u, n * k);
...
static A jtunbinr(J jt, B b, B d, B pre601, I m, A w)
{
	A y, z;
	C*u = (C*)w, *v;
	...
	p = bsize(d, 0, t, n, r, 0L);
	e = t & RAT ? n + n : t & SPARSE ? 1 + sizeof(P) / SZI : n;
	ASSERT(m >= p, EVLENGTH);
	GA(z, t, n, r, 0);
	s = AS(z);
	...
	if(t & BOX + XNUM + RAT) {
		...
		for(i = 0; i < e; ++i) {
			...
			if(i > iv[i]) {
				...
				zv[i] = unbinr(b, d, pre601, k < e ? vv[k] - j : m - j, (A)(u + j));
			}
		}
	} else if(t & SPARSE) {
		P*zp = PAV(z);
		...
		case SBT:
		case INT:
			RZ(mvw(CAV(z), v, n,  BU, b, SY_64, d));
			break;
		case FL:
			RZ(mvw(CAV(z), v, n,  BU, b, 1,    1));
			break;
		case CMPX:
			RZ(mvw(CAV(z), v, n + n, BU, b, 1,    1));
			break;
		default:
			e = n * bp(t);
			ASSERTSYS(e <= AM(z), "unbinr");
			MC(CAV(z), v, e);
		}
	RE(z);
	R z;
...
F1(jtunbin)
{
	...
	b = 0;
	if(!mvw((C*)&t, BTX(d, 1, q), 1L, BU, 0, SY_64, d)) {
		RESETERR;    /* detect reverse bytes */
		b = 1;
	}
	if(!mvw((C*)&n, BN(d, q), 1L, BU, 0, SY_64, d)) {
		RESETERR;
		b = 1;
	}
	if(!mvw((C*)&r, BR(d, q), 1L, BU, 0, SY_64, d)) {
		RESETERR;
		b = 1;
	}
	...
	if(t & DENSE) {
		...
		c = 1;
		for(i = 0; !b && i < r; ++i) {
			if(!mvw((C*)&k, v, 1L, BU, 0, SY_64, d)) {
				RESETERR;
				b = 1;
			}
			v += WS(d);
			c *= k;
			if(!(0 <= k && (!n || 0 <= c && k <= n && c <= n))) {
				b = 1;
			}
		}
		b = b || n != c;
...
static B jtisnanq(J jt, A w)
{
	...
	I m, n, t, top, yd;
	RZ(w);
	GA(x, INT, BOX & AT(w) ? 2 * AN(w) : 1, 1, 0);
	xv = AAV(x);
	*xv = w;
	top = 1;
	while(top) {
		...
		if(t & FL + CMPX) {
			v = DAV(y);
			DO(t & CMPX ? n + n : n, if(_isnan(*v++))R 1;);
		} else if(t & BOX) {
			m = top + n;
			yv = AAV(y);
			yd = (I)y * ARELATIVE(y);
			...
			u = xv + top;
			DO(n, q = YVR(i); if(AT(q)&FL + CMPX + BOX)*u++ = q;);
			top = u - xv;
		}
	}
	R 0;
...
F1(jtisnan)
{
	...
	if     (t & FL  ) {
		...
		v = DAV(w);
		DO(n, *u++ = _isnan(*v) || _isnan(*(v + 1)); v += 2;);
	} else if(t & BOX ) {
		wv = AAV(w);
		wd = (I)w * ARELATIVE(w);
		DO(n, *u++ = isnanq(WVR(i)););
