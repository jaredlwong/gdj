...
static void mtow(UC* src, I srcn, US* snk)
{
	US c, c1, c2;
	while(srcn--) {
		c = *src++;
...
static I mtowsize(UC* src, I srcn)
{
	US c, c1, c2, cf;
	I r = 0;
	while(srcn--) {
...
static I wtomsize(US* src, I srcn)
{
	US w;
	I r = 0;
	while(srcn--) {
...
F1(jttoutf16)
{
	...
	if(LIT & t) {
		...
		if(!b) {
			if(1 == AR(w)) {
				R ca(w);   // ascii list unchanged ascii scalar as list
			};
			GA(z, LIT, 1, 1, 0);
			*CAV(z) = *CAV(w);
			R z;
...
void jttoutf8x(J jt, C* f, I n, US* fw)
{
	I q;
