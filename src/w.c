...
static A jtconstr(J jt, I n, C*s)
{
	A z;
	C b, c, p, *t, *x;
	I m = 0;
	p = 0;
	t = s;
	DO(n - 2, c = *++t; b = c == CQUOTE; if(!b || p)m++;    p = b && !p;);
	if(0 == m) {
		R aqq;
	} else if(1 == m && (z = chr[(UC)s[1]])) {
		R z;
	}
	GA(z, LIT, m, 1 != m, 0);
	x = CAV(z);
	p = 0;
	t = s;
	DO(n - 2, c = *++t; b = c == CQUOTE; if(!b || p)*x++ = c; p = b && !p;);
	R z;
}
...
F2(jtenqueue)
{
		...
		if(128 > c && (y = ds(e))) {
			*x++ = y;
				...
				case CQ:
					...
					break;
				case CA:
					ASSERTN(vnm(wl, wi), EVILNAME, nfs(wl, wi));
					RZ(*x++ = nfs(wl, wi));
				}
	}
	if(6 <= n && TAIA(0, n - 1) && ((b = TRBRACE(n - 2)) || RPAR & AT(v[n - 2]) && TRBRACE(n - 3))) {
		I c, j, p, q;
		...
		} else if(7 <= n && !(AT(v[3])&VERB + ADV + CONJ)) {
			I c, j, q;
			/* abc=:abc xyz}~ pqr */
			j = 3;
			q = 0;
			DO(n - j, y = v[j++]; c = AT(y); if(c & LPAR)++q; else if(c & RPAR)--q; if(!q)break;);
			if(!q && j < n - 2 && TRBRACE(j) && v[1 + j] == ds(CTILDE)) {
				v[j] = ds(CAMIP);
			}
		}
	} else if(7 <= n && TNAME(0) && TASGN(1) && TNAME(2) && TRBRACE(3) && TVERB(n - 2, CLAMIN)) {
		A p, *yv, z1;
		I c, j, k, m;
		/* abc=: pqr}x,y,:z */
		b = 1;
	...
	j += 2;);
		if(b) {
			...
			s = NAV(v[0])->s;
			j = 4;
			DO(m, yv[i] = p = v[j]; j += 2; if(AN(p) && !memcmp(s, NAV(p)->s, k))c = i;);
			yv[m] = v[2];
			RZ(yv[m + 1] = sc(c));
			yv[m + 2] = z;
			x[0] = v[0];
...
static A jtfsmdo(J jt, I f, A s, A m, I*ijrd, A w, A w0)
{
	A x, z;
	C*cc, *wv0;
	...
	case  0: {
		B *wv = BAV(w);
		FSMF(A, BOX, 1, 1,   wv[i] , EMIT0b, ZVAx);
	}
	break;
	case  1: {
		B *wv = UAV(w);
		FSMF(B, B01, 1, 1,   wv[i] , EMIT1, ZVAx);
	}
	break;
	case  2: {
		B *wv = BAV(w);
		FSMF(I, INT, 2, 2,   wv[i] , EMIT2, ZVAx);
	}
	break;
	case  3: {
		B *wv = BAV(w);
		FSMF(I, INT, 1, 1,   wv[i] , EMIT3, ZVAx);
	}
	break;
	case  4: {
		B *wv = BAV(w);
		FSMF(I, INT, 2, 3,   wv[i] , EMIT4, ZVAx);
	}
	break;
	case  5: {
		B *wv = BAV(w);
		FSMF(I, INT, 2, 6,   wv[i] , EMIT5, ZVA5);
	}
	break;

	case 10: {
		UC*wv = UAV(w);
		FSMF(A, BOX, 1, 1, mv[wv[i]], EMIT0c, ZVAx);
	}
	break;
	case 11: {
		UC*wv = UAV(w);
		FSMF(C, LIT, 1, 1, mv[wv[i]], EMIT1, ZVAx);
	}
	break;
	case 12: {
		UC*wv = UAV(w);
		FSMF(I, INT, 2, 2, mv[wv[i]], EMIT2, ZVAx);
	}
	break;
	case 13: {
		UC*wv = UAV(w);
		FSMF(I, INT, 1, 1, mv[wv[i]], EMIT3, ZVAx);
	}
	break;
	case 14: {
		UC*wv = UAV(w);
		FSMF(I, INT, 2, 3, mv[wv[i]], EMIT4, ZVAx);
	}
	break;
	case 15: {
		UC*wv = UAV(w);
		FSMF(I, INT, 2, 6, mv[wv[i]], EMIT5, ZVA5);
	}
	break;

	case 20: {
		I *wv = AV(w);
		FSMF(A, BOX, 1, 1,   wv[i] , EMIT0x, ZVAx);
	}
	break;
	case 21: {
		I *wv = AV(w);
		FSMF(C, t0, wr, wm,  wv[i] , EMIT1x, ZVAx);
	}
	break;
	case 22: {
		I *wv = AV(w);
...
F1(jtfsmvfya)
{
	PROLOG;
	A a, *av, m, s, x, z, *zv;
	I ad, an, c, e, f, ijrd[4], k, p, q, *sv, *v;
	RZ(a = w);
	...
	p = v[0];
	q = v[1];
	ASSERT(2 == v[2], EVLENGTH);
	v = sv;
	DO(p * q, k = *v++; e = *v++; ASSERT(0 <= k && k < p && 0 <= e && e <= 6, EVINDEX););
	ijrd[0] = 0;
	ijrd[1] = -1;
	ijrd[2] = 0;
	ijrd[3] = -1;
	if(4 == an) {
		...
		if(2 <= n) {
			ijrd[1] = j = *v++;
			ASSERT(j == -1 || 0 <= j && j < i, EVINDEX);
		}
		if(3 <= n) {
			ijrd[2] = r = *v++;
			ASSERT(       0 <= r && r < p, EVINDEX);
		}
		if(4 == n) {
			ijrd[3] = d = *v++;
			ASSERT(d == -1 || 0 <= d && d < q, EVINDEX);
		}
	}
	m = 2 == an ? mtv : AVR(2);
	c = AN(m);
	...
	if(!c && 1 == AR(m)) {
		/* m is empty; w must be integer vector */
	} else if(NUMERIC & AT(m)) {
		ASSERT(c == AN(alp), EVLENGTH);
		RZ(m = vi(m));
		v = AV(m);
		DO(c, k = v[i]; ASSERT(0 <= k && k < q, EVINDEX););
	} else {
	...
	zv[1] = s;
	zv[2] = m;
	RZ(zv[3] = vec(INT, 4L, ijrd));
	R z;
}    /* check left argument of x;:y */

static A jtfsm0(J jt, A a, A w, C chka)
{
	PROLOG;
	A*av, m, s, x, w0 = w;
	B b;
	I ad, c, f, *ijrd, k, md, n, p, q, *v;
	RZ(a && w);
	if(chka) {
		RZ(a = fsmvfya(a));
	}
	...
	if(!c && 1 == AR(m)) {
		...
		if(!(B01 & AT(w))) {
			RZ(w = w0 = vi(w));
			v = AV(w);
			DO(n, k = v[i]; ASSERT(0 <= k && k < q, EVINDEX););
		}
	} else if(NUMERIC & AT(m)) {
		ASSERT(b, EVDOMAIN);
		ASSERT(1 >= AR(w), EVRANK);
		...
		mv = AAV(m);
		md = (I)m * ARELATIVE(m);
		DO(c, j = i; t = AADR(md, mv[i]); if(r && r == AR(t))DO(*AS(t), *v++ = j;) else {
			*v++ = j;
		});
		if(b) {
			RZ(m = from(indexof(y, alp), x));
			v = AV(m);
			DO(AN(alp), k = v[i]; ASSERT(0 <= k && k < q, EVINDEX););
		} else {
			ASSERT(q > c, EVINDEX);
			RZ(w = from(indexof(y, w), x));
		}
	}
