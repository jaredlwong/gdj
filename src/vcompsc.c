...
static F2( jti0eps)
{
	R indexofsub( II0EPS, w, a);
}
static F2( jti1eps)
{
	R indexofsub( II1EPS, w, a);
}
static F2( jtj0eps)
{
	R indexofsub( IJ0EPS, w, a);
}
static F2( jtj1eps)
{
	R indexofsub( IJ1EPS, w, a);
}
static F2(jtsumeps)
{
	R indexofsub(ISUMEPS, w, a);
}
static F2(jtanyeps)
{
	R indexofsub(IANYEPS, w, a);
}
static F2(jtalleps)
{
	R indexofsub(IALLEPS, w, a);
}
static F2(jtifbeps)
{
	R indexofsub(IIFBEPS, w, a);
}
...
AF jtatcompf(J jt, A a, A w, A self)
{
	AF f;
	I ar, at, m, wr, wt;
	RZ(a && w);
	...
	m = VAV(self)->flag % 256;
	if(1 < ar || 1 < wr) {
		if(32 <= m && m <= 37 || 40 <= m && m <= 45 || 48 <= m && m <= 53) {
			R(AF)jtfslashatg;
		}
		RZ(7 == m % 8);
	}
	...
	f = atcompX[m];
	if(!f) {
		if(at & B01 + INT + FL && wt & B01 + INT + FL) {
			f = atcompxy[9 * m + 3 * (at & B01 ? 0 : at & INT ? 1 : 2) + (wt & B01 ? 0 : wt & INT ? 1 : 2)];
		} else if(at & LIT && wt & LIT) {
			f = atcompC[m];
		} else if(at & SBT && wt & SBT) {
			f = atcompSB[m];
		}
	}
	R f;
