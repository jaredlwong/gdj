...
static B jtevinit(J jt)
{
	...
	jt->evm = q;
	if(jt->jerr) {
		printf("evinit failed; error %hhi\n", jt->jerr);
		R 0;
	} else {
		R 1;
	}
...
#if (SYS & SYS_DOS)
	t = EM_ZERODIVIDE + EM_INVALID;
	_controlfp(t, t);
#endif
#if (SYS & SYS_OS2)
	t = EM_ZERODIVIDE + EM_INVALID + EM_OVERFLOW + EM_UNDERFLOW;
	_control87(t, t);
#endif
#if (SYS & SYS_FREEBSD)
	fpsetmask(0);
...
C jtjinit2(J jt, int dummy0, C**dummy1)
{
	jt->sesm = 1;
	R jinit3();
}
