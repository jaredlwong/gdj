...
static D jtsummag(J jt, A w)
{
	A t = aslash(CPLUS, mag(w));
	R t ? *DAV(t) : 0.0;
}
...
static Z jtnewt(J jt, I m, Z*a, Z x, I n)
{
	I i, j;
	D e = EPS / 1024.0;
	Z c, p, q, *v;
...
static B jtdeflateq(J jt, B k, I m, Q*v, Q x)
{
	Q q, r, *u;
	u = v + m;
	q = *u--;
	DO(m, r = *u--;       q = qplus(r, qtymes(q, x)););
	RE(0);
	RZ(QEQ(q, zeroQ));
	u = v + m;
	q = *u--;
	DO(m, r = *u; *u-- = q; q = qplus(r, qtymes(q, x)););
	R 1;
...
static void jtdeflate(J jt, B k, I m, Z*v, Z x)
{
	if(k) {
		...
		v += m;
		q = *v--;
		DO(m, r = *v; *v-- = q; q = zplus(r, ztymes(q, x)););
	} else {
		D a, b, d, p, q, r;
		a = 2 * x.re;
		b = -(x.re * x.re + x.im * x.im);
		v += m;
		p = v--->re;
		q = v--->re;
...
static Z jtlaguerre(J jt, I m, Z*a, Z x)
{
	D ax, e;
	I i, j;
	Z b, c, d, g, g2, h, p, q, s, sq, y, zm, zm1;
	zm = zrj0((D)m);
	zm1 = zrj0((D)m - 1);
	for(i = 0;; ++i) {
		...
		if(zmag(zminus(x, y)) <= EPS * zmag(x)) {
			R x;
		}
	}
}   /* Press et al., "Numerical Recipes in C" */

static Q jtmultiple(J jt, D x, Q m)
{
	A y;
	Q q1, q2, q1r2;
	q1r2.n = xone;
	q1r2.d = xplus(xone, xone);
	QRE(y = cvt(RAT, scf(x)));
...
static Q jtmaxdenom(J jt, I n, Q*v)
{
	Q z;
	X*u, x, y;
	u = 1 + (X*)v;
...
static B jtrfcq(J jt, I m, A w, A*zz, A*ww)
{
	A q, x, y, z;
	B b;
	I i, j, wt;
	...
	xv = ZAV(x);
	RZ(y = take(sc(1 + m), x));
	yv = ZAV(y); /* deflated complex  poly */
	RZ(q = take(sc(1 + m), w));
	qv = QAV(q); /* deflated rational poly */
	GA(z, RAT, m, 1, 0);
	zv = QAV(z);      /* exact rational roots   */
	i = j = 0;
	while(i < m) {
...
static F1(jtrfc)
{
	...
	case 0:
		R link(zero, mtv);
	case 1:
		r = ravel(negate(aslash(CDIV, take(num[2], w))));
		break;
	default:
		if(t & CMPX) {
			r = rfcz(m, w);
...
F2(jtpoly2)
{
	...
	RZ(a && w);
	if(1 < AR(a)) {
		R rank2ex(a, w, 0L, 1L, 0L, jtpoly2);
	}
	an = AN(a);
	at = AT(a);
	b = 1 && BOX & at;
	...
	if(t & FL + CMPX) {
		...
		if(!j) {
			DO(t & FL ? wn : wn + wn, x = wd[i]; if(x == inf || x == infm) { j = 1; break; });
		}
	}
	if(!j && !(t & XNUM + RAT)) {
	...
	case 0:
		R tymes(c, df2(negate(a), w, eval("*/@(+/)")));
	case 1:
		DO(wn, p = d; x = *wd++; DO(an, p *= x - ad[i];); *zd++ = p;);
		break;
	case 2:
		DO(wn, q = e; y = *wz++; DO(an, q = ztymes(q, zminus(y, az[i]));); *zz++ = q;);
		break;
	case 3:
		R df2(w, a, eval("(^/i.@#) +/ .* ]"));
	case 4:
		DO(wn, p = d; x = *wd++; j = an; DO(an, p = ad[--j] + x * p;); *zd++ = p;);
		break;
	case 5:
		DO(wn, q = e; y = *wz++; j = an; DO(an, q = zplus(az[--j], ztymes(y, q));); *zz++ = q;);
	}
	R z;
}    /* a p. w */
