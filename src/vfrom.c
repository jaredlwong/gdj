...
F1(jtcatalog)
{
	...
	A b, *wv, x, z, *zv;
	C*bu, *bv, **pv;
	I*cv, i, j, k, m = 1, n, p, *qv, r = 0, *s, t = 0, *u, wd;
	...
	wv = AAV(w);
	wd = (I)w * ARELATIVE(w);
	DO(n, x = WVR(i); if(AN(x)) {
	p = AT(x);
		t = t ? t : p;
		ASSERT(HOMO(t, p), EVDOMAIN);
		RE(t = maxtype(t, p));
	});
	RE(t = maxtype(B01, t));
	k = bp(t);
...
F2(jtifrom)
{
	A z;
	C*wv, *zv;
	I acr, an, ar, *av, j, k, m, p, pq, q, *s, wcn, wcr, wf, wk, wn, wr, *ws, zn;
	RZ(a && w);
	ar = AR(a);
	...
	jt->rank = 0;
	if(ar > acr) {
		R rank2ex(a, w, 0L, acr, wcr, jtifrom);
	}
	an = AN(a);
	wn = AN(w);
	ws = AS(w);
	...
	if(AT(w)&FL + CMPX) {
		...
		default:
			if     (0 == k % sizeof(I))IFROMLOOP2(I, k / sizeof(I))
						...
						if(1 == an) {
							wv += k * j;
							DO(m,                     x = (S*)zv; u = (S*) wv;      DO(q, *x++ = *u++;); zv += k;   wv += wk;);
						} else {
							DO(m, DO(an, SETJ(av[i]);
						}
						   x = (S*)zv;
						u = (S*)(wv + k * j);
						DO(q, *x++ = *u++;);
...
static F2(jtbfrom)
{
	...
	B*av, *b;
	C*wv, *zv;
	I acr, an, ar, k, m, p, q, r, *s, *u = 0, wcn, wcr, wf, wk, wn, wr, *ws, zn;
	RZ(a && w);
	ar = AR(a);
	acr = jt->rank ? jt->rank[0] : ar;
	wr = AR(w);
	...
	jt->rank = 0;
	if(ar > acr) {
		R rank2ex(a, w, 0L, acr, wcr, jtbfrom);
	}
	an = AN(a);
	wn = AN(w);
	ws = AS(w);
	...
	default:
		if(1 == an) {
			wv += k**av;
			DO(m, MC(zv, wv, k); zv += k; wv += wk;);
		}
#if !SY_64 && SY_WIN32
		else {
			A x;
			C*v, *xv, *xv00, *xv01, *xv02, *xv03, *xv04, *xv05, *xv06, *xv07, *xv08, *xv09, *xv10, *xv11,
			*xv12, *xv13, *xv14, *xv15;
			I i, j, k4 = k * 4;
			GA(x, LIT, 16 * k4, 1, 0);
			xv = CAV(x);
...
A jtfrombu(J jt, A a, A w, I wf)
{
	A p, q, z;
	B b = 0;
	I ar, *as, h, m, r, *u, *v, wcr, wr, *ws;
	ar = AR(a);
	as = AS(a);
	h = as[ar - 1];
	wr = AR(w);
	...
	if(r == wr) {
		...
		*(v + wf) = m;
		...
		ICPY(v, ws, wf);
		*(v + wf) = m;
		ICPY(v + wf + 1, ws + wf + h, wcr - h); /* q is reshape(.,w) */
		z = irs2(pdt(a, p), q, 0L, RMAX, wcr + 1 - h, jtifrom);
	}
	R z;
}    /* (<"1 a){"r w, dense w, integer array a */

B jtaindex(J jt, A a, A w, I wf, A*ind)
{
	A*av, q, z;
	I ad, an, ar, c, j, k, t, *u, *v, *ws;
	RZ(a && w);
	an = AN(a);
	*ind = 0;
	if(!an) {
...
static B jtaindex1(J jt, A a, A w, I wf, A*ind)
{
	A z;
	I c, k, n, t, *v, *ws;
	RZ(a && w);
...
static A jtafrom2(J jt, A p, A q, A w, I r)
{
	A z;
	C*wv, *zv;
	I c, d, e, j, k, m, pn, pr, *pv,
	...
	default:        {
		C*v = wv, *x = zv - k;
		I n = c * wk;
		DO(m, DO(pn, j = e * pv[i]; DO(qn, MC(x += k, v + k * (j + qv[i]), k);)); v += n;);
		R z;
	}
...
{
	PROLOG;
	A c, ind, p = 0, q, *v, x, y = w;
	B b = 1, bb = 1;
	I acr, ar, cd, i = 0, j, k, m, n, pr, r, *s, t, wcr, wf, wr;
	RZ(a && w);
	ar = AR(a);
	acr =  jt->rank ? jt->rank[0] : ar;
	wr = AR(w);
	wcr = r = jt->rank ? jt->rank[1] : wr;
	wf = wr - wcr;
	jt->rank = 0;
	if(ar) {
		if(ar == acr && wr == wcr) {
	...
	for(; i < n; i += 2) {
		...
		if(p != ace && q != ace) {
			...
			b = 0;
			y = irs2(p, y, 0L, AR(p), r - i, jtifrom);
		} else if(q != ace)   {
			b = 0;
			y = irs2(q, y, 0L, AR(q), r - j, jtifrom);
		}
		p = 0;
	}
	RE(y);
...
F2(jtfrom)
{
	...
	case 0:
		R at&BOX ? afrom(a, w)  : at&B01 ? bfrom(a, w) : ifrom(a, w);
	case 1:
		R at&BOX ? frombs(a, w) :                  fromis(a, w);
	case 2:
		R fromsd(a, w);
	default:
		R fromss(a, w);
