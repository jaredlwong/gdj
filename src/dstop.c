...
static B stopsub(C*p, C*nw, I md)
{
	C*q, *s;
	I n;
	s = strchr(p, ';');
...
B jtdbstop(J jt, DC d, I i)
{
	...
	if(i == d->dcstop) {
		d->dcstop = -2;    /* not stopping if already stopped at the same place */
		R 0;
	}
	RZ(jt->dbstops);
	s = CAV(jt->dbstops);
	...
	while(s) {
		...
		if(!u || u > v) {
			e = !strncmp(s, t, MAX(n, v - s));
		} else {
			p = u - s;
			q = v - u - 1;
			e = p <= n && !strncmp(s, t, p) && q <= n && !strncmp(1 + u, t + n - q, q);
		}
		if(e) {
			s = 1 + v;
			if(stopsub(s, nw, md)) {
