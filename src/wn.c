...
static NUMH(jtnumj)
{
	...
	if(ta) {
		...
		v->re = y == 0.5 * PI || y == 1.5 * PI ? 0 : x * cos(y);
		v->im = y == PI ? 0 : x * sin(y);
	} else {
		v->re = x;
		v->im = y;
...
static NUMH(jtnumi)
{
	...
	RZ(19 >= n);
	j = 0;
	DO(n, RZ(t = memchr(dig, *s++, 10L)); j = 10 * j + (t - dig););
	RZ(0 <= j || neg && j == IMIN);
	*(I*)vv = 0 > j || !neg ? j : -j;
	R 1;
...
static NUMH(jtnumx)
{
	...
	if('-' == d) {
		RZ(2 >= n);
		RZ(*v = vci(1 == n ? XPINF : XNINF));
		R 1;
	}
	n -= b + c;
	RZ(m = (n + XBASEN - 1) / XBASEN);
	k = n - XBASEN * (m - 1);
	GA(y, INT, m, 1, 0);
	yv = m + AV(y);
...
static NUMH(jtnume)
{
	...
	if(td = memchr(s, '.', n)) {
		nf = te ? (te - td) - 1 : n - (td - s) - 1;
		if(nf) {
			RZ(numx(nf, td + 1, &f.n));
		}
	}
	ni = td ? td - s : te ? te - s : n;
	RZ(numx(ni, s, &i.n));
	x = i;
	if(nf) {
		y.n = iv1;
		y.d = x10(nf);
		RE(x = '-' == *s ? qminus(x, qtymes(f, y)) : qplus(x, qtymes(f, y)));
	}
	if(te) {
		if(0 > e) {
			y.n = iv1;
...
static B jtnumb(J jt, I n, C*s, Z*v, Z b)
{
	A c, d, y;
	I k;
...
static void jtnumcase(J jt, I n, C*s, B*b, B*j, B*x, B*q, B*ii)
{
	B e;
	C c;
	*x = *q = *ii = 0;
	...
	*b = memchr(s, 'b', n) || memchr(s, 'p', n);
	if(!*j && !*b) {
		...
		*ii = 1;
		...
		DO(n, c = s[i]; e = !s[1 + i]; if(c == '.' || c == 'e' || c == 'x' && !e) {
		*x = *q = *ii = 0;
		R;
	});
	}
...
A jtconnum(J jt, I n, C*s)
{
	...
	t = q ? RAT   : x ? XNUM  : b || j ? CMPX    : ii ? INT   : FL;
	k = bp(t);
	GA(z, t, m, 1 != m, 0);
	v = CAV(z);
	if(ii) {
		DO(m, d = i + i; e = yv[d]; if(!numi(yv[1 + d] - e, e + s, v)) {
		ii = 0;
		break;
	}
	v += k;);
	...
	e = yv[d];
	ASSERT(f(jt, yv[1 + d] - e, e + s, v), EVILNUM);
	v += k;
	  );
	if(t & FL + CMPX) {
	RZ(z = cvt0(z));
...
static A jtexec2r(J jt, A a, A w, I n, I m, I c)
{
	A z;
	B b, e;
	C d, *u, *uu, *v, *x, *y;
...
F2(jtexec2)
{
	...
	if(at & CMPX) {
		z = cvt0(exec2z(a, w, n, m, c));
	} else if(q) {
		z =     exec2q(a, w, n, m, c);
	} else if(x && at & B01 + INT + XNUM) {
		z =     exec2x(a, w, n, m, c);
	} else {
		z = cvt0(exec2r(a, w, n, m, c));
	}
	R bcvt(0, z);
}
