...
A jtstcreate(J jt, C k, I p, I n, C*u)
{
	A g, *pv, x, y;
	C s[20];
	I m, *nv;
	...
	case 0:  /* named    locale */
		RZ(x = nfs(n, u));
		LOCNAME(g) = x;
		LOCPATH(g) = ra(1 == n && 'z' == *u ? vec(BOX, 0L, 0L) : zpath);
		symbis(x, g, jt->stloc);
		break;
...
B jtsymbinit(J jt)
{
	...
	GA(q, SYMB, ptab[3], 1, 0);
	jt->stloc = q;
	RZ(q = apv(n, -1L, 0L));
	jt->stnum = q;
	GA(q, INT, n, 1, 0);
	jt->stptr = q;
	memset(AV(q), C0, n * SZI);
...
static A jtstfindnum(J jt, B b, I k)
{
	A y;
	I j;
	RZ(y = indexof(jt->stnum, sc(k)));
	j = *AV(y);
	if(j < AN(jt->stnum)) {
		R*(j + AAV(jt->stptr));
	} else if(b) {
		ASSERT(k >= jt->stmax, EVLOCALE);
		R stcreate(1, jt->locsize[1], k, 0L);
	} else {
		R 0;
	}
}    /* stfind for numbered locales */

A jtstfind(J jt, B b, I n, C*u)
{
	I old;
	L*v;
	if(!n) {
...
static A jtvlocnl(J jt, B b, A w)
{
	...
	for(i = 0; i < n; ++i) {
		...
		if(b) {
			ASSERTN(vlocnm(m, s), EVILNAME, nfs(m, s));
		}
	}
	R w;
...
F1(jtlocnc)
{
	...
	for(i = 0; i < n; ++i) {
		...
		if(!vlocnm(m, u)) {
			...
			zv[i] = 0 <= probenum(u) ? 1 : -1;
		} else {
			zv[i] = probe(nfs(m, u), jt->stloc) ? 0 : -1;
		}
	}
	R z;
...
static F1(jtlocnlx)
{
	...
	if(2 & m) {
		GA(y, BOX, jt->stused, 1, 0);
		yv = AAV(y);
		pv = AAV(jt->stptr);
		nv = AV(jt->stnum);
		DO(AN(jt->stptr), if(pv[i]) {
		sprintf(s, FMTI, nv[i]);
			if(jt->nla[*s]) {
				RZ(yv[n++] = cstr(s));
				if(n == jt->stused) {
...
static A jtlocale(J jt, B b, A w)
{
	A g, *wv, y;
	I wd;
	RZ(vlocnl(1, w));
...
F1(jtloccre1)
{
	RZ(w);
	if(AN(w)) {
		R rank2ex(mark, vlocnl(1, w), 0L, 0L, 0L, jtloccre);
	}
	ASSERT(1 == AR(w), EVRANK);
	R loccrenum(mark);
}    /* 18!:3  create locale */
...
F2(jtloccre2)
{
	RZ(a && w);
	if(AN(w)) {
		R rank2ex(a, vlocnl(1, w), 0L, 0L, 0L, jtloccre);
	}
	ASSERT(1 == AR(w), EVRANK);
...
F1(jtlocexmark)
{
	...
	for(i = 0; i < n; ++i) {
		...
		if(g) {
			c = 1;
			DO(1 + jt->fcalli, if(g == jt->fcallg[i].g) {
			jt->fcallg[i].flag = 1 + b;
				jt->fcallg[i].ptr = b ? j : (I)v;
				c = 0;
				break;
			});
			if(c) {
