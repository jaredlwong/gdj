...
static B jtforinit(J jt, CDATA*cv, A t)
{
	A x;
	C*s, *v;
	I k;
	...
	if(0 < k && cv->n) {                    /* for_xyz.            */
		s = 4 + CAV(cv->line);
		RZ(cv->x = x = ra(str(6 + k, s)));
		cv->xv = v = CAV(x);
		MC(k + v, "_index", 6L); /* index name          */
		cv->iv = s;                            /* item name           */
	}
	R 1;
...
static void jttryinit(J jt, TD*v, I i, CW*cw)
{
	I j = i, t = 0;
	v->d = v->t = 0;
	while(t != CEND) {
...
{
	PROLOG;
	A cd, cl, cn, h, *hv, *line, loc = jt->local, t, td, u, v, z;
	B b, lk, named, ox = jt->xdefn;
	CDATA*cv;
	CW*ci, *cw;
	DC d = 0;
	I hi, i = 0, j, m, n, od = jt->db, old, r = 0, st, tdi = 0;
	TD*tdv;
	V*sv;
	X y;
	RE(0);
	...
	sv = VAV(self);
	st = AT(self);
	lk = jt->glock || VLOCK & sv->flag;
	named = VNAMED & sv->flag ? 1 : 0;
	cn = jt->curname;
	cl = jt->curlocn;
	...
	if(sv->flag & VTRY1 + VTRY2) {
		GA(td, INT, NTD * WTD, 2, 0);
		*AS(td) = NTD;
		*(1 + AS(td)) = WTD;
		tdv = (TD*)AV(td);
	}
	...
	if(jt->db && jt->sitop && DCCALL == jt->sitop->dctype && self == jt->sitop->dcf) {
		jt->sitop->dcloc = jt->local;
		jt->sitop->dcc = hv[1 + hi];
		jt->sitop->dci = (I)&i;
	}
	old = jt->tbase + jt->ttop;
	while(0 <= i && i < n) {
		if(0 < jt->pmctr && C1 == jt->pmrec && named) {
			pmrecord(cn, cl, i, a ? VAL2 : VAL1);
		}
		...
		case CTRY:
			...
			if(jt->db) {
				jt->db = (tdv + tdi)->d ? jt->dbuser : DBTRY;
			}
			++tdi;
			++i;
			break;
		...
		case CBBLOCK:
			tpop(old);
			z = parsex(vec(BOX, ci->n, line + ci->i), lk, ci, d);
			if(z || DB1 == jt->db || DBERRCAP == jt->db || !jt->jerr) {
				++i;
			} else if(EWTHROW == jt->jerr) {
				if(tdi && (j = (tdv + tdi - 1)->t)) {
					i = 1 + j;
					RESETERR;
				} else {
					BASSERT(0, EWTHROW);
		...
		case CTBLOCK:
			...
			if(t || DB1 == jt->db || DBERRCAP == jt->db || !jt->jerr) {
				++i;
			} else if(EWTHROW == jt->jerr) {
				if(tdi && (j = (tdv + tdi - 1)->t)) {
					i = 1 + j;
		...
		case CRETURN:
			if(cd) {
				rat(z);
				DO(AN(cd) / WCD - r, unstackcv(cv); --cv; ++r;);
			}
			i = ci->go;
			break;
		case CCASE:
	...
	tpop(_ttop);
	if(z) {
		ASSERT(st & ADV + CONJ || AT(z)&NOUN, EVSYNTAX);
	}
	tpush(z);
	R z;
}
...
static F1(jtxopcall)
{
	R jt->db&&DCCALL == jt->sitop->dctype ? jt->sitop->dca : mark;
}
...
static B jtxop(J jt, A h)
{
	A p, x, y;
	B b, *pv;
	I*xv;
	GA(x, INT, jt->dotnames ? 12 : 6, 1, 0);
	xv = AV(x);
	xv[0] = (I)mnam;
	xv[1] = (I)nnam;
	xv[2] = (I)unam;
	...
	RZ(y = raze(from(v2(0L, HN), h)));
	AT(y) = INT;
	RZ(p = eps(x, y));
	pv = BAV(p);
	b = (pv[0] || pv[1] || pv[2] || pv[3]) && (pv[4] || pv[5]);
	if(!b && jt->dotnames) {
		b = (pv[6] || pv[7] || pv[8] || pv[9]) && (pv[10] || pv[11]);
	}
	R b;
...
static B jtsent12c(J jt, A w, A*m, A*d)
{
	C*p, *q, *r, *s, *x;
...
static B jtsent12b(J jt, A w, A*m, A*d)
{
	A t, *wv, y, *yv;
	I j, *v, wd;
	...
	wv = AAV(w);
	wd = (I)w * ARELATIVE(w);
	GA(y, BOX, AN(w), AR(w), AS(w));
	yv = AAV(y);
	DO(AN(w), RZ(yv[i] = vs(WVR(i))););
	RZ(t = indexof(y, link(chr[':'], str(1L, ":"))));
	v = AV(t);
	j = MIN(*v, *(1 + v));
	*m = take(sc(j  ), y);
	*d = drop(sc(j + 1), y);
...
F2(jtcolon)
{
	...
	I flag = 0, n, p;
	RZ(a && w);
	if(VERB & AT(a) && VERB & AT(w)) {
		V*v;
		v = VAV(a);
		if(CCOLON == v->id && VERB & AT(v->f) && VERB & AT(v->g)) {
			a = v->f;
		}
		v = VAV(w);
		if(CCOLON == v->id && VERB & AT(v->f) && VERB & AT(v->g)) {
	...
	switch(n) {
	case 1:
		R fdef(CCOLON, ADV,  b ? xop1 : xadv, 0L,    num[n], 0L, h, flag, RMAX, RMAX, RMAX);
	case 2:
		R fdef(CCOLON, CONJ, 0L, b ? xop2 : jtxdefn, num[n], 0L, h, flag, RMAX, RMAX, RMAX);
	case 3:
		R fdef(CCOLON, VERB, xn1, jtxdefn,       num[n], 0L, h, flag, RMAX, RMAX, RMAX);
	case 4:
		R fdef(CCOLON, VERB, xn1, jtxdefn,       num[n], 0L, h, flag, RMAX, RMAX, RMAX);
	case 13:
		R vtrans(w);
