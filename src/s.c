...
B jtsymext(J jt, B b)
{
	...
	s[0] = m;
	s[1] = symcol;
	xn = m * symcol;      /* new pool array shape        */
	GA(x, INT, xn, 2, s);
	v = AV(x);               /* new pool array              */
	if(b) {
		ICPY(v, AV(y), yn);    /* copy old data to new array  */
	}
...
B jtsymfree(J jt, L*u)
{
	I q;
	q = u->next;
	if(q) {
		(q + jt->sympv)->prev = u->prev;
	}
	if(LHEAD & u->flag) {
		*(I*)u->prev = q;
		if(q) {
			(q + jt->sympv)->flag |= LHEAD;
		}
	} else {
		(u->prev + jt->sympv)->next = q;
	}
	fa(u->name);
	u->name = 0;                  /* zero out data fields        */
	fa(u->val );
	u->val = 0;
	u->sn = u->flag = u->prev = 0;
...
L*jtprobe(J jt, A a, A g)
{
	...
	hv = AV(g) + (k ? k : 1);
	if(!*hv) {
		R jt->cursymb = 0;    /* (0) empty slot    */
	}
	v = *hv + jt->sympv;
	while(1) {
		u = NAV(v->name);
		if(m == u->m && !memcmp(s, u->s, m)) {
			R jt->cursymb = v;    /* (1) exact match   */
		}
		if(!v->next) {
			R jt->cursymb = 0;    /* (2) link list end */
		}
		v = v->next + jt->sympv;
	}
}

static L*jtprobeis(J jt, A a, A g)
{
	C*s;
	I*hv, k, m;
	L*v;
...
static L*jtsyrd1(J jt, A a, A g, B b)
{
	A*v, x, y;
	L*e;
	NM*av;
	if(b && jt->local && (e = probe(a, jt->local))) {
		av = NAV(a);
		R av->e = e;
	}
	RZ(g && (y = LOCPATH(g)));
...
static A jtlocindirect(J jt, I n, C*u)
{
	A a, g = jt->global, x, y;
	B lcl = 1;
	C*s, *v, *xv;
...
static A jtdllsymaddr(J jt, A w, C flag)
{
	A*wv, x, y, z;
	I i, n, wd, *zv;
	L*v;
...
B jtredef(J jt, A w, L*v)
{
	...
	while(c = c->dclnk) {
		jt->curname = c->dca;
		ASSERT(!(DCCALL == c->dctype && v == (L*)c->dcn), EVSTACK);
	}
	jt->curname = oldn;
	R 1;
}    /* check for changes to stack */

A jtsymbis(J jt, A a, A w, A g)
{
	A x;
	I m, n, wn, wr, wt;
	NM*v;
	...
	if(n == m)ASSERT(!(jt->local && g == jt->global && probe(a, jt->local)), EVDOMAIN)
			...
			C*s = 1 + m + v->s;
			RZ(g = NMILOC & v->flag ? locindirect(n - m - 2, 1 + s) : stfind(1, n - m - 2, s));
		}
	RZ(e = probeis(a, g));
	if(jt->db) {
		RZ(redef(w, e));
	...
	e->sn = jt->slisti;
	if(jt->stch && (m < n || jt->local != g && jt->stloc != g)) {
		e->flag |= LCH;
	}
	R mark;
