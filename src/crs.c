...
static A jtsprarg(J jt, I f, A x)
{
	A q;
	B*b, c;
	I r;
...
static A jtsprinit(J jt, I f, I r, I*s, I t, P*p)
{
	A a, a1, z;
	I n, *u, *v;
	P*zp;
...
static B*jtspredge(J jt, A y, I f, I*zm)
{
	A q;
	B*b;
	I c, m, n, *v;
	...
	if(n) {
		if(f) {
			v = AV(y);
			DO(n - 1, if(b[i] = 1 && ICMP(v, v + c, f))++m; v += c;);
		} else {
			memset(b, C0, n);
		}
		b[n - 1] = 1;
...
static A jtsprz(J jt, A z0, A y, A e, I f, I*s)
{
	A a, a0, q, y0, z;
	B d;
	I c, et, h, m, n, r, t, *u, *v, zt;
	P*ep, *zp, *zq;
	RZ(z0 && y && e);
	ASSERT(AN(e), EVDOMAIN);
	if(SPARSE & AT(e)) {
		ep = PAV(e);
		ASSERT(all1(eq(SPA(ep, e), SPA(ep, x))), EVSPARSE);
		q = SPA(ep, e);
	} else {
		RZ(q = reshape(mtv, e));
		ASSERT(all1(eq(q, e)), EVSPARSE);
	}
	if(!*AS(z0)) {
		t = AT(q);
		zt = STYPE(t);
		GA(z, zt, 1L, f + AR(e), s);
		ICPY(f + AS(z), AS(e), AR(e));
		zp = PAV(z);
		SPB(zp, e, q);
		SPB(zp, a, mtv);
		GA(q, INT, 0L, 2L, &zeroZ);
		SPB(zp, i, q);
		GA(q, t, 0L, 1 + AR(z), 0L);
		*AS(q) = 0;
		ICPY(1 + AS(q), AS(z), AR(z));
		SPB(zp, x, q);
		R z;
	}
	e = q;
	...
	r = AR(z0);
	GA(z, zt, 1, f + r - 1, s);
	ICPY(AS(z) + f, AS(z0) + 1, r - 1);
	zp = PAV(z);
	SPB(zp, e, m == et ? e : cvt(m, e));
	if(d) {
		SPB(zp, a, IX(f));
		SPB(zp, i, y);
		SPB(zp, x, m == t ? z0 : cvt(m, z0));
		R z;
	}
	zq = PAV(z0);
	y0 = SPA(zq, i);
	...
	u = AV(a);
	DO(f, u[i] = i;);
	DO(c - 1, u[f + i] = v[1 + i] + f - 1;);
	SPB(zp, a, a);
	SPB(zp, x, m == t ? ra(SPA(zq, x)) : cvt(m, SPA(zq, x)));
	R z;
}    /* result processing */

A jtsprank1(J jt, A w, A fs, I mr, AF f1)
{
	PROLOG;
	A q, wx, wy, wy1, ww, z, ze, zi, *zv;
	B*wb;
	I c, i, *iv, j, k, m, n, *v, wcr, wf, wr, *ws, wt, *wv;
	P*wp, *wq;
	RZ(w);
	wr = AR(w);
	ws = AS(w);
...
static I jtspradv(J jt, I n, B*b, I f, I r, I j, P*p, A*z)
{
	A s, x;
	I k;
	P*q;
	...
	if(r) {
		...
		RZ(*z = AN(x) ? from(sc(j), x) : ca(SPA(p, e)));
	}
	R k;
}    /* advance to the next cell */

static A jtsprank2_0w(J jt, A a, A w, A fs, AF f2, I wf, I wcr)
{
	PROLOG;
	A we, ww, y, z, zi, *zv;
	B*wb;
	I f, *iv, j, *v, wc, wj, wk, wm, wn, *ws, wt, *wv;
	P*wp;
	f = wf;
	ws = AS(w);
	RZ(w = sprarg(wf, w));
...
static A jtsprank2_a0(J jt, A a, A w, A fs, AF f2, I af, I acr)
{
	PROLOG;
	A aa, ae, y, z, zi, *zv;
	B*ab;
	I f, *iv, j, *v, ac, aj, ak, am, an, *as, at, *av;
	P*ap;
	f = af;
	as = AS(a);
	RZ(a = sprarg(af, a));
...
A jtsprank2(J jt, A a, A w, A fs, I lr, I rr, AF f2)
{
	PROLOG;
	A aa, ae, we, ww, y, zi, z, *zv;
	B*ab, b, *wb;
	I ac, acr, af, aj, ak, am, an,
	ar, *as, at, *av, d, f, g, *ii, *iv, j, k, m, s, *u, *v, wc, wcr, wf, wj, wk, wm, wn, wr, *ws, wt, *wv;
	P*ap, *wp;
	RZ(a && w);
	ar = AR(a);
	as = AS(a);
	...
	DO(wf, ASSERT(ws[i], EVNONCE););
	if(!af) {
		R sprank2_0w(a, w, fs, f2, wf, wcr);
	}
	if(!wf) {
		R sprank2_a0(a, w, fs, f2, af, acr);
	}
	f = MIN(af, wf);
	g = MAX(af, wf);
	m = 1;
	...
	if(af == wf)while(av || wv) {
			if(av && wv) {
				DO(f, if(s = av[i] - wv[i])break;
			}) else {
			s = av ? -1 : 1;
		}
		if     (0 == s) {
			RZ(zv[j++] = CALL2(f2, aa, ww, fs));
				ICPY(iv, av, f);
				iv += g;
			} else if(0 > s) {
			RZ(zv[j++] = CALL2(f2, aa, we, fs));
				ICPY(iv, av, f);
				iv += g;
			} else if(0 < s) {
			RZ(zv[j++] = CALL2(f2, ae, ww, fs));
				ICPY(iv, wv, f);
				iv += g;
			}
			if(0 >= s) {
		...
		if(b && 0 < s || !b && 0 > s) {
			RZ(zv[j++] = CALL2(f2, b ? ae : aa, b ? ww : we, fs));
				ICPY(iv, b ? wv : av, g);
				iv += g;
				k = m;
			} else if(s) {
			...
			if     ( b && 0 >= s && (!v || ICMP(v, wv, f)))if(aj == an) {
				av = 0;
			} else {
				av += ak * ac;
				RE(ak = spradv(an, ab, af, acr, aj, ap, &aa));
			...
			else if(!b && 0 <= s && (!v || ICMP(v, av, f)))if(wj == wn) {
				wv = 0;
			} else {
				wv += wk * wc;
				RE(wk = spradv(wn, wb, wf, wcr, wj, wp, &ww));
...
A jtva2s(J jt, A a, A w, C id, VF ado, I cv, I t, I zt, I lr, I rr)
{
	PROLOG;
	A aa, ae, we, ww, y, zi, z, *zv;
	B*ab, b, *wb;
