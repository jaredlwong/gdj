...
F1(jtrazein)
{
	R df2(w, box(raze(w)), amp(swap(ds(CEPS)), ds(COPE)));
}
...
static F2(jtebarmat)
{
	A ya, yw, z;
	B b, *zv;
	C*au, *av, *u, *v, *v0, *wu, *wv;
	I*as, c, i, k, m, n, r, s, si, sj, t, *ws;
	RZ(a && w);
	as = AS(a);
	av = CAV(a);
	ws = AS(w);
...
static F2(jtebarvec)
{
	...
	if(t & INT || 0 == jt->ct && t & FL + CMPX) {
		DO(n, zv[i] = !memcmp(av, wv, s);
	}
	wv += k;
	  )
	else {
		GA(y, t, an, AR(a), 0);
		yv = CAV(y);
		DO(n, MC(yv, wv, s); zv[i] = equ(a, y); wv += k;);
	}
	R z;
...
static I jtebarprep(J jt, A a, A w, A*za, A*zw, I*zc)
{
	I ar, at, c = 0, ca, cw, d = IMAX, da, dw, m, n, t, wr, wt;
	ar = AR(a);
	at = AT(a);
	m = AN(a);
	wr = AR(w);
...
F2(jtebar)
{
	...
	case -2:
		R ebarmat(a, w);
	case -3:
		R df2(shape(a), w, cut(amp(a, ds(CMATCH)), num[3]));
	case -4:
		R ebarvec(a, w);
	}
	GA(z, B01, n, AR(w), 0);
	...
	switch(AT(w)) {
	case INT:
		if(c)EBLOOP(I, u[i] - c, v[k + m] - c, zv[k] = i == m)
			else {
				EBLOOP(I, u[i],  v[k + m],   zv[k] = i == m);
			}
		break;
	case C2T:
		EBLOOP(US, u[i],  v[k + m],   zv[k] = i == m);
		break;
	default:
...
F2(jti1ebar)
{
	...
	switch(AT(w)) {
	case INT:
		if(c)EBLOOP(I, u[i] - c, v[k + m] - c, if(i == m)R sc(k))
				else {
					EBLOOP(I, u[i],  v[k + m],   if(i == m)R sc(k));
				}
		break;
	case C2T:
		EBLOOP(US, u[i],  v[k + m],   if(i == m)R sc(k));
		break;
	default:
...
F2(jtsumebar)
{
	...
	switch(AT(w)) {
	case INT:
		if(c)EBLOOP(I, u[i] - c, v[k + m] - c, if(i == m)++z)
				else {
					EBLOOP(I, u[i],  v[k + m],   if(i == m)++z);
				}
		break;
	case C2T:
		EBLOOP(US, u[i],  v[k + m],   if(i == m)++z);
		break;
	default:
...
F2(jtanyebar)
{
	...
	switch(AT(w)) {
	case INT:
		if(c)EBLOOP(I, u[i] - c, v[k + m] - c, if(i == m)R one)
				else {
					EBLOOP(I, u[i],  v[k + m],   if(i == m)R one);
				}
		break;
	case C2T:
		EBLOOP(US, u[i],  v[k + m],   if(i == m)R one);
		break;
	default:
		EBLOOP(UC, u[i],  v[k + m],   if(i == m)R one);
	}
	R zero;
}    /* a ([: +./ E.) w where a and w are atoms or lists */
...
F2(jtifbebar)
{
	...
	switch(AT(w)) {
	case INT:
		if(c)EBLOOP(I, u[i] - c, v[k + m] - c, if(i == m)IFB1)
				else {
					EBLOOP(I, u[i],  v[k + m],   if(i == m)IFB1);
				}
		break;
	case C2T:
		EBLOOP(US, u[i],  v[k + m],   if(i == m)IFB1);
		break;
	default:
