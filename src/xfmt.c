...
static B jtfmtcomma(J jt, C *x, I l, I d, C *subs)
{
	C *v, *u;
	I j, n, c;
	n = l - (c = (l - !!d - d) / 4);
	u = x + l - 1;
	if(v = memchr(x, SUBd, n)) {
		j = n - (v - x);
		u -= j;
		memmove(u + 1, v, j);
		v--;
...
static I jtdpone(J jt, B bits, D w)
{
	D t;
	if(bits & BITSf) {
		R 0;
...
static F1(jtfmtparse)
{
	A x, z, *zv;
	B ml[2 + NMODVALS], mod, t;
	C c, *cu = "srqpnmdblc", *cu1 = "?bdmnpqrs", d, *s, *wv;
	I fb, i, j, mi, n, n1, p, q, vals[3] = { -1, -1, 0};
	RZ(w);
	w = AAV0(w);
	n = AN(w);
	GA(z, BOX, 1 + NMODVALS, 1, 0);
	zv = AAV(z);
	...
	for(i = 0; i < n1; ++i) {
		...
		if(!mod) {
			continue;    /* 1==mod iff not in modifier text <xx>; i.e. c is a modifier letter or 0-9 */
		}
		if(0 <= mi) {
			d = wv[mi];
			ASSERT(s = strchr(cu, d), EVDOMAIN);
			j = s - cu;
			ASSERT(ml[j], EVDOMAIN);
			ml[j] = 0;
			fb |= (I)1 << j;
			if(s = strchr(cu1, d)) {
				if(i - mi > 3) {
					RZ(zv[s - cu1] = str(i - mi - 3, wv + mi + 2));
				}
			} else {
				ASSERT(1 == i - mi, EVDOMAIN);
			}
		}
		mi = i;
		if('0' <= c && c <= '9') {
			RZ(widthdp(str(n - i, wv + i), vals, vals + 1));
			break;
		}
	}
	if(mtv != zv[NMODVALS]) {
...
#endif
typedef union u_DI8_tag {
	I8 i;
	D d;
} DI8;

static D jtroundID(J jt, I d, D y)
{
	D f, q, c, h;
	DI8 f8, q8, c8;
	q = ppwrs[d] * y;
...
static D jtafzrndID(J jt, I dp, D y)
{
	R SGN(y)*roundID(dp, ABS(y));
}
/* round-to-nearest, solve ties by rounding Away From Zero */

static D jtexprndID(J jt, I d, D y)
{
	I e, s;
	D f, q, c, x;
	DI8 f8, y8, c8;
	s = SGN(y);
	e = -(I)jfloor(log10(y = ABS(y)));
	if(308 >= ABS(d + e)) {
		e += d;
		d = 0;
	}
...
static B jtsprintfI(J jt, C *x, I m, I dp, I iw, C *subs)
{
	I r, g;
	x += m - 1;
	DO(dp, *x-- = '0';);
...
static B jtsprintfnD(J jt, C *x, I m, I dp, D dw, C *subs)
{
	I nd;
	int decpt, sign;
	if(dw == 0) {
...
static B jtsprintfeD(J jt, C *x, I m, I dp, D dw, C *subs)
{
	I y, y0;
	int decpt, sign;
...
static F2(jtfmtprecomp)
{
	A*as, base, fb, len, strs, *u, z;
	B*bits, *bw;
	D dtmp, *dw;
	...
	if(1 < nf) {
		GA(base, INT, nf * 4, 2, 0);
		s = AS(base);
		*s++ = nf;
		*s = 4;
	} else {
		GA(base, INT, 3 + nc, 1, 0);
	}
	GA(strs, BOX, nf * NMODVALS, 2, 0);
	s = AS(strs);
	*s++ = nf;
	*s = NMODVALS;
	GA(len, INT, n, wr, ws);
	...
	*u++ = fb;
	...
	if(1 == nf) {
		MC(ib, AV(*u), SZI * 3);
		memset(ib + 3, C0, SZI * nc);
		MC(as, u + 1, SZA * NMODVALS);
	} else {
		DO(nf, MC(ib, AV(*u), SZI * 3);
	}
	ib[3] = 0;
	ib += 4;
	MC(as, u + 1, SZA * NMODVALS);
...
case B01:
	...
	case FL:
		...
		for(i = 0; i < n; ++i) {
			...
			dtmp = ABS(*dw);
			if(d == -1) {
				*bits |= BITSe * (TNE(0, dtmp) && (TLT(dtmp, 1e-9) || TLT(2e9, dtmp)));
			}
			*bits |= BITS_ * !memcmp(dw, &inf , SZD) +
			...
			if(d == -1) {
				...
				*bits |= BITSz * (TEQ(*dw, 0) || (!(*bits & BITSf + BITSe) && TLT(dtmp, npwrs[d] / 2)));
			}
			bits++;
			dw++;
			iv++;
...
static A jtfmtallcol(J jt, A a, A w, I mode)
{
	A *a1v, base, fb, len, strs, *u, v, x;
	B *bits, *bv;
	C*cB, *cD, *cM, *cN, *cP, *cQ, *cR, *cv, **cvv, *cx, *subs;
	D dtmp, *dv;
	...
	case 2:
		coll = 0;
		ib = AV(base);
		DO(nc, if(0 < ib[0]) coll += ib[0]; else {
			coll += ib[3 + (1 < nf ? 0 : i % nc)];
			}
		if(1 < nf) ib += 4; );
		zs[0] = prod(wr - 1, ws);
		zs[1] = coll;
		...
		if(0 < l && l < *il) {
			...
			if(*bits & BITSf) {
				...
				case INT:
					...
					if(mC) {
						RZ(fmtcomma(cv + y, *il - y - g, d, subs));
					}
					if     (*iv < 0 && mMN) {
						MC(cv, cM, nM);
						MC(cv + *il - nN, cN, nN);
...
static A jtfmtxi(J jt, A a, A w, I mode, I *omode)
{
	I lvl;
	RZ(a && w);
	*omode = 0;
	...
	if(!AN(w)) {
		RZ(w = reshape(shape(w), chr[' ']));
	}
	if(JCHAR & AT(w)) {
		R df1(w, qq(atop(ds(CBOX), ds(CCOMMA)), one));
	}
	...
	if(setjmp(((struct dtoa_info*)jt->dtoa)->_env)) {
		ASSERTSYS(jt->jerr, "dtoa");
	}
	if(lvl = level(w)) {
		A*wv = AAV(w), x;
		I wd = (I)w * ARELATIVE(w);
		ASSERT(1 >= lvl, EVDOMAIN);
		DO(AN(w), x = WVR(i); ASSERT(1 >= AR(x), EVRANK); if(AN(x)) {
		ASSERT(AT(x)&JCHAR + NUMERIC, EVDOMAIN);
			ASSERT(!(AR(x) && AT(x)&NUMERIC), EVRANK);
		});
		R df2(reitem(shape(w), a), w, amp(foreign(num[8], num[0]), ds(COPE)));
	} else {
		if(XNUM + RAT + CMPX & AT(w)) {
			RZ(w = cvt(FL, w));
		}
		*omode = mode;
...
{
	RZ(w);    /* 8!:0 monad */
	R fmt02(AR(w) ? reshape(sc(*(AS(w) + AR(w) - 1)), ace) : ace, w);
}
F1(jtfmt11)
{
	RZ(w);    /* 8!:1 monad */
	R fmt12(AR(w) ? reshape(sc(*(AS(w) + AR(w) - 1)), ace) : ace, w);
}
F1(jtfmt21)
{
	RZ(w);    /* 8!:2 monad */
	R fmt22(AR(w) ? reshape(sc(*(AS(w) + AR(w) - 1)), ace) : ace, w);
}
