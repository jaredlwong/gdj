...
static KF1(jtcvt2bit)
{
	I c, i, m, q, r, r1, wr, *ws, *wv;
	UC k, *zv = (UC*)yv;
	wv = AV(w);
	wr = AR(w);
	ws = AS(w);
...
static KF1(jtBfromI)
{
	...
	v = AV(w);
	x = (B*)yv;
	DO(n, p = *v++; if(0 == p || 1 == p)*x++ = (B)p; else {
		R 0;
	});
	R 1;
}
...
static KF1(jtBfromD)
{
	...
	if(!p)*x++ = 0; else if(FEQ(1.0, p))*x++ = 1; else {
				R 0;
			});
	R 1;
}
...
static KF1(jtIfromX)
{
	...
	n = AN(w);
	RZ(p = xc(IMAX));
	RZ(q = xminus(negate(p), xc(1L)));
	for(i = 0; i < n; ++i) {
		c = v[i];
		RZ(1 != xcompare(q, c) && 1 != xcompare(c, p));
		m = AN(c);
		u = AV(c) + m - 1;
		a = 0;
		DO(m, a = *u-- +a * XBASE;);
...
static KF1(jtQfromD)
{
	...
	for(i = 0; i < n; ++i) {
		...
		if     (t == inf) {
			...
			e -= 1023;
			if(recip) {
				q.d = xtymes(xd1(t / pow(2.0, e - 53.0)), xpow(xc(2L), xc(e - 53)));
				q.n = ca(iv1);
			} else     {
				q.n = xtymes(xd1(t / pow(2.0, e - 53.0)), xpow(xc(2L), xc(e - 53)));
				q.d = ca(iv1);
			}
		}
		if(neg) {
...
static B jtDXfI(J jt, I p, A w, DX*x)
{
	B b;
...
static B jtccvt(J jt, I t, A w, A*y)
{
	A d;
	I n, r, *s, wt, *wv, *yv;
	RZ(w);
	...
	case 1:
		...
		break;
	case 2:
		RZ(*y = sparseit(cvt(DTYPE(t), w), IX(r), cvt(t, zero)));
		R 1;
	case 3:
		t1 = DTYPE(t);
		GA(*y, t, 1, r, s);
...
// else if(n&&t&JCHAR){ASSERT(HOMO(t,wt),EVDOMAIN); RZ(*y=uco1(w)); R 1;}
	...
	R 1;
	case CVCASE(XNUM, B01 ):
		R XfromB(w, yv);
	case CVCASE(RAT , B01 ):
		GA(d, XNUM, n, r, s);
		R XfromB(w, AV(d))&&QfromX(d, yv);
	case CVCASE(FL  , B01 ): {
		D*x = (D*)yv;
		B*v = (B*)wv;
		DO(n, *x++   = *v++;);
	...
	case CVCASE(B01 , INT ):
		R BfromI(w, yv);
	case CVCASE(XNUM, INT ):
		R XfromI(w, yv);
	case CVCASE(RAT , INT ):
		GA(d, XNUM, n, r, s);
		R XfromI(w, AV(d))&&QfromX(d, yv);
	case CVCASE(FL  , INT ): {
		D*x = (D*)yv;
		I*v =    wv;
		DO(n, *x++   = (D) * v++;);
	...
	case CVCASE(CMPX, FL  ): {
		Z*x = (Z*)yv;
		D t, *v = (D*)wv;
		DO(n, t = *v++; x++->re = t || _isnan(t) ? t : 0.0;);
	}
		R 1;  /* -0 to 0*/
	case CVCASE(BIT , CMPX):
		GA(d, FL, n, r, s);
		RZ(DfromZ(w, AV(d)));
		R cvt2bit(d, yv);
	case CVCASE(B01 , CMPX):
		GA(d, FL, n, r, s);
		RZ(DfromZ(w, AV(d)));
		R BfromD(d, yv);
	case CVCASE(INT , CMPX):
		GA(d, FL, n, r, s);
		RZ(DfromZ(w, AV(d)));
		R IfromD(d, yv);
	case CVCASE(XNUM, CMPX):
		GA(d, FL, n, r, s);
		RZ(DfromZ(w, AV(d)));
		R XfromD(d, yv);
	case CVCASE(RAT , CMPX):
		GA(d, FL, n, r, s);
		RZ(DfromZ(w, AV(d)));
		R QfromD(d, yv);
	case CVCASE(FL  , CMPX):
		R DfromZ(w, yv);
	case CVCASE(B01 , XNUM):
		R BfromX(w, yv);
	case CVCASE(INT , XNUM):
		R IfromX(w, yv);
	case CVCASE(RAT , XNUM):
		R QfromX(w, yv);
	case CVCASE(FL  , XNUM):
		R DfromX(w, yv);
	case CVCASE(CMPX, XNUM):
		GA(d, FL,  n, r, s);
		RZ(DfromX(w, AV(d)));
		R ccvt(t, d, y);
	case CVCASE(B01 , RAT ):
	...
	case CVCASE(INT , RAT ):
		...
		RZ(XfromQ(w, AV(d)));
		R IfromX(d, yv);
	case CVCASE(XNUM, RAT ):
		R XfromQ(w, yv);
	case CVCASE(FL  , RAT ):
		R DfromQ(w, yv);
	case CVCASE(CMPX, RAT ):
		GA(d, FL,  n, r, s);
		RZ(DfromQ(w, AV(d)));
		R ccvt(t, d, y);
	default:
...
A jtbcvt(J jt, C mode, A w)
{
	...
	if(mode || !(AT(w)&XNUM + RAT)) {
		z = ccvt(B01, w, &y) ? y : ccvt(INT, w, &y) ? y : ccvt(FL, w, &y) ? y : w;
	}
	jt->fuzz = ofuzz;
	jt->rank = oq;
	RNE(z);
...
#else
		z1 = *(1 + v);
		DO(n, if(z0 == u[0] && z1 == u[1])u[0] = u[1] = 0; u += 2;);
#endif
	}
	R w;
...
F1(jtxco1)
{
	...
	R cvt(AT(w)&B01 + INT + XNUM ? XNUM : RAT, w);
}
