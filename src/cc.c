...
static F2(jtcut02m)
{
	A z;
	C*u, *v;
	I*av, c, d, e0, e1, j0, j1, k0, k1, m0, m1, *s, t, wk;
	s = AS(w);
	m0 = s[0];
	m1 = s[1];
	t = AT(w);
...
{
	DECLF;
	A h = 0, *hv, q, qq, *qv, y, z, *zv;
	C id;
	I*as, c, d, e, hn, i, ii, j, k, m, n, *u, *ws;
	RZ(a && w);
	if(VGERL & sv->flag) {
		h = sv->h;
		hv = AAV(h);
...
DF2(jtrazecut0)
{
	A z;
	C*v, *wv, *zu, *zv;
	I ar, *as, *av, c, d, i, j, k, m, n, q, wt, zn;
	RZ(a && w);
	n = AN(w);
	wt = AT(w);
	wv = CAV(w);
	...
	as = AS(a);
	m = 2 == ar ? 1 : *as;
	if(!((2 == ar || 3 == ar) && wt & IS1BYTE && 1 == AR(w))) {
		R raze(df2(a, w, cut(ds(CBOX), zero)));
	}
	ASSERT(2 == as[ar - 2] && 1 == as[ar - 1], EVLENGTH);
	RZ(a = vib(a));
	av = AV(a);
	RZ(z = exta(wt, 1L, 1L, n / 2));
	zn = AN(z);
	zv = CAV(z);
	zu = zn + zv;
	for(i = 0; i < m; ++i) {
		...
		q = 0 <= k ? k : k == IMIN ? IMAX : -k;
		d = 0 <= j ? MIN(q, n - j) : MIN(q, n + 1 + j);
		while(zu < d + zv) {
			c = zv - CAV(z);
			RZ(z = ext(0, z));
			zn = AN(z);
	...
	for(i = 0; i < an; ++i) {
		...
		if(!bn && m) {
			...
			if(!AR(b)) {
				if(*BAV(b)) {
					RZ(xv[i] = IX(m));
					RZ(yv[i] = reshape(sc(m), 0 < q ? one : zero));
				} else {
					xv[i] = yv[i] = mtv;
				}
				continue;
...
static A jtselx(J jt, A x, I r, I i)
{
	A z;
	I c, k;
	c = aii(x);
	k = c * bp(AT(x));
	GA(z, AT(x), r * c, AR(x), AS(x));
	*AS(z) = r;
	MC(CAV(z), CAV(x) + i * k, r * k);
	R z;
}    /* (i+i.r){x */

static A jtsely(J jt, A y, I r, I i, I j)
{
	A z;
	I c, *s, *v;
	c = *(1 + AS(y));
...
static DF2(jtcut2sx)
{
	...
	ap = PAV(a);
	if(!(equ(zero, SPA(ap, e)) && AN(SPA(ap, a)))) {
		R cut2(cvt(B01, a), w, self);
	}
	vf = VAV(fs);
	if(VGERL & sv->flag) {
		h = sv->h;
	...
	*yu++ = p = pfx ? n : -1;
	...
	if(t & DENSE) {
		...
		case 0:
			...
			for(i = 1; i <= m; ++i) {
				j = yu[i];
				DO(q = r - p, if(j < yv[c * (p + i)]) {
				q = i;
				break;
			});
				qn = q;
				if(neg)DO(qn = r - p, if(j - 1 < yv[c * (p + i)]) {
					qn = i;
					break;
				});
...
static C*jtidenv0(J jt, A a, A w, V*sv, I zt, A*zz)
{
	A fs, y;
	*zz = 0;
	fs = sv->f;
	...
	if(zt < AT(y)) {
		*zz = df1(cut2(a, w, cut(ds(CBOX), sv->g)), amp(fs, ds(COPE)));
		R 0;
	}
	if(zt > AT(y)) {
		RE(y = cvt(zt, y));
...
static DF2(jtcut2)
{
	...
	A h = 0, *hv, y, z = 0, *za;
	B b, neg, pfx;
	C id, id1, sep, *u, *v, *v1, *wv, *zc;
	I c, cv, e = 0, d, hn, i, k, ke, m = 0, n, old, p, q, r, *s, wd, wt, *zi, *zs;
	V*vf;
	VF ado;
	PREF2(jtcut2);
	if(SB01 & AT(a) || SPARSE & AT(w)) {
		R cut2sx(a, w, self);
	}
	p = n = IC(w);
	wt = AT(w);
	...
	if(a != mark) {
		if(!AN(a) && n) {
			if(VGERL & sv->flag) {
				h = sv->h;
				ASSERT(AN(h), EVLENGTH);
				h = *AAV(h);
				R CALL1(VAV(h)->f1, w, h);
			} else {
				R CALL1(f1, w, fs);
			}
		}
		...
		v = CAV(a);
		sep = C1;
	} else if(1 >= AR(w) && wt & IS1BYTE) {
		a = w;
		v = CAV(a);
		sep = v[pfx ? 0 : n - 1];
	} else {
		RZ(a = n ? eps(w, take(num[pfx ? 1 : -1], w)) : mtv);
		v = CAV(a);
		sep = C1;
	}
	...
	case CTAIL:
		...
		zc = CAV(z);
		*AS(z) = m;
		EACHCUT(if(d)MC(zc, v1 + k * (d - 1), k); else {
			fillv(wt, c, zc);
			}
		zc += k;);
		break;
	...
	case CSLASH:
		...
		if(ado) {
			...
			*AS(z) = m;
			...
			EACHCUT(if(d)ado(jt, 1L, d * c, d, zc, v1); else {
				if(!z0) {
						z0 = idenv0(a, w, sv, zt, &y);
						if(!z0) {
							if(y) {
								R y;
							} else {
	...
	if(!z) {
		...
		if(!m) {
			y = reitem(zero, w);
			y = h ? df1(y, *hv) : CALL1(f1, y, fs);
			RESETERR;
			R iota(over(zero, shape(y ? y : mtv)));
		}
		GA(z, BOX, m, 1, 0);
		za = AAV(z);
		switch((wd ? 2 : 0) + (h ? 1 : 0)) {
		case 0:
			EACHCUTG(MC(AV(y), v1, d * k);  RZ(y = CALL1(f1, y, fs));     );
			break;
		case 1:
			EACHCUTG(MC(AV(y), v1, d * k);  RZ(y = df1(y, hv[(i - 1) % hn])););
			break;
		case 2:
			EACHCUTG(MCREL(AV(y), v1, d); RZ(y = CALL1(f1, y, fs));     );
			break;
		case 3:
			EACHCUTG(MCREL(AV(y), v1, d); RZ(y = df1(y, hv[(i - 1) % hn])););
		}
		if(!allbx) {
			RZ(z = ope(z));
		}
...
static A jtpartfscan(J jt, A a, A w, I cv, B pfx, C id, C ie)
{
	A z = 0;
	B*av;
	I m, n, zt;
	...
	case PSCASE(CPLUS,   INT, B01):
		PSLOOP(I, B, s += x,      0   );
		break;
	case PSCASE(CPLUS,   FL, FL ):
		NAN0;
		PSLOOP(D, D, s += x,      0.0 );
		NAN1;
		break;
	case PSCASE(CMAX,    INT, INT):
		PSLOOP(I, I, s = MAX(s, x), IMIN);
		break;
	case PSCASE(CMAX,    FL, FL ):
		PSLOOP(D, D, s = MAX(s, x), -inf);
		break;
	case PSCASE(CMIN,    INT, INT):
		PSLOOP(I, I, s = MIN(s, x), IMAX);
		break;
	case PSCASE(CMIN,    FL, FL ):
		PSLOOP(D, D, s = MIN(s, x), inf );
		break;
	case PSCASE(CMAX,    B01, B01):
	case PSCASE(CPLUSDOT, B01, B01):
		PSLOOP(B, B, s |= x,      0   );
		break;
	case PSCASE(CMIN,    B01, B01):
	case PSCASE(CSTARDOT, B01, B01):
		PSLOOP(B, B, s &= x,      1   );
		break;
	case PSCASE(CNE,     B01, B01):
		PSLOOP(B, B, s ^= x,      0   );
		break;
	case PSCASE(CEQ,     B01, B01):
		PSLOOP(B, B, s = s == x,    1   );
		break;
	}
	R z;
...
DF2(jtrazecut2)
{
	A fs, gs, x, y, z = 0;
	B b, neg, pfx;
	C id, ie = 0, sep, *u, *v, *wv, *zv;
	I c, cv = 0, d, k, m = 0, n, p, q, r, *s, wt;
	V*fv, *sv, *vv;
	VF ado = 0;
	RZ(a && w);
	sv = VAV(self);
	gs = CFORK == sv->id ? sv->h : sv->g;
	vv = VAV(gs);
	y = vv->f;
	fs = VAV(y)->g;
	p = n = IC(w);
	...
	if((id == CBSLASH || id == CBSDOT) && (vv = VAV(fv->f), CSLASH == vv->id)) {
		ie = vaid(vv->f);
		if(id == CBSLASH) {
			vapfx(ie, wt, &ado, &cv);    /* [: ; <@(f/\ );.n */
		} else {
			vasfx(ie, wt, &ado, &cv);    /* [: ; <@(f/\.);.n */
		}
	}
	if(SPARSE & AT(w)) {
		R raze(cut2(a, w, gs));
	...
	if(ado) {
		...
		zt = rtype(cv);
		zk = c * bp(zt);
		if(1 == r && !neg && B01 & AT(a) && p == n && v[pfx ? 0 : n - 1]) {
			RE(z = partfscan(a, w, cv, pfx, id, ie));
			if(z) {
				R z;
			}
		}
		...
		while(p) {
			...
			if(d = q - neg) {
				...
				if(jt->jerr) {
					R jt->jerr == EWOV ? razecut2(a, w, self) : 0;
				}
				m += d;
				zv += d * zk;
			}
		...
		B b1 = 0;
		I old, wc = c, yk, ym, yr, *ys, yt; /* general f */
		RZ(x = gah(r, w));
		ICPY(AS(x), s, r);
		while(p) {
			if(u = memchr(v + pfx, sep, p - pfx)) {
			...
			*AS(x) = d;
			...
			if(!z) {
				...
				c = aii(y);
				yk = c * bp(yt);
				GA(z, yt, n * c, MAX(1, yr), ys);
				*AS(z) = n;
				zv = CAV(z);
			}
			if(!(yt == AT(y) && yr == AR(y) && (1 >= yr || !ICMP(1 + AS(y), 1 + ys, yr - 1)))) {
				z = 0;
				break;
			}
			while(IC(z) <= m + ym) {
	...
	if(z) {
		*AS(z) = m;
		...
		R raze(cut2(B01 & AT(a) ? a : eq(scc(sep), a), w, gs));
	}
}    /* ;@(<@f);.n or ([: ; <@f;.n) , monad and dyad */

DF1(jtrazecut1)
{
	R razecut2(mark, w, self);
}


static A jttesos(J jt, A a, A w, I n)
{
	A p;
	I*av, c, k, m, *pv, s, *ws;
	RZ(a && w);
...
static A jttesmatu(J jt, A a, A w, A self, A p, B e)
{
	DECLF;
	A x, y, z, z0;
	C*u, *v, *v0, *wv, *yv, *zv;
...
static A jttesmat(J jt, A a, A w, A self, A p, B e)
{
	DECLF;
	A y, z, *zv;
	C*u, *v, *v0, *wv, *yv;
...
F2(jtcut)
{
	...
	if(NOUN & AT(a)) {
		flag = VGERL;
		RZ(h = fxeachv(1L, a));
		ASSERT(3 != k && -3 != k, EVNONCE);
	}
	switch(k) {
	case 0:
		R fdef(CCUT, VERB, jtcut01, jtcut02, a, w, h, flag, RMAX, 2L, RMAX);
	case 1:
	case -1:
	case 2:
	case -2:
		R fdef(CCUT, VERB, jtcut1, jtcut2,  a, w, h, flag, RMAX, 1L, RMAX);
	case 3:
	case -3:
		R fdef(CCUT, VERB, jttess1, jttess2, a, w, h, flag, RMAX, 2L, RMAX);
	default:
		ASSERT(0, EVDOMAIN);
	}
}
