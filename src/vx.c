...
int jtxcompare(J jt, X a, X w)
{
	...
	if(1 == m && (x == XPINF || x == XNINF)) {
		R 0 < x ? !(1 == n && y == XPINF) : -!(1 == n && y == XNINF);
	}
	if(1 == n && (y == XPINF || y == XNINF)) {
		R 0 < y ? -!(1 == m && x == XPINF) : !(1 == m && x == XNINF);
	}
	if(m != n) {
		R m > n ? s : -s;
	}
	j = m;
	DO(m, --j; if(av[j] != wv[j])R av[j] > wv[j] ? 1 : -1;);
	R 0;
...
static X jtshift10(J jt, I e, X w)
{
	A z;
	...
	if(r) {
		c = 0;
		DO(n, k = c + d**wv++; *zv++ = k % XBASE; c = k / XBASE;);
		if(c) {
			*zv = c;
		}
	} else {
...
B jtxdivrem(J jt, X a, X w, X*qz, X*rz)
{
	B b, c;
	I*av, d, j, n, *qv, r, y;
	X q;
	...
	switch(2 * b + c) {
	case 0:
		DO(n, --j; d = r * XBASE - av[j]; r = d % y; qv[j] =  d / y ;);
		r = -r;
		break;
	case 1:
		DO(n, --j; d = r * XBASE - av[j]; r = d % y; qv[j] = -(d / y););
...
X jtxdiv(J jt, X a, X w, I mode)
{
	...
	switch((0 <= c ? 2 : 0) + (0 <= d)) {
	case 0:
		R xdiv(negate(a), negate(w), mode);
	case 1:
		R negate(xdiv(negate(a), w, mode == XMFLR ? XMCEIL : mode == XMCEIL ? XMFLR : mode));
	case 2:
		R negate(xdiv(a, negate(w), mode == XMFLR ? XMCEIL : mode == XMCEIL ? XMFLR : mode));
	default:
		if(1 != (e = xcompare(a, w))) {
...
XF2(jtxrem)
{
	...
	switch((0 <= c ? 2 : 0) + (0 <= d)) {
	case 0:
		R negate(xrem(negate(a), negate(w)));
	case 1:
		y = xrem(negate(a), w);
		R xcompare(y, xzero) ? xplus(a, y) : y;
	case 2:
		y = xrem(a, negate(w));
		R xcompare(y, xzero) ? xminus(a, y) : y;
	default:
		R 0 <= (e = xcompare(a, w)) ? (e ? w : xzero) : xminus(w, xtymes(a, xdiv(w, a, XMFLR)));
	}
}
...
XF2(jtxlcm)
{
	R xtymes(a, xdiv(w, xgcd(a, w), XMEXACT));
}

static X jtxexp(J jt, X w, I mode)
{
	I k, m;
	X s, y;
	RZ(w);
...
XF2(jtxpow)
{
	...
	if(0 > d) {
		...
		r = jt->xmode == XMCEIL;
		R xc(0 < c ? r : 1 & e ? r - 1 : r);
	}
	t = a;
	z = xone;
	m = jt->xmod ? *XAV(jt->xmod) : 0;
	if(!m || 1 > xcompare(w, xc(IMAX))) {
		ASSERT(m || 2 >= AN(w), EVLIMIT);
		RE(e = xint(w));
		if(m)while(e) {
				if(1 & e) {
					RZ(z = xrem(m, xtymes(z, t)));
				}
				RZ(t = xrem(m, xsq(t)));
				e >>= 1;
			}
...
static XF2(jtxroot)
{
	...
	ASSERT(0 <= d, EWIMAG);
	if(1 == wn && (0 == d || 1 == d)) {
		R 1 == d ? xone : 0 <= c ? xzero : vci(XPINF);
	}
	if(!c && 0 < d) {
		R vci(XPINF);
	}
	...
	x = xlogabs(w) / r;
	if(x < 709.78) {
		RZ(q = ceil1(cvt(RAT, scf(exp(x)))));
		z = *XAV(q);
	} else        {
		RZ(q = cvt(XNUM, scf(ceil(x))));
		z = xexp(*XAV(q), XMCEIL);
	}
	RZ(n = xc(r));
	RZ(n1 = xc(r - 1));
	...
	if(XMFLR == jt->xmode || !xcompare(w, xtymes(z, p))) {
		R z;
	}
	if(XMCEIL == jt->xmode) {
		R xplus(z, xone);
...
static D jtxlogd1(J jt, X w)
{
	ASSERT(0 <= XDIG(w), EWIMAG);
	R xlogabs(w);
}

static Z jtxlogz1(J jt, X w)
{
	Z z;
	z.re = xlogabs(w);
...
XF2(jtxbin)
{
	...
	case 0: /* 0 0 0 */
		R xbinp(a, w);
	case 3: /* 0 1 1 */
		z = xbinp(a, xminus(a, xplus(w, xone)));
		R*AV(a) % 2 ? negate(z) : z;
	case 6: /* 1 1 0 */
		z = xbinp(xminus(xc(-1L), w), xminus(xc(-1L), a));
		R*AV(d) % 2 ? negate(z) : z;
	}
}

static A jtpiev(J jt, I n, X b)
{
	A e;
	I ek, i, n1 = n - 1;
	X bi, e0, e1, *ev, t;
...
F1(jtdigits10)
{
	...
	if(!AR(w))switch(AT(w)) {
		...
		case RAT:
			...
			n = AN(x);
			v = AV(x);
			b = 0 <= v[n - 1] && equ(iv1, QAV(w)->d);
		}
	if(!b) {
		R rank1ex(thorn1(w), 0L, 0L, jtexec1);
	}
	...
	zv = zv0;
	v = zv0 + n - 1;
	DO(n / 2, c = *zv; *zv++ = *v; *v-- = c;); /* reverse in place */
	R z;
}    /* "."0@": w */
