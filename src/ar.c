...
static void vdone(I m, I n, B*x, B*z, B pc)
{
	...
	switch((r ? 2 : 0) + pc) {
	case 0:
		DO(m, s = 0; DO(q, s ^= *y++;); PARITYW;                            *z++ = !b;);
		break;
	case 1:
		DO(m, s = 0; DO(q, s ^= *y++;); PARITYW;                            *z++ = b;);
		break;
	case 2:
		DO(m, s = 0; DO(q, s ^= *y++;); PARITYW; u = (UC*)y; DO(r, b ^= *u++;); *z++ = !b; x += n; y = (UI*)x;);
		break;
	case 3:
		DO(m, s = 0; DO(q, s ^= *y++;); PARITYW; u = (UC*)y; DO(r, b ^= *u++;); *z++ = b; x += n; y = (UI*)x;);
		break;
	...
	*z++ = s;
	...
	tu = (UC*)&t;
		...
		for(i = 0; i < m; ++i, z += d) {
			...
			memset(tv, C0, p);
			DO(r,   DO(dw, tv[i] += v[i];); x += d; v = (UI*)x;);
			DO(d, z[i] += tu[i];) ;
		}
	}
...
static A jtredsp1a(J jt, C id, A z, A e, I n, I r, I*s)
{
	A t;
	B b, p = 0;
	D d = 1;
	...
	case CSTARDOT:
		R n ? lcm(z, e) : ca(e);
	case CMIN:
		R n ? minimum(z, e) : ca(e);
	case CMAX:
		R n ? maximum(z, e) : ca(e);
	case CPLUS:
		if(n && equ(e, zero)) {
			R z;
		}
		DO(r, d *= s[i];);
		t = tymes(e, d > IMAX ? scf(d - n) : sc((I)d - n));
		R n ? plus (z, t) : t;
	case CSTAR:
		if(n && equ(e, one )) {
			R z;
		...
		DO(r, d *= s[i];);
		t = expn2(e, d > IMAX ? scf(d - n) : sc((I)d - n));
		R n ? tymes(z, t) : t;
	case CEQ:
		p = 1; /* fall thru */
	case CNE:
...
static A jtredsp1(J jt, A w, A self, C id, VF ado, I cv, I f, I r, I zt)
{
	A e, x, z;
	I m, n;
	P*wp;
...
DF1(jtredravel)
{
	...
	while(1) {
		...
		if(jt->jerr != EWOV) {
			R redsp1a(id, z, SPA(wp, e), n, AR(w), AS(w));
		};
	}
}  /* f/@, w */

static A jtredspd(J jt, A w, A self, C id, VF ado, I cv, I f, I r, I zt)
{
	A a, e, x, z, zx;
	I c, m, n, *s, t, *v, wr, *ws, xf, xr;
	P*wp, *zp;
	RZ(w);
	...
	c = m ? AN(x) / m : 0;
	n = s[xf];
	GA(zx, zt, AN(x) / n, AR(x) - 1, s);
	ICPY(xf + AS(zx), 1 + xf + s, xr - 1);
	ado(jt, m, c, n, AV(zx), AV(x));
	RE(0);
	switch(id) {
	case CPLUS:
		if(!equ(e, zero)) {
	...
	wr = AR(w);
	ws = AS(w);
	GA(z, STYPE(AT(zx)), 1, wr - 1, ws);
	if(1 < wr) {
		ICPY(f + AS(z), f + 1 + ws, wr - 1);
	}
	zp = PAV(z);
	RZ(a = ca(a));
	v = AV(a);
	DO(AN(a), if(f < v[i])--v[i];);
	SPB(zp, a, a);
...
static B jtredspsprep(J jt, C id, I f, I zt, A a, A e, A x, A y, I*zm, I**zdv, B**zpv, I**zqv, C**zxxv, A*zsn)
{
	...
	B*pv;
	C*xxv;
	I*dv, j, k, m, mm, *qv = 0, *u, *v, yc, yr, yr1, *yv;
	v = AS(y);
	yr = v[0];
	yc = v[1];
	yr1 = yr - 1;
	RZ(d = grade1(eq(a, sc(f))));
	dv = AV(d);
	DO(AN(a), if(i != dv[i]) {
	RZ(q = grade1p(d, y));
		qv = AV(q);
	...
	case CSTARDOT:
		...
		break;
	case CMIN:
		j = !equ(e, zt & B01 ? one : zt & INT ? sc(IMAX) : ainf     );
		break;
	case CMAX:
		j = !equ(e, zt & B01 ? zero : zt & INT ? sc(IMIN) : scf(infm));
		break;
	case CEQ:
		j = !*BAV(e);
		break;
...
static B jtredspse(J jt, C id, I wm, I xt, A e, A zx, A sn, A*ze, A*zzx)
{
	A b;
	B nz;
	I t, zt;
	...
	case CPLUS:
		if(nz) {
			RZ(zx = plus (zx,       tymes(e, sn) ));
		}
		RZ(e =       tymes(e, sc(wm)) );
		break;
	case CSTAR:
		if(nz) {
			RZ(zx = tymes(zx, bcvt(1, expn2(e, sn))));
		}
		RZ(e = bcvt(1, expn2(e, sc(wm))));
		break;
	case CPLUSDOT:
		if(nz) {
			RZ(zx = gcd(zx, from(b, over(zero, e))));
		}
		break;
	case CSTARDOT:
		if(nz) {
			RZ(zx = lcm(zx, from(b, over(one , e))));
		}
		break;
	case CMIN:
		if(nz) {
			RZ(zx = minimum(zx, from(b, over(zt & B01 ? one : zt & INT ? sc(IMAX) : ainf,     e))));
		}
		break;
	case CMAX:
		if(nz) {
			RZ(zx = maximum(zx, from(b, over(zt & B01 ? zero : zt & INT ? sc(IMIN) : scf(infm), e))));
		}
		break;
	case CEQ:
		ASSERT(B01 & xt, EVNONCE);
		if(nz) {
			RZ(zx = eq(zx, eq(zero, residue(num[2], sn))));
		}
		if(!(wm % 2)) {
			e = one;
		}
	...
	case CNE:
		ASSERT(B01 & xt, EVNONCE);
		if(nz) {
			RZ(zx = ne(zx, eq(one, residue(num[2], sn))));
		}
		if(!(wm % 2)) {
			e = zero;
		}
...
static A jtredsps(J jt, A w, A self, C id, VF ado, I cv, I f, I r, I zt)
{
	A a, a1, e, sn, x, x1 = 0, y, z, zx, zy;
	B*pv;
	C*xv, *xxv, *zv;
	I*dv, i, m, n, *qv, *sv, *v, wr, xk, xt, wm, *ws, xc, yc, yr, *yu, *yv, zk;
	P*wp, *zp;
	RZ(w);
	ASSERT(strchr(fca, id), EVNONCE);
	wr = AR(w);
	...
	xv = CAV(x);
	xk = xc * bp(xt);
	GA(zx, zt, m * xc, AR(x), AS(x));
	*AS(zx) = m;
	zv = CAV(zx);
	zk = xc * bp(zt);
	GA(zy, INT, m * (yc - 1), 2, 0);
	v = AS(zy);
	v[0] = m;
	v[1] = yc - 1;
	yu = AV(zy);
	...
	for(i = 0; i < m; ++i) {
		...
		if(1 < n) {
			if(qv) {
				u = xxv;
				DO(n, MC(u, xv + xk * v[i], xk); u += xk;);
			}
			ado(jt, 1L, n * xc, n, zv, qv ? xxv : xv);
			RE(0);
		} else   if(zk == xk) {
			MC(zv, qv ? xv + xk**v : xv, xk);
		} else   {
			if(!x1) {
				GA(x1, xt, xc, 1, 0);
			}
			MC(AV(x1), qv ? xv + xk**v : xv, xk);
			RZ(y = cvt(zt, x1));
			MC(zv, AV(y), zk);
		}
		zv += zk;
	...
	if(sn) {
		RZ(redspse(id, wm, xt, e, zx, sn, &e, &zx));
	}
	RZ(a1 = ca(a));
	v = AV(a1);
	n = 0;
	DO(AN(a), if(f != v[i])v[n++] = v[i] - (f < v[i]););
	GA(z, STYPE(AT(zx)), 1, wr - 1, ws);
	if(1 < r) {
		ICPY(f + AS(z), f + 1 + ws, r - 1);
	}
	zp = PAV(z);
...
static DF1(jtreducesp)
{
	A a, g, x, y, z;
	B b;
	C id;
	I cv, f, n, r, rr[2], *v, wn, wr, *ws, wt, zt;
	P*wp;
	VF ado;
	RZ(w);
	wr = AR(w);
	...
	jt->rank = 0;
	if(1 == wr) {
		z = redsp1(w, self, id, ado, cv, f, r, zt);
	} else {
		wp = PAV(w);
		a = SPA(wp, a);
		v = AV(a);
...
static B jtreduce2(J jt, A w, C id, I f, I r, A*zz)
{
	A z = 0;
	B b = 0, btab[258], *zv;
	I c, d, m, wn, wr, *ws, *wv;
	...
	wv = AV(w);
	m = prod(f, ws);
	c = m ? wn / m : prod(r, f + ws);
	d = c / 2;
	switch(BR2CASE(AT(w), id)) {
	case BR2CASE(B01, CEQ     ):
		if(b = 1 == r) {
			BTABIFX( ==   );
		}
		break;
	case BR2CASE(B01, CNE     ):
		if(b = 1 == r) {
			BTABIFX( !=   );
		}
		break;
	case BR2CASE(B01, CLT     ):
		if(b = 1 == r) {
			BTABIFX( <    );
		}
		break;
	case BR2CASE(B01, CLE     ):
		if(b = 1 == r) {
			BTABIFX( <=   );
		}
		break;
	case BR2CASE(B01, CGT     ):
		if(b = 1 == r) {
			BTABIFX( >    );
		}
		break;
	case BR2CASE(B01, CGE     ):
		if(b = 1 == r) {
			BTABIFX( >=   );
		}
		break;
	case BR2CASE(B01, CMAX    ):
	case BR2CASE(B01, CPLUSDOT):
		if(b = 1 == r) {
			BTABIFX( ||   );
		}
		break;
	case BR2CASE(B01, CPLUSCO ):
		if(b = 1 == r) {
			BTABPFX(BNOR );
		}
		break;
	...
	case BR2CASE(B01, CSTARDOT):
		if(b = 1 == r) {
			BTABIFX( &&   );
		}
		break;
	case BR2CASE(B01, CSTARCO ):
		if(b = 1 == r) {
			BTABPFX(BNAND);
		}
		break;
	case BR2CASE(LIT, CEQ     ):
		BR2IFX(C, == );
		break;
	case BR2CASE(LIT, CNE     ):
		BR2IFX(C, != );
		break;
	case BR2CASE(INT, CEQ     ):
		BR2IFX(I, == );
		break;
	case BR2CASE(INT, CLT     ):
		BR2IFX(I, <  );
		break;
	case BR2CASE(INT, CLE     ):
		BR2IFX(I, <= );
		break;
	case BR2CASE(INT, CGT     ):
		BR2IFX(I, >  );
		break;
	case BR2CASE(INT, CGE     ):
		BR2IFX(I, >= );
		break;
	case BR2CASE(INT, CNE     ):
		BR2IFX(I, != );
		break;
	case BR2CASE(FL, CEQ     ):
		BR2PFX(D, TEQ);
...
static DF1(jtreduce)
{
	A z;
	C id;
	I c, cv, f, m, n, r, rr[2], t, wn, wr, *ws, wt, zt;
	VF ado;
	RZ(w);
	wr = AR(w);
	r = jt->rank ? jt->rank[1] : wr;
	...
	if(jt->jerr) {
		R jt->jerr == EWOV ? (rr[1] = r, jt->rank = rr, reduce(w, self)) : 0;
	} else {
		R cv&VRI + VRD ? cvz(cv, z) : z;
	}
}    /* f/"r w main control */

static A jtredcatsp(J jt, A w, A z, I r)
{
	A a, q, x, y;
	B*b;
	I c, d, e, f, j, k, m, n, n1, p, *u, *v, wr, *ws, xr;
	P*wp, *zp;
	ws = AS(w);
	wr = AR(w);
	f = wr - r;
	...
	if(c && d) {       /* sparse sparse */
		...
		u = AS(x);
		GA(q, AT(x), AN(x), xr - 1, u);
		v = k + AS(q);
		*v = u[k] * u[1 + k];
		ICPY(1 + v, 2 + k + u, xr - k - 2);
...
static DF1(jtredcat)
{
	...
	if(ARELATIVE(w)) {
		AFLAG(z) = AFREL;
		z = relocate((I)w - (I)z, z);
	}
	R z;
}    /* ,/"r w */

static DF1(jtredsemi)
{
	I f, n, r, *s, wr;
	RZ(w);
	wr = AR(w);
	...
	if(BOX & AT(w)) {
		R irs2(rank1ex(curtail(w), 0L, r - 1, jtbox), tail(w), 0L, r, r - 1, jtover);
	} else {
		R irs1(w, 0L, r - 1, jtbox);
	}
}    /* ;/"r w */

static DF1(jtredstitch)
{
	A c, y;
	I f, n, r, *s, *v, wr;
	RZ(w);
	...
	ASSERT(n, EVDOMAIN);
	if(1 == n) {
		R irs1(w, 0L, r, jthead);
	}
	if(1 == r) {
		R 2 == n ? ca(w) : irs2(irs2(num[-2], w, 0L, 0L, 1L, jtdrop), irs2(num[-2], w, 0L, 0L, 1L, jttake), 0L, 1L, 0L, jtover);
	}
	if(2 == r) {
		R irs1(w, 0L, 2L, jtcant1);
	}
	RZ(c = IX(wr));
	v = AV(c);
	v[f] = f + 1;
	...
	if(SPARSE & AT(w)) {
		...
		v = AV(x);
		ICPY(v, AS(y), f + 1);
		RE(v[f + 1] = mult(s[f], s[f + 2]));
		ICPY(v + f + 2, s + 3 + f, r - 3);
		R reshape(x, y);
	} else {
		v = AS(y);
		RE(v[f + 1] = mult(s[f], s[f + 2]));
...
static DF1(jtredstiteach)
{
	A*wv, y;
	I n, p, r, t, wd;
	RZ(w);
	n = AN(w);
	if(!(2 < n && 1 == AR(w) && BOX & AT(w))) {
		R reduce(w, self);
	}
	wv = AAV(w);
	wd = (I)w * ARELATIVE(w);
...
static DF1(jtredcateach)
{
	A*u, *v, *wv, x, *xv, z, *zv;
	I f, m, mn, n, r, wd, wr, *ws, zm, zn;
	RZ(w);
	wr = AR(w);
	ws = AS(w);
	r = jt->rank ? jt->rank[1] : wr;
	...
	n = r ? ws[f] : 1;
	if(!r || 1 >= n) {
		R reshape(repeat(ne(sc(f), IX(wr)), shape(w)), n ? w : ace);
	}
	if(!(BOX & AT(w))) {
		R df1(cant2(sc(f), w), qq(ds(CBOX), one));
	}
	zn = AN(w) / n;
	zm = prod(f, ws);
	m = zm ? AN(w) / (zm * n) : prod(r - 1, ws + f + 1);
	mn = m * n;
	GA(z, BOX, zn, wr - 1, ws);
	ICPY(AS(z) + f, ws + f + 1, r - 1);
	GA(x, BOX, n, 1, 0);
	xv = AAV(x);
	zv = AAV(z);
	wv = AAV(w);
...
A jtaslash (J jt, C c,    A w)
{
	RZ(   w);
	R df1(  w,   slash(ds(c))     );
}
A jtaslash1(J jt, C c,    A w)
{
	RZ(   w);
	R df1(  w, qq(slash(ds(c)), one));
}
A jtatab   (J jt, C c, A a, A w)
{
	RZ(a && w);
	R df2(a, w,   slash(ds(c))     );
}


static AHDRR(jtmeanD, D, D)
{
	I d, i;
	D*y;
	D v, *zz;
...
DF1(jtmean)
{
	...
	n = r ? ws[f] : 1;
	if(!(wn && 2 < n && wt & INT + FL)) {
		R divide(df1(w, qq(slash(ds(CPLUS)), sc(r))), sc(n));
	}
	GA(z, FL, wn / n, MAX(0, wr - 1), ws);
	if(1 < r) {
		ICPY(f + AS(z), f + 1 + ws, r - 1);
	}
	m = prod(f, ws);
	c = m ? wn / m : prod(r, f + ws);
	if(wt & INT) {
		meanI(m, c, n, DAV(z), AV(w));
	} else {
		meanD(m, c, n, DAV(z), DAV(w));
	}
	RE(0);
	R z;
}    /* (+/%#)"r w */
