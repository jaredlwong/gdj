...
F1(jtpinv)
{
	...
	n = AN(w);
	v = AV(w);
	DO(n, m = 0 > v[i] ? MAX(m, -1 - v[i]) : MAX(m, v[i]););
	m += 0 < n;
	R indexof(pfill(m, w), IX(m));
}    /* permutation inverse */

A jtpind(J jt, I n, A w)
{
	A z;
	I j, m, *v;
	RE(n);
...
static F1(jtcfd)
{
	...
	AN(z) = *AS(z) = zn = i;
	j = zn - 1;
	DO(zn / 2, x = zv[i]; zv[i] = zv[j]; zv[j] = x; --j;);
	R z;
}    /* cycle from direct */

static A jtdfc(J jt, I n, A w)
{
	PROLOG;
	A b, q, *wv, z;
	B*bv;
	...
	for(j = AN(w) - 1; 0 <= j; j--) {
		...
		DO(qn, ASSERT(bv[qv[i]], EVINDEX); bv[qv[i]] = 0;);
		DO(qn, bv[qv[i]] = 1;);
		c = x[qv[0]];
		DO(qn - 1, x[qv[i]] = x[qv[i + 1]];);
		x[qv[qn - 1]] = c;
	}
	EPILOG(z);
}    /* direct from cycle */
...
{
	F1RANK(1, jtcdot1, 0);
	R BOX&AT(w) ? dfc(ord(raze(w)), w) : cfd(w);
}
...
F1(jtpparity)
{
	...
	for(i = 0; i < m; ++i) {
		j = p = 0;
		memset(u, C1, n);
		DO(n, k = v[i]; if(0 > k)v[i] = k += n; if(0 <= k && k < n && u[k])u[k] = 0; else {
				j = 1 + n;
				break;
			});
		for(; j < n; ++j)if(j != v[j]) {
