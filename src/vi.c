...
{
	UI z = 0;
	DO(k,       z = (i + 1000003)**v++   ^z << 1;      );
	R z;
}

static UI hicnz(    I k, UC*v)
{
	UI z = 0;
	UC c;
	DO(k, c = *v++; if(c && c != 255)z = (i + 1000003) * c ^ z << 1;);
	R z;
}

static UI hicx(J jt, I k, UC*v)
{
	UI z = 0;
	I*u = jt->hiv;
	DO(jt->hin, z = (i + 1000003) * v[*u++] ^ z << 1;      );
	R z;
}
...
static UI jthid(J jt, D d)
{
	R 10495464745870458733U * (jt->ctmask&*(I*)&d);
}
#else
...
static UI jthid(J jt, D d)
{
	DI x;
	x.d = d;
	R 888888883U * (x.i[LSW]&jt->ctmask) + 2838338383U * x.i[MSW];
}
#endif


static UI jthia(J jt, D hct, A y)
{
	UC*yv;
	D d;
	I n, t;
	...
	case RAT:
		u = (Q*)yv;
		d = xdouble(u->n) / xdouble(u->d);
	}
	R hid(d * hct);
}

static UI jthiau(J jt, A y)
{
	I m, n;
	UC*v = UAV(y);
	UI z = 2038074751;
	...
	case XNUM:
		u = XAV(y);
		DO(m, x = *u++; v = UAV(x); z += hicnz(AN(x) * SZI, UAV(x)););
		R z;
	case INT:
		z = hicnz(n    * SZI, UAV(y));
		R z;
	default:
		R hic(n * bp(AT(y)), UAV(y));
	}
}
...
static B jteqx(J jt, I n, X*u, X*v)
{
	DO(n, if(!equ(*u, *v))R 0; ++u; ++v;);
	R 1;
}
static B jteqq(J jt, I n, Q*u, Q*v)
{
	DO(n, if(!QEQ(*u, *v))R 0; ++u; ++v;);
	R 1;
}
static B jteqd(J jt, I n, D*u, D*v)
{
	DO(n, if(!teq(*u, *v))R 0; ++u; ++v;);
	R 1;
}
static B jteqz(J jt, I n, Z*u, Z*v)
{
	DO(n, if(!zeq(*u, *v))R 0; ++u; ++v;);
	R 1;
}

static B jteqa(J jt, I n, A*u, A*v, I c, I d)
{
	DO(n, if(!equ(AADR(c, *u), AADR(d, *v)))R 0; ++u; ++v;);
	R 1;
}
...
static IOF(jtiosc)
{
	...
	case C2T:
		...
		break;
	case CMPX:
		SCDO(Z, *wv, !zeq(x, av[j]));
		break;
	case XNUM:
		SCDO(A, *wv, !equ(x, av[j]));
		break;
	case RAT:
		SCDO(Q, *wv, !QEQ(x, av[j]));
		break;
	case INT:
		SCDO(I, *wv, x != av[j]      );
		break;
	...
	case BOX:  {
		RDECL;
		SCDO(A, AADR(wd, *wv), !equ(x, AADR(ad, av[j])));
	}
	break;
	case FL:
		if(0 == jt->ct)SCDO(D, *wv, x != av[j])
			else {
				SCDO(D, *wv, !teq(x, av[j]));
			}
	}
	R z;
...
static B jtusebs(J jt, A a, I ac, I m)
{
	A*av, x;
	I ad, t;
	if(!(BOX & AT(a) && 0 == jt->ct)) {
...
static A jtnodupgrade(J jt, A a, I acr, I ac, I acn, I ad, I n, I m, B b, B bk)
{
	A*av, h, *u, *v;
	I*hi, *hu, *hv, l, m1, q;
	RZ(h = irs1(a, 0L, acr, jtgrade1));
	hv = AV(h) + bk * (m - 1);
	av = AAV(a);
	if(!b)for(l = 0; l < ac; ++l, av += acn, hv += m) {
			...
			if(bk) {
				hu = --hi;
				DO(m - 1, q = *hi--; v = av + n * q; if(!eqa(n, u, v, ad, ad)) {
				u = v;
				*hu-- = q;
			});
				m1 = hv - hu;
...
static IOF(jtiobs)
{
	...
	B b, bk, *yb, *zb;
	C*zc;
	I acn, ad, *hu, *hv, l, m1, md, s, wcn, wd, *zi, *zv;
	...
	zc = (C*)zv;
	if(mode < IPHOFFSET) {
		RZ(*hp = h = nodupgrade(a, acr, ac, acn, ad, n, m, b, bk));
	}
	if(w == mark) {
		R mark;
	}
	...
	for(l = 0; l < ac; ++l, av += acn, wv += wcn, hv += m) {
		...
		if(b)switch(md) {
			case IIDOT:
				BSLOOPAA(hi++, zv[p] = p, zv[q] = p, zv[q] = p = q);
				zv += m;
				break;
			case IICO:
				BSLOOPAA(hi--, zv[p] = p, zv[q] = p, zv[q] = p = q);
				...
				break;
			case INUBSV:
				BSLOOPAA(hi++, zb[p] = 1, zb[q] = 0, zb[q] = 1  );
				zb += m;
				break;
			case INUB:
				BSLOOPAA(hi++, yb[p] = 1, yb[q] = 0, yb[q] = 1  );
				DO(m, if(yb[i]) {
				MC(zc, av + i * n, k);
					zc += k;
				});
...
{
	I q = m + m, *v = ptab;
	DO(nptab - 1, if(q <= *v)break; ++v;);
	R*v;
}


A jtindexofsub(J jt, I mode, A a, A w)
{
	PROLOG;
	A h = 0, hi = mtv, z = mtv;
	AF fn;
	...
	RZ(a && w);
	ar = AR(a);
	acr = jt->rank ? jt->rank[0] : ar;
	af = ar - acr;
	wr = AR(w);
	wcr = jt->rank ? jt->rank[1] : wr;
	wf = wr - wcr;
	jt->rank = 0;
	as = AS(a);
	at = AT(a);
	...
	if(mk) {
		...
		r = acr ? acr - 1 : 0;
		f1 = wcr - r;
		if(0 > f1 || ICMP(as + af + 1, ws + wf + f1, r)) {
			I f0, *v;
			m = acr ? as[af] : 1;
			f0 = MAX(0, f1);
			RE(zn = mult(prod(f, s), prod(f0, ws + wf)));
			switch(mode) {
			case IIDOT:
			case IICO:
				GA(z, INT, zn, f + f0, s);
				if(af) {
					ICPY(f + AS(z), ws + wf, f0);
				}
				v = AV(z);
				DO(zn, *v++ = m;);
				R z;
			...
			case INUB:
			case INUBI:
				ASSERTSYS(0, "indexofsub"); // impossible
			}
		}
	}
	if(at & SPARSE || wt & SPARSE) {
		if(1 >= acr) {
			R af ? sprank2(a, w, 0L, acr, RMAX, jtindexof) : wt&SPARSE ? iovxs(mode, a, w) : iovsd(mode, a, w);
		}
		if(af || wf) {
			R sprank2(a, w, 0L, acr, wcr, jtindexof);
		}
		switch((at & SPARSE ? 2 : 0) + (wt & SPARSE ? 1 : 0)) {
		case 1:
			EPILOG(indexofxx(mode, a, w));
		case 2:
			EPILOG(indexofxx(mode, a, w));
		case 3:
			EPILOG(indexofss(mode, a, w));
		}
	}
	m = acr ? as[af] : 1;
	n = acr ? prod(acr - 1, as + af + 1) : 1;
	RE(zn = mult(prod(f, s), prod(f1, ws + wf)));
	RE(t = mk ? at : maxtype(at, wt));
	k1 = bp(t);
	k = n * k1;
	th = HOMO(at, wt);
	jt->min = ss = 0;
	...
	wk *= 1 < wc;
	if(th && t != at) {
		RZ(a = t & XNUM ? xcvt(XMEXMT, a) : cvt(t, a))
	} else if(t & FL + CMPX) {
		RZ(a = cvt0(a))
	}
	if(th && t != wt) {
		RZ(w = t & XNUM ? xcvt(XMEXMT, w) : cvt(t, w))
	} else if(t & FL + CMPX && a != w) {
		RZ(w = cvt0(w))
	}
	if(AT(a)&INT + SBT && k == SZI) {
		I r;
		irange(AN(a)*k1 / SZI, AV(a), &r, &ss);
		if(ss) {
			jt->min = r;
		}
	}
	...
	if(!mk)switch(mode) {
		...
		case IICO:
			GA(z, INT, zn, f + f1,     s);
			if(af) {
				ICPY(f + AS(z), ws + wf, f1);
			}
			break;
		case INUBSV:
			GA(z, B01, zn, f + f1 + !acr, s);
			if(af) {
				ICPY(f + AS(z), ws + wf, f1);
			}
			if(!acr) {
		...
		case INUB:
			q = MIN(m, p);
			GA(z, t, mult(q, aii(a)), MAX(1, wr), ws);
			*AS(z) = q;
			break;
		case ILESS:
			GA(z, t, AN(w), MAX(1, wr), ws);
			break;
		case IEPS:
			GA(z, B01, zn, f + f1,     s);
			if(af) {
	...
	if(!(mk || m && n && zn && th))switch(mode) {
		case IIDOT:
			R reshape(shape(z), sc(n ? m : 0  ));
		case IICO:
			R reshape(shape(z), sc(n ? m : m - 1));
		case INUBSV:
			R reshape(shape(z), take(sc(m), one));
		case INUB:
			AN(z) = 0;
			*AS(z) = m ? 1 : 0;
			R z;
		case ILESS:
			if(m) {
				...
				MC(AV(z), AV(w), k1 * AN(w));
			}
			R z;
		case IEPS:
			R reshape(shape(z), m && (!n || th) ? one : zero);
		case INUBI:
			R m ? iv0 : mtv;
		case II0EPS:
			R sc(n ? 0L        : c         );
	...
	if(a != w && !mk && 1 == acr && (1 == wc || ac == wc) && (D)m * n * zn < 13 * ((D)m * n + zn) && (mode == IIDOT || mode == IICO || mode == IEPS)) {
		...
		if     (t & BOX)          fn = b && (1 < n || usebs(a, ac, m)) ? jtiobs : 1 < n ? jtioa : b ? jtioax1 :
			...
			fn = b || t & B01 + JCHAR + INT + SBT ? jtioc : 1 == n ? (t & FL ? jtiod1 : jtioz1) : t & FL ? jtiod : jtioz;
		}
		if(fn != jtiobs) {
			GA(h, ht, p, 1, 0);
		}
	...
	if(mk) {
		...
		default:
			ztype = 0;
			break;  /* integer vector      */
		case ILESS:
			ztype = 1;
			break;  /* type/shape from arg */
		case IEPS:
			ztype = 2;
			break;  /* boolean vector      */
		case IANYEPS:
		case IALLEPS:
			ztype = 3;
			break;  /* boolean scalar      */
		case ISUMEPS:
		case II0EPS:
		case II1EPS:
		case IJ0EPS:
		case IJ1EPS:
			ztype = 4;       /* integer scalar      */
		}
		xv[0] = mode;
		xv[1] = n;
		xv[2] = k;
...
A jtindexofprehashed(J jt, A a, A w, A hs)
{
	A h, hi, *hv, x, z;
	AF fn;
	I ar, *as, at, c, f1, k, m, mode, n,
	...
	RE(c = prod(f1, ws));
	if(mode == ILESS && (t != wt || AFLAG(w)&AFNJA + AFREL || n != aii(w))) {
		R less(w, a);
	}
	if(!(m && n && c && HOMO(t, wt) && t >= wt)) {
		R indexofsub(mode, a, w);
	}
	switch(ztype) {
	case 0:
		GA(z, INT, c,    f1, ws);
...
{
	RZ(w);
	if(SPARSE & AT(w) || AFLAG(w)&AFNJA + AFREL) {
		R repeat(nubsieve(w), w);
	}
...
F2(jteps)
{
	...
	rv[1] = l = jt->rank ? jt->rank[0] : AR(a);
	jt->rank = 0;
	if(SPARSE & AT(a) + AT(w)) {
		R lt(irs2(w, a, 0L, r, l, jtindexof), sc(r ? * (AS(w) + AR(w) - r) : 1));
	}
	jt->rank = rv;
...
F1(jtnubind0)
{
	...
	RZ(w);
	jt->ct = 0.0;
	z = SPARSE & AT(w) ? icap(nubsieve(w)) : indexofsub(INUBI, w, w);
	jt->ct = oldct;
	R z;
}    /* I.@(~:!.0) w */
...
F1(jtsclass)
{
	...
	RZ(w);
	if(!AR(w)) {
		R reshape(v2(1L, 1L), one);
	}
	n = IC(w);
	RZ(x = indexof(w, w));
	if(DENSE & AT(w)) {
		R atab(CEQ, repeat(eq(IX(n), x), x), x);
	}
	p = PAV(x);
	e = SPA(p, e);
	y = SPA(p, i);
	RZ(xy = stitch(SPA(p, x), y));
	if(n > *AV(e)) {
		RZ(xy = over(xy, stitch(e, less(IX(n), y))));
	}
	RZ(xy = grade2(xy, xy));
	v = AV(xy);
	c = *AS(xy);
...
A jtiocol(J jt, I mode, A a, A w)
{
	A h, z;
	I ar, at, c, d, m, p, t, wr, *ws, wt;
	void(*fn)();
	RZ(a && w);
	...
	default:
		ASSERT(0, EVNONCE);
	case FL:
		fn = mode == IICO ? jtjocold : jtiocold;
		ctmask(jt);
		break;
	case CMPX:
		fn = mode == IICO ? jtjocolz : jtiocolz;
		ctmask(jt);
		break;
	}
	fn(jt, m, c, d, a, w, z, h);
