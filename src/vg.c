...
static void jtmsmerge(J jt, I n, I*u, I*v)
{
	I m, q, *x, *xx, *y, *yy, *z;
	int c;
	q = n / 2;
...
void jtmsort(J jt, I n, I*u, I*v)
{
	I a, b, c, d, q, ui, uj, uk, ul, um;
	switch(n) {
	case 2:
		if(VS(0, 1)) {
			XC(0, 1);
...
static GF(jtgrx)
{
	...
	switch(t) {
	case BOX:
		jt->comp = ARELATIVE(w) ? compr : compa;
		break;
	case C2T:
		jt->comp = compu;
		break;
	case INT:
		jt->comp = c == n ? compi1 : compi;
		break;
	case FL:
		jt->comp = c == n ? compd1 : compd;
		break;
	case CMPX:
		jt->comp = compd;
		jt->compn = 2 * d;
...
void irange(I n, I*v, I*base, I*top)
{
	I d, i, m = n / 2, p, q, x, y;
	if(n > m + m) {
		p = q = *v++;
...
static GF(jtgri)
{
	A x, y;
	B b, up;
	I d, e, *g, *h, i, j, k, p, ps, q, s, *v, *wv, *xv, *yv;
	wv = AV(w);
	d = c / n;
	k = 4 * n;
	irange(AN(w), wv, &q, &p);
	if(!p || k < p || (0.69 * d * (p + 2 * n)) > n * log((D)n)) {
		R c == n&&n > 65536 / 1.5 ? gri1(m, c, n, w, zv) : grx(m, c, n, w, zv);
	}
	if(0 < q && q < k - p) {
		p += q;
		q = 0;
	...
	for(e = d - 2, b = 0; 0 <= e; --e) {
		...
		if(b) {
			...
			if(q) {
			DO(n, h[yv[*(v + d * g[i]) - q]++] = g[i];
		})
		else {
			DO(n, h[yv[*(v + d * g[i])  ]++] = g[i];
		});
	}
	if(b) {
		DO(n, zv[i] = xv[i];
...
F2(jtgrade2 )
{
	GBEGIN( 1, -1);
	RZ(a && w);
	z = SPARSE & AT(w) ? grd2sp(a, w) : gr2(a, w);
	GEND(z);
}
F2(jtdgrade2)
{
...
{
	A q, t = 0;
	I c, i = 0, j, m, m0, m1, n, qn = 53, *qv, wt;
	RZ(a && w);
	n = AN(w);
	wt = AT(w);
	if(!(!AR(a) && AT(a)&B01 + INT&&4 < n && 1 == AR(w) && wt & FL + INT)) {
		R from(a, grade2(w, w));
	}
	RE(j = i0(a));
	if(0 > j) {
	...
	RZ(q = df2(sc(qn), sc(IMAX), atop(ds(CQUERY), ds(CDOLLAR))));
	qv = AV(q);
	if(wt & FL) {
		OSLOOP(D, scf)
	} else {
