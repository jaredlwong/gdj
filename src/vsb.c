...
static void showdepth(J jt, I node, int **ptr, I* size, I depth)
{
	if(LEFT(node) == 0) {
		...
		showdepth(jt,  LEFT(node), ptr, size, depth + 1);
	}
	...
	if(RIGHT(node) == 0) {
		...
		showdepth(jt, RIGHT(node), ptr, size, depth + 1);
	}
}
#endif

static __inline int Vcompare(J jt, I a, I b)
{
	I m, n;
	SBU*u, *v;
	UC*s, *t;
	...
	case 0: {
		DO(MIN(m, n), if(*s != *t)R * s < *t; ++s; ++t;);
	}
	break;
	case 1: {
		q = (U2*)t;
		n /= 2;
		DO(MIN(m, n), if(*s != *q)R * s < *q; ++s; ++q;);
	}
	break;
	case 2: {
		p = (U2*)s;
		m /= 2;
		DO(MIN(m, n), if(*p != *t)R * p < *t; ++p; ++t;);
	}
	break;
	case 3: {
		p = (U2*)s;
		...
		m /= 2;
		n /= 2;
		DO(MIN(m, n), if(*p != *q)R * p < *q; ++p; ++q;);
	}
	}
	R m < n;
}

static __inline void rotateLeft(J jt, I x)
{
	I y;
#ifdef TMP
	tmp_lr++;
	...
	y = RIGHT(x);
	if (RIGHT(x) = LEFT(y)) {
		PARENT(LEFT(y)) = x;    /* establish x->right link  */
	}
	if (y) {
		PARENT(y)       = PARENT(x);    /* establish y->parent link */
	}
	if (PARENT(x) == 0) {
		ROOT            = y;
	...
	if (LEFT(y) = x) {
		PARENT(x)       = y;    /* link x and y             */
	}
}


static __inline void rotateRight(J jt, I x)
{
	I y;
#ifdef TMP
	tmp_rr++;
	...
	y = LEFT(x);
	if (LEFT(x) = RIGHT(y)) {
		PARENT(RIGHT(y)) = x;    /* establish x->left link   */
	}
	if (y) {
		PARENT(y)       = PARENT(x);    /* establish y->parent link */
	}
...
static __inline void insertFixup(J jt, I x)
{
	B b;
	I y;
...
{
	I current, dist, i, keep1, keep2, lorder, parent, rorder, to_the_left, to_the_right;
	SBU *x;
#ifdef TMP
	static I icount = 0;
#endif
	...
	while (current != 0) {
		if (compEQ(key, current)) {
			return STATUS_DUPLICATE_KEY;
		}
		parent = current;
		if(compLT(key, current)) {
			to_the_right = current;
	...
	if(rorder - lorder < 2) { /*  if(rorder-lorder<(2*GAP)) { */
		...
		while(to_the_right && to_the_left && (ORDER(to_the_right) - ORDER(to_the_left)) < (GAP*++i)) {
			keep1 = to_the_left;
			to_the_left = DOWN(to_the_left);
			keep2 = to_the_right;
			to_the_right = UP  (to_the_right);
		}

#ifdef TMP
		if(!to_the_left ) {
			UP(0)  = keep1, i++, tmp_lhit++ ;
		}
		if(!to_the_right) {
			DOWN(0) = keep2, i++, tmp_rhit++;
		}
		...
		while(--i) {
			to_the_left = UP  (to_the_left);
			lorder += dist;
			ORDER(to_the_left ) = lorder;
			to_the_right = DOWN(to_the_right);
			rorder -= dist;
			ORDER(to_the_right) = rorder;
		}
	}
...
static I jtsbextend(J jt, I n, C*s, UI h, I hi)
{
	A x;
	I c, *hv, j, p;
	SBU*v;
	...
	if(AN(jt->sbs) < n + jt->sbsn) {       /* extend sbs strings           */
		GA(x, LIT, 2 * (n + jt->sbsn), 1, 0);
		MC(CAV(x), jt->sbsv, jt->sbsn);
		fa(jt->sbs);
		ra(x);
		jt->sbs = x;
		jt->sbsv = CAV(x);
...
static SB jtsbinsert(J jt, B c2, I n, C*s, UI h, I hi)
{
	I c, m, p;
	SBU*u;
...
static SB jtsbprobe(J jt, B c2, I n, C*s)
{
	B b;
	C*t;
	I hi, hn, ui;
	...
	while(1) {
		...
		if(0 > ui) {
			R sbinsert(c2, n, s, h, hi);    /* new symbol                   */
		}
		u = SBUV(ui);
...
static A jtsbunstr(J jt, I q, A w)
{
	A z;
	B c2;
	I i, j, m, wn;
	...
	if(c2) {
		...
		zv = SBAV(z);
		if(q == -1) {
			for(i = j = 1; i <= wn; ++i)if(c == wv[i] || i == wn) {
					RE(*zv++ = sbprobe(c2, 2 * (i - j), (C*)(j + wv)));
					j = i + 1;
				}
		} else     {
			for(i = j = 0; i < wn; ++i)if(c == wv[i]       ) {
					RE(*zv++ = sbprobe(c2, 2 * (i - j), (C*)(j + wv)));
					j = i + 1;
				}
		}
...
static A jtsbunlit(J jt, C cx, A w)
{
	A z;
	B c2;
	I i, m, wc, wr, *ws;
	...
	zv = SBAV(z);
	if(!wc) {
		memset(zv, C0, m * sizeof(SB));
	} else if(c2) {
		C2 c = (C2)cx, *s, *wv = (C2*)AV(w);
		for(i = 0; i < m; ++i) {
			s = wc + wv;
			DO(wc, if(c != *--s)break;); /* exclude trailing "blanks"    */
...
F1(jtsb1)
{
	...
	case C2T:
	case LIT:
		abc = (1 >= AR(w) ? sbunstr(-1L, w) : sbunlit(' ', w));
		break;
	case BOX:
		abc = (sbunbox(w));
	}
...
F1(jtsb1)
{
	...
	case C2T:
	case LIT:
		R 1 >= AR(w) ? sbunstr(-1L, w) : sbunlit(' ', w);
	case BOX:
		R sbunbox(w);
	}
...
static A jtsblit(J jt, C c, A w)
{
	...
	v = v0 = SBAV(w);
	ASSERT(!n || SBT & AT(w), EVDOMAIN);
	DO(n, u = SBUV(*v++); k = u->n; if(u->flag & SBC2) {
	c2 = 1;
	k /= 2;
}
if(m < k)m = k;);
	v = v0;
	GA(z, c2 ? C2T : LIT, n * m, 1 + AR(w), AS(w));
	*(AR(w) + AS(z)) = m;
	if(c2) {
		C2*zv = (C2*)AV(z);
		DO(n, u = SBUV(*v++); C2FSB(zv, u, 3, m, c););
	} else  {
		C*zv = CAV(z);
		memset(zv, c, n * m);
		DO(n, u = SBUV(*v++); MC(zv, SBSV(u->i), u->n); zv += m;);
	}
	R z;
}    /* literal array for symbol array w padded with c */


static F1(jtsbhashstat)
{
	A z;
	I j, k, n, p, *zv;
	SBU*v;
	...
	GA(z, INT, n, 1, 0);
	zv = AV(z);
	DO(n, j = v++->h % p; k = 1; while(i != (jt->sbhv)[j]) {
	j = (j + 1) % p;
		++k;
	} *zv++ = k;);
	R z;
...
static A jtsbcheck1(J jt, A una, A sna, A u, A s, A h, A roota, A ff, A gp)
{
	PROLOG;
	A x, *xv, y;
	B b, *dnv, *lfv, *rtv, *upv;
	C*ptv, *sv;
	I c, f, g, hn, *hv, i, j, r, sn, un, *yv;
	SBU*uv, *v;
	RZ(una && sna && u && s && h);
	...
	b = 0;
	DO(AN(h), j = hv[i]; if(-1 == j)b = 1; else {
		ASSERTD(0 <= j && j < c, "h index");
		});
	ASSERTD(b, "h full");
	GA(x, B01, c, 1, 0);
	lfv = BAV(x);
	...
	GA(x, BOX, c, 1, 0);
	xv = AAV(x);
	RZ(xv[0] = str(uv->n, sv + uv->i));
	GA(y, INT, c, 1, 0);
	yv = AV(y);
	yv[0] = uv->order;
	for(i = 1, v = 1 + uv; i < c; ++i, ++v) {
		B c2;
		I ord, vi, vn;
		UC*vc;
		UI k;
		...
		yv[i] = ord = v->order;
		j = v->parent;
		ASSERTD(    0 <= j && j < c && 2 >= ++ptv[j], "u parent");
		j = v->left;
		ASSERTD(!j || 0 <= j && j < c && 1 >= ++lfv[j] &&     ord > (j + uv)->order , "u left"       );
		j = v->right;
		ASSERTD(!j || 0 <= j && j < c && 1 >= ++rtv[j] &&     ord < (j + uv)->order , "u right"      );
		j = v->down;
		ASSERTD(    0 <= j && j < c && 1 >= ++dnv[j] && (!j || ord > (j + uv)->order), "u predecessor");
		j = v->up;
		ASSERTD(    0 <= j && j < c && 1 >= ++upv[j] && (!j || ord < (j + uv)->order), "u successor"  );
	}
...
static F1(jtsbcheck)
{
	R sbcheck1(sc(jt->sbun), sc(jt->sbsn), jt->sbu, jt->sbs, jt->sbh, sc(ROOT), sc(FILLFACTOR), sc(GAP));
}

static F1(jtsbsetdata)
{
	A h, s, u, *wv, x;
	I wd;
	RZ(w);
