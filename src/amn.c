...
static I jtcsize(J jt, A z, A ind)
{
	B*b;
	I h, j, m, r, *s;
	P*zp;
	...
	m = 1;
	j = h = *(AR(ind) + AS(ind) - 1);
	DO(r - h, if(b[j])m *= s[j]; ++j;);
	R m;
}    /* data cell size in z for each element of index array ind */

static B jtiaddr(J jt, A z, A ind, A*i1, A*i2)
{
	A a, ai, as, ii, jj, q, t, x, y;
	I c, d, e, h, i, *iv, *jv, m, n, p, *qv, *s, *u, *v, *yv;
	P*zp;
	zp = PAV(z);
	a = SPA(zp, a);
	x = SPA(zp, x);
	...
	y = SPA(zp, i);
	if(!all1(q)) {
		RZ(y = repeatr(q, y));    /* indexed cols of index mat   */
	}
	m = *AS(y);
	yv = AV(y);
	RZ(ai = IX(h));
	RZ(as = less(IX(AR(z)), a));
	u = AV(as);
	n = AN(as);       /* dense axes                  */
	GA(t, INT, n, 1, 0);
	v = AV(t);                           /* shape of indexed dense axes */
	e = 0;
	d = 1;
	DO(n, if(h > u[i])v[e++] = s[i]; else {
		d *= s[i];
		});
...
static A jtzpadn(J jt, A z, A ind, B ip)
{
	A a, ai, i1, p, p1, q, t, x, x0, y, y0, y1;
	B*b;
	I c, d, h, m, n;
	P*zp;
	zp = PAV(z);
	...
	d = 0;
	DO(h, if(b[i])++d;);
	RZ(i1 = d < h ? repeatr(t, ind) : ind);
	if(2 != AR(ind)) {
		RZ(i1 = d ? reshape(v2(AN(i1) / d, d), i1) : mtm);
	}
	RZ(t = gt(sc(h), a));
	RZ(y1 = all1(t) ? y : repeatr(t, y));
	RZ(p = nub(less(i1, y1)));
	if(c = AN(a) - d) {
		RZ(t = from(less(a, ai), shape(z)));
		RZ(p1 = odom(2L, c, AV(t)));
		n = *AS(p1);
		if(m = *AS(p)) {
			RZ(p = stitch(repeat(sc(n), p), reshape(v2(n * m, c), p1)));
		}
	...
	if(m = *AS(p)) { /* new cells being added */
		...
		RZ(t = shape(x));
		*AV(t) = m;
		RZ(x = from(q, over(x, reshape(t, SPA(zp, e)))));
		if(ip) {
			ra(y);
			ra(x);
			fa(y0);
...
static A jtastdn(J jt, A a, A z, A ind)
{
	A a1, q, r, s;
	B*b;
	I ar, *as, *av, d, ir, n, n1, *v, zr, *zs;
	P*zp;
	ar = AR(a);
	as = AS(a);
	zr = AR(z);
	...
	memset(b, C0, zr);
	DO(AN(a1), b[av[i]] = 1;);
	memset(b, !memchr(b, C1, n) ? C0 : C1, n);
	GA(r, INT, zr - n1, 1, 0);
	v = AV(r);
	*v++ = ar - (zr - n);
	DO(zr - n, *v++ = 1;);
...
A jtamne(J jt, A a, A z, A ind, B ip)
{
	A i1, i2, x, y;
	C*u, *v;
	I*iv, *jv, k, m, n, vk, xk;
...
A jtamna(J jt, A a, A z, A ind, B ip)
{
	A i1, i2, x;
	C*u, *v;
	I*iv, *jv, k, n, vk, xk;
	...
	jv = AV(i2);
	if(AR(a)) {
		DO(n, mvc(vk, v + xk * iv[i] + k * jv[i], vk, u);
	}
	u += vk;
	  )
	else {
		DO(n, mvc(vk, v + xk * iv[i] + k * jv[i], k, u);
	});
	R z;
}    /* a (<"1 ind)}z, dense a,integer array ind, sparse z */

A jtamnsp(J jt, A a, A z, A ind, B ip)
{
	A i1, i2, t;
	C*ev, *u, *v, *vv;
	I c, *dv, i, *iv, j, *jv, k, m, n, p, q, r, *s, *yv, zk;
	P*ap, *zp;
	RZ(a && z && ind);
	r = AR(a);
	ap = PAV(a);
	...
	for(i = 0; i < n; ++i) {
		...
		for(j = 0; j < c; ++j) {
			q = r;
			DO(r, --q; ++dv[q]; if(dv[q] < s[q])break; dv[q] = 0;);
			q = 1;
			while(p < m) {
				DO(r, if(q = yv[i] - dv[i])break;);
				if(0 <= q) {
					break;
				} ++p;
				yv += r;
