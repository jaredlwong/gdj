...
static B consf(A w)
{
	...
	C c;
	c = ID(w);
	if(c == CFCONS || c == CQQ && (f = VAV(w)->f, NOUN & AT(f))) {
		R 1;
	}
	R 0;
...
static F1(jtinvfork)
{
	...
	if(CAMP == ID(gi)) {
		...
		if     (NOUN & AT(v->f))RZ(gi = folk(v->f,     v->g, ds(CRIGHT)))
				...
				RZ(gi = folk(v->g, swap(v->f), ds(CRIGHT)));
			}
	}
	R fong(fi, gi);
}

static DF1(jtexpandf)
{
	A f;
	RZ(w && self);
	f = VAV(self)->f;
...
{
	A*wv, x, z = w;
	I i, j, m, r, *s, t = 0, *u, **v, *y, wn, wr, *ws, wd;
	RZ(w);
	ASSERT(0, EVNONCE);
...
static F1(jtinvamp)
{
	...
	case CSTAR:
		R amp(recip(x), h);
	case CMINUS:
		R nf ? w : amp(x, ds(CPLUS));
	case CDIV:
		R nf ? w : amp(x, ds(CSTAR));
	case CROOT:
		R amp(ds(nf ? CEXP : CLOG), x);
	case CEXP:
		R ng && equ(x, num[2]) ? ds(CROOT) : amp(x, ds(nf ? CLOG : CROOT));
	case CLOG:
		R nf ? amp(x, ds(CEXP)) : amp(ds(CROOT), x);
	case CJDOT:
		R nf ? atop(inv(ds(CJDOT)), amp(ds(CMINUS), x)) : amp(ds(CMINUS), jdot1(x));
	case CRDOT:
		R nf ? atop(inv(ds(CRDOT)), amp(ds(CDIV  ), x)) : amp(ds(CDIV  ), rdot1(x));
	case CLBRACE:
		R nf ? amp(pinv(x), h) : amp(x, ds(CIOTA));
	case COBVERSE:
		ff = VAV(h)->g;
		R amp(nf ? x : ff, nf ? ff : x);
	case CPDERIV:
		if(!AR(h)) {
			R ds(CPDERIV);
		}
	...
	case CTHORN:
		...
		break;
	case CTILDE:
		if(ff = VAV(h)->f, VERB & AT(ff)) {
			R invamp(amp(nf ? ff : x, nf ? x : ff));
		} else {
			ff = unname(h);
			R invamp(amp(nf ? x : ff, nf ? ff : x));
		}
	case CSCO:
		ASSERT(nf, EVDOMAIN);
		RE(n = i0(x));
		ASSERT(n && -6 <= n && n <= 6, EVDOMAIN);
		R amp(sc(-n), h);
	case CUCO:
		ASSERT(nf, EVDOMAIN);
		RE(n = i0(x));
		ASSERT(1 <= n && n <= 4 || 7 <= n && n <= 8, EVDOMAIN);
	...
	case CBANG:
		...
		h = lrep(x);
		if(nf) {
			q[5] = over(over(h, cstr("&<@|@{:}")), over(h, cstr(",:")));
			q[6] = over(h, cstr("%:y*!"));
			q[7] = h;
		} else  {
			q[5] = cstr("1>.{.@/:\"1|y-/(i.!])");
	...
	case CATOMIC:
		if(ng) {
			ASSERT(equ(x, nub(x)), EVDOMAIN);
			R obverse(atop(f, amp(x, ds(CIOTA))), w);
		}
	case CCYCLE:
		if(nf && AR(x) <= (c == CCYCLE)) {
			R obverse(eva(w, "/:@x@(i.@#) { ]"), w);
		}
		break;
	case CDROP:
		if(!(nf && 1 >= AR(x))) {
	...
	case CDOT:
		...
		break;
	case CQQ:
		if(ng && equ(x, one) && equ(f, eval("i.\"1"))) {
			R hook(ds(CFROM), ds(CEQ));
		}
		break;
	case CBSLASH:
		if(nf && (n = i0(x), 0 > n) && (d = ID(u->f), d == CLEFT || d == CRIGHT)) {
			R slash(ds(CCOMMA));
		}
		break;
	case CIBEAM:
		...
		if(NOUN & AT(x) && equ(x, num[3]) && NOUN & AT(y)) {
			RE(n = i0(f));
			if(all1(eps(y, v2(4L, 5L)))) {
				ASSERT(n && -2 <= n && n <= 2, EVDOMAIN);
				R amp(sc(-n), g);
			}
			if(all1(eps(y, v2(1L, 3L)))) {
				ASSERT(0 == n || 1 == n || 10 == n || 11 == n, EVDOMAIN);
				R foreign(x, num[2]);
			}
		}
		break;
...
F1(jtinv)
{
	...
	id = ID(w);
	v = VAV(w);
	if(s = strchr(invf[0], id)) {
		R ds(invf[1][s - invf[0]]);
	}
	f = v->f;
	nf = f && AT(f)&NOUN + NAME;
	vf = f && !nf;
	...
	case CWORDS:
		R eval("}:@;@(,&' '&.>\"1) :. ;:");
	case CBANG:
		R eval("3 :'(-(!-y\"_)%1e_3&* !\"0 D:1 ])^:_ <.&170^:(-:+)^.y' :. !");
	case CXCO:
		R amp(num[-1], w);
	case CSPARSE:
		R fdef(CPOWOP, VERB, jtdenseit, 0L, w, num[-1], 0L, 0L, RMAX, RMAX, RMAX);
	case CPCO:
		R fdef(CPOWOP, VERB, jtplt,    0L, w, num[-1], 0L, 0L, 0L,  0L,  0L  );
	case CQCO:
		R eval("*/");
	case CUCO:
		R amp(num[3], w);
	...
	case CPOWOP:
		if(vf && ng) {
			RE(p = i0(g));
			R - 1 == p ? f : 1 == p ? inv(f) : powop(0 > p ? f : inv(f), sc(ABS(p)));
		}
		if(VGERL & v->flag) {
			R*(1 + AAV(v->h));
		}
	...
	case CBSDOT:
		if(CSLASH == ID(f) && (ff = VAV(f)->f, ff && VERB & AT(ff))) {
			...
			switch(ID(ff)) {
			case CPLUS:
				R obverse(eval(b ? "- 1&(|.!.0)" : " - |.!.0"), w);
			case CSTAR:
				R obverse(eval(b ? "% 1&(|.!.1)" : " % |.!.1"), w);
			case CEQ:
				R obverse(eval(b ? "= 1&(|.!.1)" : " = |.!.1"), w);
			case CNE:
				R obverse(eval(b ? "~:1&(|.!.0)" : " ~:|.!.0"), w);
			case CMINUS:
				R obverse(eval(b ? "+ 1&(|.!.0)" : "(- |.!.0) *\"_1 $&1 _1@#"), w);
			case CDIV:
				R obverse(eval(b ? "* 1&(|.!.1)" : "(% |.!.1) ^\"_1 $&1 _1@#"), w);
			}
		}
		break;
	case CCUT:
		if(CBOX == ID(f) && ng && (p = i0(g), 1 == p || 2 == p)) {
			R fdef(CPOWOP, VERB, jtbminv, 0L, w, num[-1], 0L, 0L, RMAX, RMAX, RMAX);
		}
		break;
	case CIBEAM:
		p = i0(f);
...
F1(jtiden)
{
	...
	case CCOMMA:
		R eval("i.@(0&,)@(2&}.)@$");
	case CDOT:
		if(!(ip(w, CPLUS, CSTAR) || ip(w, CPLUSDOT, CSTARDOT) || ip(w, CNE, CSTARDOT))) {
			break;
		}
	case CDOMINO:
		R atop(atop(ds(CEQ), ds(CGRADE)), ds(CHEAD));
	case CCYCLE:
	case CLBRACE:
		R atop(ds(CGRADE), ds(CHEAD));
	case CSLASH:
		if(VERB & AT(f)) {
			R atop(iden(f), ds(CPOUND));
		}
		break;
	case CPLUS:
	case CMINUS:
