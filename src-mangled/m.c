...
B jtmeminit(J jt)
{
	I k, m = MLEN;
	k = 1;
	DO(m, msize[i] = k; k += k;); /* OK to do this line in each thread */
	jt->tbase = -NTSTACK;
	jt->ttop = NTSTACK;
	jt->mmax = msize[m - 1];
...
F1(jtspcount)
{
	...
	GA(z, INT, 2 * m, 2, 0);
	v = AV(z);
	DO(m, j = 0; x = (MS*)(jt->mfree[i]); while(x) {
	x = (MS*)(x->a);
		++j;
	}
	if(j) {
...
static int __cdecl spfreecomp(const void *x, const void *y)
{
	R *(I*)x < *(I*)y ? -1 : 1;
}
...
B jtspfree(J jt)
{
	...
	for(i = 6; i <= PLIML; ++i) {
		...
		jt->mfree[i] = u;
		jt->mfreet[i] = 1048576 + jt->mfreeb[i];
	}
	R 1;
}    /* free unused blocks */
...
static A jtma(J jt, I m)
{
	...
	if(jt->mfree[j]) {        /* allocate from free list         */
		...
		ASSERT(v, EVWSFULL);
		u = (C*)v;
		DO(PSIZE / n, x = (MS*)u; u += n; x->a = (I*)u; x->j = (C)j; x->mflag = 0;);
		x->a = 0;
		((MS*)v)->mflag = MFHEAD;
		z = (A)(mhw + v);
		jt->mfree[j] = ((MS*)v)->a;
	...
	JBREAK0;
	jt->bytes += n;
	jt->bytesmax = MAX(jt->bytes, jt->bytesmax);
	x = (MS*)z - 1;
	x->a = 0;
	x->j = (C)j;
	R z;
...
static A jttraverse(J jt, A w, AF f)
{
	...
	case XD: {
		DX*v = (DX*)AV(w);
		DO(AN(w), CALL1(f, v->x, 0L); ++v;);
	}
	break;
	case RAT: {
		A*v = AAV(w);
		DO(2 * AN(w), CALL1(f, *v++, 0L););
	}
	break;
	case XNUM:
	case BOX:
		if(!(AFLAG(w)&AFNJA + AFSMM)) {
			A*wv = AAV(w);
			I wd = (I)w * ARELATIVE(w);
			DO(AN(w), CALL1(f, WVR(i), 0L););
		}
		break;
	case VERB:
	case ADV:
...
static void jttf(J jt)
{
	A t = jt->tstacka;
	jt->tstacka = *jt->tstack;
	jt->tstack = AAV(jt->tstacka);
	jt->tbase -= NTSTACK;
	jt->ttop = NTSTACK;
	fr(t);
...
I jttpop(J jt, I old)
{
	while(old < jt->tbase + jt->ttop)if(1 < jt->ttop) {
			fr(jt->tstack[--jt->ttop]);
		} else {
			tf();
		}
	R old;
...
A jtga(J jt, I t, I n, I r, I*s)
{
	A z;
	I m, w;
	if(t & BIT) {
		const I c = 8 * SZI;          /* bit type: pad last axis to fullword */
		ASSERTSYS(1 >= r || s, "ga bit array shape");
		if(1 >= r) {
			w = (n + c - 1) / c;
		} else {
			RE(w = mult(prod(r - 1, s), (s[r - 1] + c - 1) / c));
		}
		w += WP(INT, 0L, r);
		m = SZI * w;
	...
	AFLAG(z) = 0;
	AK(z) = AKX(z);
	AM(z) = msize[((MS*)z - 1)->j] - (AK(z) + sizeof(MS));
	AT(z) = 0;
	tpush(z);
	AT(z) = t;
	if(1 == r && !(t & SPARSE)) {
		*AS(z) = n;
		...
		ICPY(AS(z), s, r);    /* 1==n always if t&SPARSE */
	}
	R z;
}

A jtgah(J jt, I r, A w)
{
	A z;
...
A jtext(J jt, B b, A w)
{
	...
	if(b) {
		ra(z);    /* 1=b iff w is permanent */
		fa(w);
	}
	*AS(z) = m1 = AM(z) / k;
	AN(z) = m1 * c;   /* "optimal" use of space */
	if(!(t & DIRECT)) {
		memset(CAV(z) + m * k, C0, k * (m1 - m));
	}
	R z;
}

A jtexta(J jt, I t, I r, I c, I m)
{
	A z;
	I k, m1;
	GA(z, t, m * c, r, 0);
...
B jtcheckmf(J jt)
{
	...
	for(j = 0; j <= PLIML; ++j) {
		i = 0;
		y = 0;
		x = (MS*)(jt->mfree[j]); /* head ptr for j-th pool */
		while(x) {
			++i;
			c = x->mflag;
			if(!(j == x->j)) {
