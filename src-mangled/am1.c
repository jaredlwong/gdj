...
static A jtistd1(J jt, A z, A ind)
{
	A*iv, j, *jv, x;
	I d, i, id, n, r, *s;
	RZ(z && ind);
	ASSERT(1 >= AR(ind), EVRANK);
	if(AN(ind) && !(BOX & AT(ind))) {
		ASSERT(NUMERIC & AT(ind), EVINDEX);
	...
	d = n;
	DO(n, --d; x = IVR(d); if(!equ(x, ace))break;);
	n = n ? 1 + d : d;
	GA(j, BOX, n, 1, 0);
	jv = AAV(j);
	for(i = 0; i < n; ++i) {
...
static A jtastd1(J jt, A a, A z, A ind)
{
	A*iv, q, r, s, s1, *sv, x;
	B b;
	I ar, *as, d, id, j, m, n, *rv, zr, *zs;
	P*zp;
	I*s1v;
	ar = AR(a);
	as = AS(a);
...
static A jtssel(J jt, A z, A ind)
{
	A a, *iv, p, q, x, y;
	B*b;
	I*av, c, i, id, j, m, n, *u, *v, *yv;
	P*zp;
	zp = PAV(z);
	y = SPA(zp, i);
	v = AS(y);
	...
	for(i = 0; i < n; ++i) {
		...
		if(x != ace) {
			u = yv + i;
			DO(m, v[i] = b[i] ? *u : -1; u += c;);
			RZ(p = eps(q, 1 < AR(x) ? ravel(x) : x));
			b = BAV(p);
		}
	}
	R p;
...
static B jtipart(J jt, A z, A ind, A*i1, A*i2)
{
	A*iv, p, *pv, q, *qv, x;
	B*b;
	I c, d, id, n;
	...
	*i1 = p;
	...
	qv = AAV(q);
	*i2 = q;
	DO(n, x = IVR(i); if(b[i])*pv++ = x; else {
		*qv++ = x;
	});
	R 1;
}    /* partition index into sparse and dense parts */

static A jtdcube(J jt, A z, A i2)
{
	A*iv, x, y;
	I i, id, m, n, *s;
	P*zp;
...
static A jtscuba(J jt, A z, A i1, B u)
{
	A*iv, q = 0, x;
	I c, d, id, j, n, *s, *v;
	P*zp;
	...
	for(j = n - 1; 0 <= j; --j) {
		...
		if(q) {
			d = *AS(q);
			RZ(q = stitch(repeat(sc(d), x), reitem(sc(c * d), q)));
		} else {
			RZ(q = reshape(v2(c, 1L), x));
		}
	}
	R q;
}    /* index cube relative to sparse axes; 1=u iff unique (remove duplicates) */

static A jtscubb(J jt, A z, A i1)
{
	A a, q, x, y;
	I c, d, h, j, *s, *v, *xv;
	P*zp;
...
static A jtscubc(J jt, A z, A i1, A p)
{
	A a, q, s, y, y1;
	B*qv;
	I c, d, h, j = -1, m, n, *sv, *u, *v;
	P*zp;
	zp = PAV(z);
	a = SPA(zp, a);
	n = AN(i1);
...
static A jtscube(J jt, A z, A i1, A p)
{
	A a, y;
	P*zp;
	zp = PAV(z);
...
static A jtiindx(J jt, A z, A i1)
{
	A q, j, j1, y;
	I c, d, e, h, i, *jv, m, n, *qv, *v, *yv;
	P*zp;
	c = AN(i1);
	zp = PAV(z);
	y = SPA(zp, i);
	if(c == *(1 + AS(y))) {
		R indexof(y, scuba(z, i1, 0));
	}
	...
	yv = AV(y);
	GA(q, INT, n, 1, 0);
	qv = AV(q);        /* q: # members in each group */
	for(i = h = 0; i < n; ++i) {
		e = 1;
		d = jv[i];
		v = yv + c * d;
...
static A jtzpad1(J jt, A z, A t, B ip)
{
	A q, s, x, x0, y, y0;
	I m;
	P*zp;
	...
	if(m = *AS(t)) { /* new cells being added */
		...
		RZ(s = shape(x0));
		*AV(s) = m;
		RZ(x = from(q, over(x0, reshape(s, SPA(zp, e)))));
		if(ip) {
			ra(y);
			ra(x);
			fa(y0);
...
A jtam1e(J jt, A a, A z, A ind, B ip)
{
	A e, i1, i2, p, x, y;
	B*pv;
	C*u, *v;
	...
	s = AS(x);
	k = bp(AT(x));
	xk = k * prod(r - (n + 1), s + (n + 1));
	vk = k * prod(r - 1, s + 1);
	if(!n) {
		DO(m, if(pv[i])mvc(xk, v, k, u);
	}
v += vk;
  )
else {
...
A jtam1a(J jt, A a, A z, A ind, B ip)
{
	...
	if(!n && !m) {
		a1 = SPA(zp, a);
		R ar ? sparseit(a0, a1, e) : sparseit(reshape(shape(z), a), a1, a);
	}
	if(n) {
		RZ(t = dcube(z, i2));
		jv = AV(t);
...
DO(c, mvc(xk, v + xk * jv[i], uk, u); if(ar)u += uk;);
	  )
	else {
		DO(*AS(x), v += vk;
	}
...
A jtam1sp(J jt, A a, A z, A ind, B ip)
{
	R amnsp(a, z, ope(catalog(istd1(z, ind))), ip);
}
