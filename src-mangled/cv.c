...
static F2(jtfitct)
{
	...
	RZ(w = cvt(FL, w));
	d = *DAV(w);
	ASSERT(0 <= d && d < 5.82076609134675e-11, EVDOMAIN);
	R CDERIV(CFIT, jtfitct1, jtfitct2, sv->mr, sv->lr, sv->rr);
}

static DF1(jtfitfill1)
{
	DECLFG;
	A z;
	jt->fill = gs;
...
{
	V*sv = VAV(self);
	R df1(  w, fit(fix(sv->f), sv->g));
}
static DF2(jtfitf2)
{
	V*sv = VAV(self);
	R df2(a, w, fit(fix(sv->f), sv->g));
}
...
F2(jtfit)
{
	...
	case CPOWOP:
		if(VERB & AT(sv->g) || !equ(num[-1], sv->g)) {
			R fitct(a, w);
		}
		f = sv->f;
		c = ID(f);
		if(c == CPOUND) {
			ASSERT(!AR(w), EVRANK);
			R CDERIV(CFIT, 0, jtfitfill2, m, l, r);
		}
		ASSERT(c == CAMP, EVDOMAIN);
		f = VAV(f)->g;
		ASSERT(CPOUND == ID(f), EVDOMAIN);
	case CPOUND:
	case CTAKE:
	case CTAIL:
	case CCOMMA:
