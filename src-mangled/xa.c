...
F1(jtdispq)
{
	A z;
	ASSERTMTV(w);
	GA(z, INT, *jt->disp, 1, 0);
	ICPY(AV(z), 1 + jt->disp, *jt->disp);
	R z;
}
...
F1(jtdotnamess)
{
	...
	jt->dotnames = b = *BAV(w);
	if(c && !b) {
		ds(CMDOT) = ds(CNDOT) = ds(CUDOT) = ds(CVDOT) = ds(CXDOT) = ds(CYDOT) = 0;
	} else if(!c && b) {
		ds(CMDOT) = mdot;
		ds(CNDOT) = ndot;
		ds(CUDOT) = udot;
...
F1(jtsysparms)
{
	...
	case 0:
		...
		break;
	case 1:
		ASSERT(0, EVDOMAIN); /* jt->fdepi  can not be set */
	case 2:
		RE(m = i0(WVR(1)));
		jt->fcalln = m;
		break;
	case 3:
		ASSERT(0, EVDOMAIN); /* jt->fcalli can not be set */
	}
	R mtm;
}
