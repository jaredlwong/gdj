...
C spellin(I n, C*s)
{
	...
	case 2:
		c = s[1];
		j = c == CESC1 ? 1 : c == CESC2 ? 2 : 0;
		R j&&(t = (C*)strchr(spell[0], p)) ? spell[j][t - spell[0]] : 0;
	case 3:
		c = s[1];
		d = s[2];
		if(p == CSIGN && d == CESC2 && '1' <= c && c <= '9') {
			R CFCONS;
		}
		if(t = (C*)strchr(sp3[1], p)) {
			j = t - sp3[1];
			R c == sp3[2][j]&&d == sp3[3][j] ? sp3[0][j] : 0;
		}
...
void spellit(C c, C*s)
{
	...
	if(0 <= c && (UC)c <= 127) {
		s[0] = c;
	} else if(q = (C*)strchr(spell[1], c)) {
		k = q - spell[1];
		s[0] = spell[0][k];
		s[1] = CESC1;
	} else if(q = (C*)strchr(spell[2], c)) {
		k = q - spell[2];
		s[0] = spell[0][k];
		s[1] = CESC2;
	} else if(q = (C*)strchr(sp3[0],  c)) {
		k = q - sp3[0];
		s[0] = sp3[1][k];
		s[1] = sp3[2][k];
		s[2] = sp3[3][k];
...
A jtspella(J jt, A w)
{
	...
	c = v->id;
	if(c == CFCONS) {
		R over(thorn1(v->h), chr[':']);
	}
	spellit(c, s);
	R str(s[2] ? 3L : s[1] ? 2L : 1L, s);
}

A jtspellout(J jt, C c)
{
	C s[3];
	spellit(c, s);
	R str(s[2] ? 3L : s[1] ? 2L : 1L, s);
