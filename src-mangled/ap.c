...
static B jtpscanlt(J jt, I m, I c, I n, B*z, B*x, B p)
{
	A t;
	B*v;
	I d, i;
	...
	memset(z, !p, m * c);
	if(1 == d) {
		DO(m, if(v = memchr(x, p, n)) * (z + (v - x)) = p;
	}
z += c;
x += c;
  )
...
AHDRP(ltpfxB, B, B)
{
	pscanlt(m, c, n, z, x, C1);
}
AHDRP(lepfxB, B, B)
{
	pscanlt(m, c, n, z, x, C0);
}
...
{
	A t;
	B b, *cc = "\000\001\000", e, *p = cc + pp, *v;
	C*u;
	I d, i, j;
	if(c == n)for(i = 0; i < m; ++i) {
		...
		for(i = 0; i < m; ++i) {
			e = pp;
			memset(u, C0, d);
			DO(n, j = i; DO(d, if(u[i])z[i] = '1' == u[i]; else if(a == x[i]) {
				b = 1 && j % 2;
				z[i] = b != pa;
					u[i] = b != ps ? '1' : '0';
				} else {
...
AHDRP(  gtpfxB, B, B)
{
	pscangt(m, c, n, z, x, C0, C1, C0, C0);
}
AHDRP(  gepfxB, B, B)
{
	pscangt(m, c, n, z, x, C1, C0, C1, C1);
}
AHDRP( norpfxB, B, B)
{
	pscangt(m, c, n, z, x, C1, C0, C1, C0);
}
AHDRP(nandpfxB, B, B)
{
	pscangt(m, c, n, z, x, C0, C1, C0, C1);
}
...
{
	C er = 0;
	I d = c / n, i, j, n1 = n - 1, *xx = x, *y, *zz = z;
	if(1 == d) {
		if(1 == n) {
			DO(m, *z++ = *x++;
		})
...
static DF1(jtprefix)
{
	...
	I r;
	RZ(w);
	if(jt->rank && jt->rank[1] < AR(w)) {
		r = jt->rank[1];
		jt->rank = 0;
		R rank1ex(w, self, r, jtprefix);
	}
	jt->rank = 0;
	R eachl(apv(IC(w), 1L, 1L), w, atop(fs, ds(CTAKE)));
}    /* f\"r w for general f */

static DF1(jtgprefix)
{
	A h, *hv, z, *zv;
	I m, n, r;
	RZ(w);
...
static DF1(jtpscan)
{
	...
	if(2 > n || !wn) {
		if(id) {
			jt->rank = 0;
			R r ? ca(w) : reshape(over(shape(w), one), w);
		} else {
			R prefix(w, self);
		}
	}
	...
	if(jt->jerr) {
		R jt->jerr == EWOV ? (rr[1] = r, jt->rank = rr, pscan(w, self)) : 0;
	} else {
...
static DF2(jtinfixd)
{
	A fs, z;
	C*x, *y;
	I c = 0, d, k, m, n, p, q, r, *s, wd, wr, *ws, wt, zc;
	...
	if(0 > m) {
		...
		ASSERT(IMAX - 1 > n - m, EVDOMAIN);
		d = MAX(0, 1 + n - m);
	}
	if(fs = VAV(self)->f, CCOMMA == ID(fs)) {
		RE(c = aii(w));
		RE(zc = mult(p, c));
		r = 2;
	} else {
	...
	*s++ = zc;
	...
	if(AN(z))switch((0 > m ? 2 : 0) + (wd ? 1 : 0)) {
		case 0:
			q = p * k;
			DO(d, MC(x, y, q);    x += q; y += k;);
			break;
		case 1:
			q = p * k;
			DO(d, MCREL(x, y, p); x += q; y += k;);
			break;
		case 2:
			MC(x, y, n * k);
			if(q = d * p - n) {
...
static A jtmovsumavg1(J jt, I m, A w, A fs, B avg)
{
	A y, z;
	D d = (D)m;
	I c, p, s, t, wt;
	...
	case 0:
		MOVSUMAVG(B, I, INT, I, INT, x,  SETZ );
		break;
	case 1:
		MOVSUMAVG(B, I, INT, D, FL, x / d, SETZD);
		break;
	case 2:
		irange(AN(w), AV(w), &s, &t);
		t = 0 < t && IMAX >= d * ((D)s + (D)t);
		if(t)        MOVSUMAVG(I, I, INT, I, INT, x,  SETZ )
			else {
				MOVSUMAVG(I, D, FL, D, FL, x,  SETZ );
			}
		break;
	case 3:
		MOVSUMAVG(I, D, FL, D, FL, x / d, SETZD);
		break;
	case 4:
		NAN0;
		MOVSUMAVG(D, D, FL, D, FL, x,  SETZ );
		NAN1;
		break;
	case 5:
		NAN0;
		MOVSUMAVG(D, D, FL, D, FL, x / d, SETZD);
		NAN1;
		break;
	}
	R z;
...
static A jtmovsumavg(J jt, I m, A w, A fs, B avg)
{
	A z;
...
static DF2(jtmovavg)
{
	...
	PREF2(jtmovavg);
	RE(m = i0(vib(a)));
	if(0 < m && m <= IC(w) && AT(w)&B01 + FL + INT) {
		R movsumavg(m, w, self, 1);
	}
	R infix(a, w, self);
}    /* a (+/ % #)\w */
...
static A jtmovminmax(J jt, I m, A w, A fs, B max)
{
	A y, z;
	I c, i, j, p, wt;
	p = IC(w) - m;
	wt = AT(w);
	c = aii(w);
	GA(z, AT(w), c * (1 + p), AR(w), AS(w));
	*AS(z) = 1 + p;
	switch(max + (wt & INT ? 0 : 2)) {
	case 0:
		MOVMINMAX(I, INT, IMAX, <= );
		break;
	case 1:
		MOVMINMAX(I, INT, IMIN, >= );
		break;
	case 2:
		MOVMINMAX(D, FL, inf , <= );
		break;
...
static A jtmovandor(J jt, I m, A w, A fs, B or )
{
	A y, z;
	B b0, b1, d, e, *s, *t, *u, *v, x, *yv, *zv;
	I c, i, j, p;
	p = IC(w) - m;
	c = aii(w);
	x = b0 = ! or;
	b1 = or;
	GA(z, B01, c * (1 + p), AR(w), AS(w));
	*AS(z) = 1 + p;
	zv = BAV(z);
	u = v = BAV(w);
	if(1 == c) {
...
static A jtmovbwandor(J jt, I m, A w, A fs, B or )
{
	A z;
	I c, p, *s, *t, *u, x, *zv;
	p = IC(w) - m;
	c = aii(w);
	GA(z, INT, c * (1 + p), AR(w), AS(w));
	*AS(z) = 1 + p;
	zv = AV(z);
	u = AV(w);
	if(c)switch( or + (1 == c ? 0 : 2)) {
		case 0:
			DO(1 + p, x = *u++; t = u; DO(m - 1, x &= *t++;); *zv++ = x;);
			break;
		case 1:
			DO(1 + p, x = *u++; t = u; DO(m - 1, x |= *t++;); *zv++ = x;);
			break;
		case 2:
			DO(1 + p, ICPY(zv, u, c); t = u += c; DO(m - 1, s = zv; DO(c, *s++ &= *t++;);); zv += c;);
			break;
		case 3:
			DO(1 + p, ICPY(zv, u, c); t = u += c; DO(m - 1, s = zv; DO(c, *s++ |= *t++;);); zv += c;);
			break;
...
static A jtmovneeq(J jt, I m, A w, A fs, B eq)
{
	A y, z;
	B*s, *u, *v, x, *yv, *zv;
	I c, p;
	...
	*AS(z) = 1 + p;
	...
	case 2:
		DO(m, s = yv; DO(c,       *s++ ^=   *v++;););
		SETZ;
		DO(p, s = yv; DO(c,       *zv++ = *s++ ^=   *u++^ *v++;););
		break;
	case 3:
		DO(m, s = yv; DO(c, x = *s; *s++ = x == *v++;););
		SETZ;
		DO(p, s = yv; DO(c, x = *s; *zv++ = *s++ = x == *u++ == *v++;););
	}
	R z;
}    /* m ~:/\w (0=eq) or m =/\ (1=eq); boolean w; 0<m */

static A jtmovbwneeq(J jt, I m, A w, A fs, B eq)
{
	A y, z;
	I c, p, *s, *u, *v, x, *yv, *zv;
	p = IC(w) - m;
...
static DF2(jtmovfslash)
{
	A x, z;
	B b;
	C id, *wv, *zv;
	I c, cm, cv, d, m, m0, p, t, wk, wt, zk, zt;
	VF ado;
	PREF2(jtmovfslash);
	p = IC(w);
	wt = AT(w);
	RE(m0 = i0(vib(a)));
	m = 0 <= m0 ? m0 : m0 == IMIN ? p : MIN(p, -m0);
	if(2 == m0) {
		R infix2(a, w, self);
	}
	x = VAV(self)->f;
	...
	id = ID(x);
	if(wt & B01) {
		id = id == CMIN ? CSTARDOT : id == CMAX ? CPLUSDOT : id;
	}
	if(id == CBDOT && (x = VAV(x)->f, INT&AT(x) && !AR(x))) {
		id = (C) * AV(x);
	}
	switch(0 < m0 && m0 <= IC(w) ? id : 0) {
	case CPLUS:
		if(wt & B01 + INT + FL) {
			R movsumavg(m, w, self, 0);
		}
		break;
	case CMIN:
		if(wt &    INT + FL) {
			R movminmax(m, w, self, 0);
		}
		break;
	case CMAX:
		if(wt &    INT + FL) {
			R movminmax(m, w, self, 1);
		}
		break;
	case CSTARDOT:
		if(wt & B01       ) {
			R  movandor(m, w, self, 0);
		}
		break;
	case CPLUSDOT:
		if(wt & B01       ) {
			R  movandor(m, w, self, 1);
		}
		break;
	case CNE:
		if(wt & B01       ) {
			R   movneeq(m, w, self, 0);
		}
		break;
	case CEQ:
		if(wt & B01       ) {
			R   movneeq(m, w, self, 1);
		}
		break;
	case CBW1001:
		if(wt &    INT   ) {
			R movbwneeq(m, w, self, 1);
		}
		break;
	case CBW0110:
		if(wt &    INT   ) {
			R movbwneeq(m, w, self, 0);
		}
	}
	vains(id, wt, &ado, &cv);
	if(!ado || !m || m > p) {
	...
	d = 0 <= m0 ? 1 + p - m : (p + m - 1) / m;
	c = aii(w);
	cm = c * m;
	b = 0 > m0 && 0 < p % m;
	zt = rtype(cv);
	jt->rank = 0;
	GA(z, zt, c * d, AR(w), AS(w));
	*AS(z) = d;
	if((t = atype(cv)) && t != wt) {
		RZ(w = cvt(t, w));
		wt = AT(w);
	...
	if(b) {
		ado(jt, 1L, c * (p % m), p % m, zv, wv);
	}
	if(jt->jerr == EWOV) {
		RESETERR;
		R movfslash(a, cvt(FL, w), self);
	} else {
		R z;
	}
}    /* a f/\w */
...
{
	A f;
	AF f1 = jtprefix, f2 = jtinfix;
	V*v;
	RZ(w);
	if(NOUN & AT(w)) {
		R fdef(CBSLASH, VERB, jtgprefix, jtginfix, w, 0L, fxeachv(1L, w), VGERL, RMAX, 0L, RMAX);
	}
	v = VAV(w);
	f = VAV(w)->f;
	switch(v->id) {
...
A jtascan(J jt, C c, A w)
{
	RZ(w);
	R df1(w, bslash(slash(ds(c))));
}
