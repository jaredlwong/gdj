...
static F1(jtdrr)
{
	...
	if(fs) {
		RZ(df = fl & VGERL ? every(fxeach(fs), 0L, jtdrr) : drr(fs));
	}
	if(gs) {
		RZ(dg = fl & VGERR ? every(fxeach(gs), 0L, jtdrr) : drr(gs));
	}
	if(ex) {
		RZ(dg = unparsem(zero, w));
	}
	m += !b && !xop || hs && xop;
	GA(z, BOX, m, 1, 0);
	x = AAV(z);
...
F1(jtaro)
{
	...
	if(1 < m) {
		RZ(u[1] = aro(ex ? unparsem(zero, w) : xop ? hs : gs));
	}
	if(2 < m) {
		RZ(u[2] = aro(hs));
	}
...
static F1(jtfxchar)
{
	...
	if(id != CFCONS) {
		...
		m = s[n - 2] - '0';
		y = FCONS(CSIGN != *s ? sc(m) : 2 == n ? ainf : sc(-m));
	}
...
static A jtunparse1(J jt, CW*c, A x, I j, A y)
{
	A q, z;
	C*s;
	I t;
	...
	case CGOTO:
		...
		break;
	case CFOR:
		RZ(z = c->n ? *AAV(x) : spellcon(t));
		break;
	default:
		RZ(z = spellcon(t));
	}
	if(j == c->source) {
		GA(q, LIT, 1 + AN(y) + AN(z), 1, 0);
		s = CAV(q);
		MC(s, CAV(y), AN(y));
		s += AN(y);
		*s++ = ' ';
...
static A*jtunparse1a(J jt, I m, A*hv, A*zv)
{
	A*v, x, y;
	CW*u;
	I i, j, k;
...
F2(jtunparsem)
{
	...
	if(equ(mtv, hv[2]) && equ(mtv, hv[2 + HN])) {
		if(m) {
			mn = 1 + ((CW*)AV(mc) + m - 1)->source;
		}
		if(n) {
			dn = 1 + ((CW*)AV(dc) + n - 1)->source;
		}
		GA(z, BOX, p + mn + dn, 1, 0);
		zu = zv = AAV(z);
...
{
	F1RANK(0,  jtarx, 0);
	R arep(  symbrdlock(nfb(w)));
}
F1(jtdrx)
{
	F1RANK(0,  jtdrx, 0);
	R drep(  symbrdlock(nfb(w)));
}
F1(jttrx)
{
	F1RANK(0,  jttrx, 0);
	R trep(  symbrdlock(nfb(w)));
}
F1(jtlrx)
{
	F1RANK(0,  jtlrx, 0);
	R lrep(  symbrdlock(nfb(w)));
}
F1(jtprx)
{
	F1RANK(0,  jtprx, 0);
	R prep(  symbrdlock(nfb(w)));
}

F2(jtxrx)
{
	F2RANK(0, 0, jtxrx, 0);
	R xrep(a, symbrdlock(nfb(w)));
}
