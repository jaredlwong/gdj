...
static AMON(sgnZ,   Z, Z, if(jt->ct > zmag(*x))*z = zeroZ; else
{
	*z = ztrend(*x);
	})
...
static AMON(absI,   I, I, if(0 <= *x)*z = *x; else
{
	ASSERTW(IMIN < *x, EWOV);
		*z = -*x;
	})
...
static A jtva1s(J jt, A w, C id, I cv, VF ado)
{
	A e, x, z, ze, zx;
	B c;
	C ee;
...
static A jtva1(J jt, A w, C id)
{
	...
	if(jt->jerr) {
		...
		default:
			R 0;
		case VA1CASE(EWOV,  CFLOOR):
			cv = VD;
			ado = floorD;
			break;
		case VA1CASE(EWOV,  CCEIL ):
			cv = VD;
			ado = ceilD;
			break;
		case VA1CASE(EWOV,  CSTILE):
			cv = VD + VDD;
			ado = absD;
			break;
		case VA1CASE(EWIRR, CSQRT ):
			cv = VD + VDD;
			ado = sqrtD;
			break;
		case VA1CASE(EWIRR, CEXP  ):
			cv = VD + VDD;
			ado = expD;
			break;
		case VA1CASE(EWIRR, CBANG ):
			cv = VD + VDD;
			ado = factD;
			break;
		case VA1CASE(EWIRR, CLOG  ):
			cv = VD + VDD * m;
			ado = m ? (VF)logD : (VF)logXD;
			break;
		case VA1CASE(EWIMAG, CSQRT ):
			cv = VZ + VZZ;
			ado = sqrtZ;
			break;
		case VA1CASE(EWIMAG, CLOG  ):
			cv = VZ + VZZ * m;
			ado = m ? (VF)logZ : wt & XNUM ? (VF)logXZ : (VF)logQZ;
		}
		RESETERR;
	} else {
...
{
	RZ(w);
	R XNUM&AT(w)&&(jt->xmode == XMFLR || jt->xmode == XMCEIL) ? va1(w, CCIRCLE) : tymes(pie, w);
}
