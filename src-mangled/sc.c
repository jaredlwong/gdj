...
static DF2(jtunquote)
{
	A aa, fs, g, ll, oldn, oln, z;
	B lk;
	I d, i;
	L*e;
	V*v;
	...
	jt->curname = aa;
	oln = jt->curlocn;
	jt->curlocn = ll = g ? LOCNAME(g) : 0;
	ASSERT(fs, EVVALUE);
	ASSERT(AT(self) == AT(fs), EVDOMAIN);
	d = fdep(fs);
	FDEPINC(d);
	ASSERT(jt->fcalln >= jt->fcalli, EVSTACK);
	if(0 < jt->pmctr) {
		pmrecord(aa, ll, -1L, a ? VAL2 : VAL1);
	}
	lk = jt->glock || VLOCK & VAV(fs)->flag;
	i = ++jt->fcalli;
	jt->fcallg[i].sw0 = jt->stswitched;
	jt->fcallg[i].og = jt->global;
	jt->fcallg[i].flag = 0;
	jt->stswitched = 0;
	jt->fcallg[i].g = jt->global = g;
	if(jt->db && !lk) {
		z = dbunquote(a, w, fs);
	} else {
		ra(fs);    /* beware redefs down the stack */
		z = a ? dfs2(a, w, fs) : dfs1(w, fs);
		fa(fs);
	}
	if(!jt->stswitched) {
		jt->global = jt->fcallg[i].og;
	}
	jt->stswitched = jt->fcallg[i].sw0;
	if(jt->fcallg[i].flag) {
		locdestroy(i);
	}
	jt->fcallg[i].g = jt->fcallg[i].og = 0;
	jt->stswitched = 0;
	FDEPDEC(d);
	--jt->fcalli;
	if(0 < jt->pmctr) {
		pmrecord(aa, ll, -2L, a ? VAL2 : VAL1);
	}
	jt->curlocn = oln;
	jt->curname = oldn;
	spfree();
