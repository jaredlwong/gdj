...
static A jtmerge1(J jt, A w, A ind)
{
	A z;
	B*b;
	C*wc, *zc;
	...
	wd = (D*)wc;
	switch(MCASE(it, k)) {
	case MCASE(B01, sizeof(C)):
		DO(c,         *zc++ = wc[*b++ ? i + c : i];);
		break;
	case MCASE(B01, sizeof(I)):
		DO(c,         *zi++ = wi[*b++ ? i + c : i];);
		break;
//#if !SY_64
//	case MCASE(B01, sizeof(D)):
//		DO(c,         *zd++ = wd[*b++ ? i + c : i];);
//		break;
//#endif
	case MCASE(INT, sizeof(C)):
		DO(c, MINDEX; *zc++ = wc[i + c * j];);
		break;
	case MCASE(INT, sizeof(I)):
		DO(c, MINDEX; *zi++ = wi[i + c * j];);
		break;
//#if !SY_64
//	case MCASE(INT, sizeof(D)):
//		DO(c, MINDEX; *zd++ = wd[i + c * j];);
//		break;
//#endif
	default:
		if(it & B01) {
			DO(c,         MC(zc, wc + k * (*b++ ? i + c : i), k); zc += k;)
		} else {
			DO(c, MINDEX; MC(zc, wc + k * (i + c * j     ), k); zc += k;)
		}
		break;
	}
	R RELOCATE(w, z);
...
u[i] = x; p = p && NOUN & AT(x););
	if(p) {
		...
		if(p) {
			DO(m, y = u[i]; if(!(t == AT(y) && r == AR(y) && !ICMP(s, AS(y), r))) { p = 0; break; });
		}
	}
	if(!p) {
		R parse(v[m + 2]);
...
A jtmerge2(J jt, A a, A w, A ind, B ip)
{
	A z;
	I an, ar, *as, at, in, ir, *iv, k, t, wn, wt;
	RZ(a && w && ind);
	an = AN(a);
	at = AT(a);
	ar = AR(a);
	...
	if(ip && t == wt && (!(t & BOX) || AFNJA & AFLAG(w))) {
		ASSERT(!(AFRO & AFLAG(w)), EVRO);
		z = w;
	} else {
		RZ(z = cvt(t, w));
		if(ARELATIVE(w)) {
			RZ(z = relocate((I)w - (I)z, z));
		}
	}
	if(ip && t & BOX && AFNJA & AFLAG(w)) {
		A*av, t, x, y;
		A1*zv;
		I ad, *tv;
		ad = (I)a * ARELATIVE(a);
		...
		if(!y) {
			DO(in, if(!tv[i])break; smmfrr((A)tv[i]););
			R 0;
		}
		...
		if(ARELATIVE(z)) {
			A*av = AAV(a), *zv = AAV(z);
			DO(in, zv[iv[i]] = (A)AREL(av[i % an], z););
		} else            {
			C*av = CAV(a), *zv = CAV(z);
			k = bp(t);
			DO(in, MC(zv + k * iv[i], av + k * (i % an), k); );
		}
	}
	R z;
}

A jtjstd(J jt, A w, A ind)
{
	A j = 0, k, *v, x;
	B b;
	I d, i, id, n, r, *s, *u, wr, *ws;
	wr = AR(w);
	ws = AS(w);
	b = AN(ind) && BOX & AT(ind);
	if(!wr) {
	...
	if(b && AR(ind)) {
		...
		if(!j) {
			RZ(x = from(ind, increm(iota(shape(w)))));
			u = AV(x);
	...
	if(!b) {
		...
		ASSERT(n <= wr, EVINDEX);
		d = n;
		DO(n, --d; if(!equ(ace, AADR(id, v[d])))break;);
		if(n) {
			++d;
		}
		n = d;
		...
		for(i = 0; i < n; ++i) {
			...
			if(AN(x) && BOX & AT(x)) {
				...
				k = IX(d);
				if(AN(x)) {
					k = less(k, pind(d, 1 < AR(x) ? ravel(x) : x));
				}
			} else {
				k = pind(d, x);
			}
...
static A jtamendn2(J jt, A a, A w, A ind, B ip)
{
	PROLOG;
	A e, z;
	B b, sa, sw;
	...
	if(sw) {
		ASSERT(!(wt & BOX), EVNONCE);
		ASSERT(HOMO(at, wt), EVDOMAIN);
		RE(t = maxtype(at, wt));
		t1 = STYPE(t);
		RZ(a = t == at ? a : cvt(sa ? t1 : t, a));
		if(ip = ip && t == wt && t1 != BOX) {
			ASSERT(!(AFRO & AFLAG(w)), EVRO);
			z = w;
		} else {
			RZ(z = cvt(t1, w));
		...
		if(it & NUMERIC || !ir) {
			z = (b ? jtam1e : sa ? jtam1sp : jtam1a)(jt, a, z, it & NUMERIC ? box(ind) : ope(ind), ip);
		} else {
			RE(aindex(ind, z, 0L, &ind));
			ASSERT(ind, EVNONCE);
			z = (b ? jtamne : sa ? jtamnsp : jtamna)(jt, a, z, ind, ip);
		}
	} else {
		z = merge2(sa ? denseit(a) : a, w, jstd(w, ind), ip);
	}
	EPILOG(z);
}
...
static DF2(amccn2)
{
	R amendn2(a, w, VAV(self)->f, 0);
}
static DF2(amipn2)
{
	R amendn2(a, w, VAV(self)->f, (B)(!(AT(w)&RAT + XNUM) && (1 == AC(w) || AFNJA & AFLAG(w))));
}
...
static DF1(mergn1)
{
	R merge1(w, VAV(self)->f);
}
static DF1(mergv1)
{
	DECLF;
	R merge1(w, CALL1(f1, w, fs));
}
...
static A jtamend(J jt, A w, B ip)
{
	RZ(w);
	if(VERB & AT(w)) {
		R ADERIV(CRBRACE, mergv1, ip ? amipv2 : amccv2, RMAX, RMAX, RMAX);
	} else if(ger(w)) {
		R gadv(w, CRBRACE);
	} else {
		R ADERIV(CRBRACE, mergn1, ip ? amipn2 : amccn2, RMAX, RMAX, RMAX);
	}
}
