...
static DF1(jtsuffix)
{
	...
	if(jt->rank && jt->rank[1] < AR(w)) {
		r = jt->rank[1];
		jt->rank = 0;
		R rank1ex(w, self, r, jtsuffix);
	}
	jt->rank = 0;
	R eachl(IX(IC(w)), w, atop(fs, ds(CDROP)));
}    /* f\."r w for general f */

static DF1(jtgsuffix)
{
	A h, *hv, z, *zv;
	I m, n, r;
	RZ(w);
	if(jt->rank && jt->rank[1] < AR(w)) {
		r = jt->rank[1];
		jt->rank = 0;
		R rank1ex(w, self, jt->rank[1], jtgsuffix);
	}
	jt->rank = 0;
	n = IC(w);
	h = VAV(self)->h;
...
//#endif
	...
	default:
		for(i = 0; i < n1; ++i) {
			RZ(q = CALL2(f2, x, y, fs));
			RZ(t == AT(q) && r == AR(q) && !ICMP(AS(y), AS(q), r));
			MC(zv, CAV(q), k);
			zv -= k;
			AK(x) -= k;
			AK(y) -= k;
...
static DF1(jtssg)
{
	...
	V*sv = VAV(self);
	if(jt->rank && jt->rank[1] < AR(w)) {
		R rank1ex(w, self, jt->rank[1], jtssg);
	}
	jt->rank = 0;
	fs = VAV(sv->f)->f;
	f2 = VAV(fs)->f2;
...
A jtscansp(J jt, A w, A self, AF sf)
{
	A e, ee, x, z;
	B*b;
	I f, m, j, r, t, rv[2], wr;
...
static DF1(jtsscan)
{
	...
	if(jt->jerr) {
		R jt->jerr == EWOV ? (rr[1] = r, jt->rank = rr, sscan(w, self)) : 0;
	} else {
...
static DF2(jtofxinv)
{
	...
	c = v->id;
	t = AT(w);
	if(!(c == CPLUS || c == CBDOT && t & INT || (c == CEQ || c == CNE) && t & B01)) {
		R outfix(a, w, self);
	}
...
static DF2(jtofxassoc)
{
	...
	*AS(z) = 1 + d;
	...
	MC(zv,     AV(s),          kc);                     /* {.s           */
	if(1 < d) {
		ado(jt, 1, c * (d - 1), 1L, zv + kc, AV(p), kc + CAV(s));
	}  /* (}:p) f (}.s) */
	MC(zv + kc * d, CAV(p) + kc * (d - 1), kc);         /* {:p           */
	if(jt->jerr == EWOV) {
		RESETERR;
		R ofxassoc(a, cvt(FL, w), self);
	} else {
		R z;
	}
}    /* a f/\. w where f is an atomic associative fn */

static DF1(jtiota1rev)
{
	R apv(IC(w), IC(w), -1L);
}
...
{
	A f;
	AF f1 = jtsuffix, f2 = jtoutfix;
	C id;
	V*v;
	RZ(w);
	if(NOUN & AT(w)) {
		R fdef(CBSLASH, VERB, jtgsuffix, jtgoutfix, w, 0L, fxeachv(1L, w), VGERL, RMAX, 0L, RMAX);
	}
	v = VAV(w);
	switch(v->id) {
	case CPOUND:
