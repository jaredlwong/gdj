...
static F1(jtnvrpush)
{
	if(jt->nvrtop == AN(jt->nvra)) {
		RZ(jt->nvra = ext(1, jt->nvra));
		jt->nvrav = AAV(jt->nvra);
		while(AN(jt->nvrb) < AN(jt->nvra)) {
			RZ(jt->nvrb = ext(1, jt->nvrb));
		}
		jt->nvrbv = BAV(jt->nvrb);
	}
...
static void jtnvrpop(J jt, I otop)
{
	A*v = otop + jt->nvrav;
	B*b = otop + jt->nvrbv;
...
ACTION(jtmonad  )
{
	R dfs1(stack[e], stack[b]);
}
ACTION(jtdyad   )
{
	R dfs2(stack[b], stack[e], stack[1 + b]);
}
ACTION(jtadv    )
{
	R dfs1(stack[b], stack[e]);
}
ACTION(jtconj   )
{
	R dfs2(stack[b], stack[e], stack[1 + b]);
...
ACTION(jttrident)
{
	R folk(stack[b], stack[1 + b], stack[e]);
}
ACTION(jtbident )
{
	R hook(stack[b], stack[e]);
}
ACTION(jtpunc   )
{
	R stack[e - 1];
...
static ACTION(jtmove)
{
	A z;
	z = stack[MAX(0, e)];
	if(!(NAME & AT(z)) || ASGN & AT(stack[b])) {
		R z;
	}
...
static F2(jtisf)
{
	R symbis(onm(a), CALL1(jt->pre, w, 0L), jt->symb);
}
...
ACTION(jtis)
{
	...
	f = stack[1 + b];
	c = *CAV(f);
	jt->symb = jt->local && c == CASGN ? jt->local : jt->global;
	if(NAME & AT(n)) {
		symbis(n, v, jt->symb);
	} else if(!AR(n)) {
		symbis(onm(n), v, jt->symb);
	} else           {
		ASSERT(1 == AR(n), EVRANK);
		jt->pre = ger ? jtfxx : jtope;
		rank2ex(n, v, 0L, -1L, -1L, jtisf);
	}
	jt->symb = 0;
	RNE(v);
}
...
F1(jtparsea)
{
	A*s, *stack, y, z;
	AF f;
	I b, *c, e, i, j, k, m, n, otop = jt->nvrtop, *sp;
	RZ(w);
	n = m = AN(w) - 4;
	stack = AAV(w);
	jt->asgn = 0;
	...
	RZ(y = IX(AN(w)));
	sp = AV(y); /* current location in tokens */
	do {
		for(i = 0; i < NCASES; i++) {
			c = cases[i].c;
			s = n + stack;
			if(*c++&AT(*s++) && *c++&AT(*s++) && *c++&AT(*s++) && *c++&AT(*s++)) {
				break;
			}
		}
		if(i < NCASES) {
			...
			DO(b, stack[--k] = stack[--j]; sp[k] = sp[j];);
			n = k;
		} else {
