...
{
	PROLOG;
	A a1, q, q0, q1, r, r0, r1, t, *tv, t0, t1, y, z;
	I m, n, p, *s;
	RZ(w);
	if(2 > AR(w)) {
		p = AN(w);
...
F1(jtqr)
{
	...
	if(FL & AT(r)) {
		D*v = DAV(r);
		DO(n, x = ABS(*v); if(x < c)c = x; if(x > d)d = x; v += n1;);
	} else        {
		Z*v = ZAV(r);
		DO(n, x = zmag(*v); if(x < c)c = x; if(x > d)d = x; v += n1;);
	}
...
static F2(jticor)
{
	...
	v = DAV(w);
	DO(AN(w), v[i] = jfloor(0.5 + d * v[i]) / d;);
	R w;
}
...
static B jttridiag(J jt, I n, A a, A x)
{
	D*av, d, p, *xv;
	I i, j, n1 = n - 1;
	av = DAV(a);
