...
static B eqv(I af, I wf, I m, I n, I k, C*av, C*wv, B*x, B b0, B b1)
{
	B b, *xx = x;
	I c, d, mn = m * n, q;
	if     (0 == k % sizeof(I)  )EQV(I)
						...
						d = wf ? k : 0;
						if(1 == n) {
							DO(m,       *x++ = memcmp(av, wv, k) ? b0 : b1;
						}
						av += c;
						wv += d;
						  )
						else if(af < wf) {
						DO(m, DO(n, *x++ = memcmp(av, wv, k) ? b0 : b1;
					}
					   wv += k;
							);
					av += k;
...
static B jteqf(J jt, A a, A w)
{
	A p, q;
	B b;
	V*u = VAV(a), *v = VAV(w);
	if(!(AT(a) == AT(w) && u->id == v->id)) {
		R 0;
	}
	p = u->f;
	q = v->f;
	if(!(!p && !q || p && q && matchsub(0L, 0L, 1L, 1L, p, q, &b, C0, C1))) {
		R 0;
	}
	p = u->g;
	q = v->g;
	...
	p = u->h;
	q = v->h;
	R !p&&!q || p&&q&&matchsub(0L, 0L, 1L, 1L, p, q, &b, C0, C1);
}
...
static B jtmatchsub(J jt, I af, I wf, I m, I n, A a, A w, B*x, B b0, B b1)
{
	B b, c0;
	C*av, *wv;
	I at, c, j = 0, mn, p, q, t, wt;
	p = AR(a) - af;
	at = AT(a);
	mn = m && n ? m * n : 0;
	q = AR(w) - wf;
	...
	RE(t = maxtype(at, wt));
	c0 = !jt->ct && t & FL + CMPX;
	c = (af > wf ? AN(a) : AN(w)) / (mn ? mn : 1);
	b = p != q || ICMP(af + AS(a), wf + AS(w), p) || c && !HOMO(at, wt);
	if(b || !c || a == w) {
		memset(x, b ? b0 : b1, mn);
		R b ? b0 : b1;
	}
	...
	switch(c0 ? 0 : t) {
	default:
		R eqv(af, wf, m, n, c * bp(t), av, wv, x, b0, b1);
	case FL:
		INNERT(D, teq);
		R mn ? x[mn - 1] : b1;
	case CMPX:
...
static F2(jtmatchs)
{
	...
	B*b, *pv, *qv;
	D d;
	I acr, an = 0, ar, c, j, k, m, n, r, *s, *v, wcr, wn = 0, wr;
	P*ap, *wp;
	RZ(a && w);
	ar = AR(a);
	...
	jt->rank = 0;
	if(ar > acr || wr > wcr) {
		R rank2ex(a, w, 0L, acr, wcr, jtmatchs);
	}
	if(ar != wr || memcmp(AS(a), AS(w), r * SZI) || !HOMO(AT(a), AT(w))) {
		R zero;
	}
	GA(x, B01, r, 1L, 0L);
	b = BAV(x);
	...
	m = *AS(ax);
	if(wn < c || DENSE & AT(w)) {
		RZ(w = reaxis(ifb(r, b), w));
	}
	wp = PAV(w);
	we = SPA(wp, e);
	wx = SPA(wp, x);
	n = *AS(wx);
	RZ(x = indexof(SPA(ap, i), SPA(wp, i)));
	v = AV(x);
	GA(p, B01, m, 1, 0);
	pv = BAV(p);
	GA(q, B01, n, 1, 0);
	qv = BAV(q);
	memset(pv, C1, m);
	DO(n, j = *v++; if(j < m)pv[j] = qv[i] = 0; else {
		qv[i] = 1;
		});
	if(memchr(pv, C1, m) && !all1(eq(we, repeat(p, ax)))) {
		R zero;
	}
	if(memchr(qv, C1, n) && !all1(eq(ae, repeat(q, wx)))) {
		R zero;
	}
	j = 0;
	DO(m, if(pv[i])++j;);
	...
	if(!equ(from(repeat(q, x), ax), repeat(q, wx))) {
		R zero;
	}
	x = SPA(ap, a);
	v = AV(x);
...
F2(jtmatch)
{
	...
	wf = jt->rank ? AR(w) - jt->rank[1] : 0;
	jt->rank = 0;
	if(af > wf) {
		f = af;
		s = AS(a);
		RE(m = prod(wf, s));
		RE(n = prod(af - wf, wf + s));
	} else     {
		f = wf;
		s = AS(w);
		RE(m = prod(af, s));
		RE(n = prod(wf - af, af + s));
	}
	GA(z, B01, m * n, f, s);
	matchsub(af, wf, m, n, a, w, BAV(z), C0, C1);
	R z;
}    /* a -:"r w */
...
F2(jtnotmatch)
{
	...
	GA(z, B01, m * n, f, s);
	matchsub(af, wf, m, n, a, w, BAV(z), C1, C0);
	R z;
}    /* a -.@-:"r w */
