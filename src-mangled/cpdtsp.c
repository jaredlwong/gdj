...
static F2(jtpdtspmv)
{
	...
	if(DENSE & AT(w)) {
		...
		DO(n, yv++; *v++ = *yv++;);
		wp = PAV(w);
		RZ(b = eps(yj, SPA(wp, i)));
		bv = BAV(b);
		AN(yj) = *AS(yj) = *s = m = bsum(n, bv);
		v = AV(yj);
		yv = AV(y);
		GA(yi, INT, m, 2, s);
		u = AV(yi);
		...
		*v++ = *yv++;
		...
		RZ(ax = repeat(b, SPA(ap, x)));
		RZ(wx = from(indexof(SPA(wp, i), yj), SPA(wp, x)));
	}
	RZ(x = df2(yi, tymes(ax, wx), sldot(slash(ds(CPLUS)))));
	RZ(y = nub(yi));
	RZ(g = grade1(y));
	GA(z, STYPE(AT(x)), 1, 1, AS(a));
	zp = PAV(z);
	SPB(zp, a, iv0);
	SPB(zp, e, scf(0.0));
	SPB(zp, i, from(g, y));
...
static F2(jtpdtspvm)
{
	...
	if(DENSE & AT(a)) {
		...
		DO(n, *u++ = *yv++; yv++;);
		ap = PAV(a);
		RZ(b = eps(yi, SPA(ap, i)));
		bv = BAV(b);
		AN(yi) = *AS(yi) = *s = m = bsum(n, bv);
		u = AV(yi);
		yv = AV(y);
		GA(yj, INT, m, 2, s);
		v = AV(yj);
		...
		*v++ = *yv++;
		...
		RZ(ax = from(indexof(SPA(ap, i), yi), SPA(ap, x)));
		RZ(wx = repeat(b, SPA(wp, x)));
		RZ(x = tymes(ax, wx));
	}
	RZ(x = df2(yj, x, sldot(slash(ds(CPLUS)))));
	RZ(y = nub(yj));
	RZ(g = grade1(y));
	GA(z, STYPE(AT(x)), 1, 1, 1 + AS(w));
	zp = PAV(z);
	SPB(zp, a, iv0);
	SPB(zp, e, scf(0.0));
	SPB(zp, i, from(g, y));
...
static B jtmmprep(J jt, P*p, I*n, I**iv, I*m, I**nv, D**xv)
{
	A x;
	I j, k, q, *u, *v;
	x = SPA(p, x);
...
static B jtmmharvest(J jt, I ii, I zjn, A zj, D*zyv, I*n, A*zi, A*zx)
{
	A x;
	D*zxv, *zxv0;
	I j, m, p, *v, *ziv;
...
static F2(jtpdtspmm)
{
	...
	I*aiv, *aivm, i, ii, j, k, k0, m, n = 0, old, p, q, *v, wm, *wiv, *wnv, *zjv, *zjv0;
	P*zp;
	RZ(a && w);
...
F2(jtpdtsp)
{
	...
	if(AT(a)&FL + SFL && AT(w)&FL + SFL) {
		if(SPARSE & AT(a)) {
			p = PAV(a);
			x = SPA(p, a);
			ab = AR(a) == AN(x) && equ(zero, SPA(p, e));
		}
		if(SPARSE & AT(w)) {
			p = PAV(w);
			x = SPA(p, a);
			wb = AR(w) == AN(x) && equ(zero, SPA(p, e));
		}
	}
	if(ab && 1 == AR(a) && wb && 1 == AR(w)) {
		R pdtspvv(a, w);
	}
	if(ab && 2 == AR(a) &&    1 == AR(w)) {
		R pdtspmv(a, w);
