...
static A jthparm(J jt, A j, A f, A h)
{
	A z;
	if(!(VERB & AT(f))) {
		R shift1(aslash(CSTAR, atab(CPLUS, h, j)));
	}
...
static A jthgv(J jt, B b, I n, A w, A self)
{
	A c, d, e, h, *hv, j, y;
	V*sv = VAV(self);
	RZ(j = IX(n));
	...
	case 0:
		y = ascan(CSTAR, divide(tymes(c, e), d));
		break;
	case 1:
		y = divide(ascan(CSTAR, tymes(c, e)), d);
		break;
	case 2:
		y = divide(tymes(c, ascan(CSTAR, e)), ascan(CSTAR, d));
		break;
	case 3:
		y = divide(tymes(c, ascan(CSTAR, e)), d);
	}
	R b ? over(zero, ascan(CPLUS, y)) : aslash(CPLUS, y);
}    /* verb or complex cases */

static A jthgd(J jt, B b, I n, A w, A p, A q)
{
	A c, d, e, z;
	D r, s, t, *u, *v, x, *zv;
	I j, pn, qn;
...
static DF2(jthgeom2)
{
	...
	RZ(a && w);
	if(AR(w)) {
		R rank2ex(a, w, self, 0L, 0L, jthgeom2);
	}
...
DF1(jthgcoeff)
{
	...
	if(!b) {
		...
		switch((VERB & AT(sv->f) ? 2 : 0) + (VERB & AT(sv->g) ? 1 : 0)) {
		case 0:
			y = ascan(CSTAR, divide(c, d));
			break;
		case 1:
			y = divide(ascan(CSTAR, c), d);
			break;
		case 2:
			y = divide(c, ascan(CSTAR, d));
			break;
		case 3:
			y = divide(c, d);
		}
