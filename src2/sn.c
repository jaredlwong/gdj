...
B jtvnm(J jt, I n, C*s)
{
	...
	c = *s;
	d = *(s + n - 1);
	if(jt->dotnames && 2 == n && '.' == d && ('m' == c || 'n' == c || 'u' == c || 'v' == c || 'x' == c || 'y' == c)) {
		R 1;
	}
	RZ(CA == ctype[c]);
	c = 'a';
	DO(n, d = c; c = s[i]; t = ctype[c]; RZ(t == CA || t == C9); if(c == '_' && d == '_' && !b && i != n - 1) {
	j = 1 + i;
	b = 1;
});
	if(c == '_') {
		DO(j = n - 1, if('_' == s[--j])break;);
		k = n - j - 2;
		R!b&&j&&(!k || vlocnm(k, s + j + 1));
	}
	if(!b) {
		R 1;
	}
	k = 2;
	DO(n - j, c = s[j + i]; if(2 > k)k += '_' == c; else {
		RZ(CA == ctype[c]);
			k = 0;
		});
	R !k;
...
B vlocnm(I n, C*s)
{
	...
	if(C9 == ctype[*s]) {
		RZ(1 == n || '0' != *s);
		DO(n, c = s[i]; RZ('0' <= c && c <= '9'););
	}
	R 1;
}    /* validate locale name */

A jtnfs(J jt, I n, C*s)
{
	A z;
	C c, f, *t;
	I m, p;
	...
	if((1 == n || 2 == n && '.' == s[1]) && strchr("mnuvxy", c = *s)) {
		if(1 == n) {
			R c == 'y' ? ynam : c == 'x' ? xnam : c == 'v' ? vnam : c == 'u' ? unam : c == 'n' ? nnam : mnam;
		} else {
			R c == 'y' ? ydot : c == 'x' ? xdot : c == 'v' ? vdot : c == 'u' ? udot : c == 'n' ? ndot : mdot;
		}
	}
	ASSERT(n, EVILNAME);
	GA(z, NAME, n, 1, 0);
...
A jtsfn(J jt, B b, A w)
{
	NM*v;
	RZ(w);
	v = NAV(w);
	R str(b ? v->m : AN(w), v->s);
}
...
F1(jtonm)
{
	...
	RZ(x = ope(w));
	y = stdnm(x);
	ASSERTN(y, EVILNAME, nfs(AN(x), CAV(x)));
	R y;
}
...
static F1(jtnlx)
{
	...
	if(b           ) {
		RZ(z = nlxxx(jt->global));
	}
	if(b && jt->local) {
		RZ(z = over(nlxxx(jt->local), z));
	}
	if(m & SYMB      ) {
		RZ(z = over(nlsym(jt->stloc), z));
	}
...
static A jtnch1(J jt, B b, A w, I*pm, A ch)
{
	A*v, x, y;
	C*s, *yv;
	I*e, i, k, m, p, wn;
	...
	x = (A)(*e + jt->sympv)->name;
	p = AN(x);
	s = NAV(x)->s; /* locale name           */
	m = *pm;
	v = AAV(ch) + m;                           /* result to appended to */
