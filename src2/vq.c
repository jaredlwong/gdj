...
static QF2(jtqrem)
{
	...
	q = qdiv(w, a);
	m.n = xtymes(a.n, xdiv(q.n, q.d, XMFLR));
	m.d = a.d;
	z = qminus(w, m);
	QEPILOG(z);
}
...
int jtqcompare(J jt, Q a, Q w)
{
	R QCOMP(a, w);
}

static X jtqbin(J jt, Q a, Q w)
{
	ASSERT(QX1(a.d) && QX1(w.d), EWIRR);
	R xbin(a.n, w.n);
}

static D jtqlogd1(J jt, Q w)
{
	ASSERT(0 <= XDIG(w.n), EWIMAG);
	R xlogabs(w.n) - xlogabs(w.d);
}

static Z jtqlogz1(J jt, Q w)
{
	Z z;
	z.re = xlogabs(w.n) - xlogabs(w.d);
