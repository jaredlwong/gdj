...
static SF(jtsortb2)
{
	...
	for(i = 0; i < m; ++i) {
		...
		if(up) {
			j = 0;
			DO(p, x = zz[j]; DO(yv[j], *v++ = x;); yv[j] = 0; ++j;);
		} else  {
			j = p - 1;
			DO(p, x = zz[j]; DO(yv[j], *v++ = x;); yv[j] = 0; --j;);
		}
...
static SF(jtsortc2)
{
	...
	for(i = 0; i < m; ++i) {
		...
		if(C2T & AT(w) || !liln) {
			...
			if(up) {
				k = 0;
				DO(256, j = k; DO(256, DO(yv[j], *v++ = j;); yv[j] = 0; j += 256;); ++k;);
			} else  {
				k = (US)(p - 1);
				DO(256, j = k; DO(256, DO(yv[j], *v++ = j;); yv[j] = 0; j -= 256;); --k;);
			}
...
static SF(jtsorti)
{
	...
	B up;
	D p1;
	I i, j, p, ps, q, s, *wv, *yv, *zv;
	wv = AV(w);
	irange(AN(w), wv, &q, &p);
	p1 = (D)p;
	if(!p || 256 < p && 0.69 * (p1 + 2 * n) > n * log((D)n)) {
		R 3000 < n ? sorti1(m, n, n, w) : irs2(gr1(w), w, 0L, 1L, 1L, jtfrom);
	}
	if(0 < q && p1 + q < 4 * n) {
		p += q;
		q = 0;
	...
	case 0:
		j = p - 1;
		DO(p, s = yv[j]; yv[j] = 0; DO(s, *zv++ = j  ;); --j;);
			break;
		case 1:
			j = p - 1;
			DO(p, s = yv[j]; yv[j] = 0; DO(s, *zv++ = j + q;); --j;);
			break;
		case 2:
			j = 0;
			DO(p, s = yv[j]; yv[j] = 0; DO(s, *zv++ = j  ;); ++j;);
...
F2(jtgr2)
{
	PROLOG;
	A z = 0;
	I acr, d, f, m, n, *s, t, wcr;
	RZ(a && w);
	acr = jt->rank ? jt->rank[0] : AR(a);
	wcr = jt->rank ? jt->rank[1] : AR(w);
	t = AT(w);
	f = AR(w) - wcr;
	s = AS(w);
	m = prod(f, s);
	n = s[f];
	d = bp(t) * prod(wcr - 1, 1 + f + s);
	if(a == w && acr == wcr) {
	...
	if(!z) {
		RZ(z = irs2(gr1(w), a, 0L, 1L, acr, jtfrom));
	}
	EPILOG(z);
