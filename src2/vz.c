...
static D hypoth(D u, D v)
{
	D p, q, t;
	MMM(u, v);
	R INF(p) ? inf : p ? (t = q / p, p * sqrt(1 + t * t)) : 0;
}
...
B jtzeq(J jt, Z u, Z v)
{
	D a = u.re, b = u.im, c = v.re, d = v.im, p, q;
	if(a == c && b == d) {
		R 1;
	}
	if(ZEZ(u) || ZEZ(v) || !jt->ct || (0 > a != 0 > c && 0 > b != 0 > d)) {
		R 0;
	}
	if(ZOV(u) || ZOV(v)) {
		a /= 2;
...
{
	ZASSERT(!(ZINF(u) || ZINF(v)), EVNAN);
	R ZEZ(u) || ZEZ(v) ? zeroZ : ztymes(u, zdiv(v, zgcd(u, v)));
}
...
ZF1(jtzlog)
{
	...
	if(!b) {
		zi = a < 0 ? PI : 0;    /* atan2(0,v) fails in mips handheld wince - _9^2.5-1.5*/
	}
#endif
	ZEPILOG;
}
...
static ZF1(jtztan)
{
	R zdiv(zsin(v), zcos(v));
}

static ZF1(jtzp4)
{
	R zsqrt(zplus(z1, ztymes(v, v)));
}

static ZF1(jtzm4)
{
	R 1e16 < hypoth(v.re, v.im) ? v : ztymes(zplus(v, z1), zsqrt(zdiv(zminus(v, z1), zplus(v, z1))));
}

static ZF1(jtzsinh)
{
	R zmj(zsin(zjx(v)));   /* 4.5.7 */
}
...
static ZF1(jtztanh)
{
	R v.re < -TMAX ? zrj0(-1.0) : TMAX < v.re ? z1 : zdiv(zsinh(v), zcosh(v));
}

static ZF1(jtzp8)
{
	R zsqrt(ztymes(zplus(zj, v), zminus(zj, v)));
}

static ZF1(jtzasinh)
{
	R 0 > v.re ? znegate(zasinh(znegate(v))) : zlog(zplus(v, zp4(v)));
}
...
static ZF1(jtzatanh)
{
	R ztymes(zrj0((D)0.5), zlog(zdiv(zplus(z1, v), zminus(z1, v))));
}
...
static ZF1(jtzasin)
{
	R !v.im&& -1 <= v.re&&v.re <= 1 ? zrj0(asin(v.re)) : zmj(zasinh(zjx(v)));   /* 4.4.20 */
}

static ZF1(jtzacos)
{
	R zminus(zrj0(PI / 2.0), zasin(v));
}
...
#if SY_WINCE_MIPS && !defined(WIN32_PLATFORM_PSPC)
	if(!y) {
		z.re = x < 0 ? PI : 0;    /* atan2(0,v) fails in mips handheld wince - 12 o. _3 */
	}
#endif
...
ZF2(jtzcir)
{
	...
	default:
		ZASSERT(0, EVDOMAIN);
	case  0:
		R zsqrt(ztymes(zplus(z1, v), zminus(z1, v)));
	case  1:
		R zsin(v);
	case  -1:
		R zasin(v);
...
B jtztridiag(J jt, I n, A a, A x)
{
	...
	for(i = n - 2; i >= 0; --i) {
		j -= 3;
		av[i] = zdiv(zminus(av[i], ztymes(xv[j + 1], av[i + 1])), xv[j]);
	}
	R 1;
}
