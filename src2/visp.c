...
static I jtioev(J jt, I mode, A a)
{
	A ae, ax, ay, p;
	B*pv;
	I j, k, m, n, *yv;
	...
	case 1:
		...
		R n - !m;
	case 2:
		DO(m,           if(i != yv[i] || pv[i])R i;);
		R m;
	default:
		DO(m, --j; --k; if(j != yv[k] || pv[k])R j;);
		R m == n ? n : n - m - 1;
	}
}   /* index of sparse element */

A jtiovxs(J jt, I mode, A a, A w)
{
	A e, x, z;
	B h;
	I at, t, wt;
	...
	if(ap) {
		...
		if(equ(ae, e))SPB(zp, e, sc(j))
				...
				RE(k = i0(indexofsub(mode, ax, e)));
				SPB(zp, e, sc(AN(ay) ? (m > k ? yv[k] : n) : k));
			}
...
A jtiovsd(J jt, I mode, A a, A w)
{
	A ae, ax, ay, p, z;
	B h, *pv;
	I at, j, m, n, t, wt, *v, *yv;
	P*ap;
	ap = PAV(a);
	ax = SPA(ap, x);
	ay = SPA(ap, i);
	if(!AN(ay)) {
		R indexofsub(mode, ravel(ax), w);
	}
	m = AN(ax);
	n = *AS(a);
	yv = AV(ay);
...
A jtindexofxx(J jt, I mode, A a, A w)
{
	A x;
	B*b, *c, s;
	I ar, d, j, m, n, wr;
...
static A jtioe(J jt, I mode, A w)
{
	A b, j, p, y;
	I c, jn, *jv, k, n;
	P*wp;
	...
	if(1 < AR(b)) {
		RZ(b = aslash1(CSTARDOT, b));    /* b=. *./@,"_1 (3$.w)=5$.w */
	}
...
static B jtioresparse(J jt, B aw, A*za, A*zw)
{
	A a, e, w;
	B*ab, ac = 0, *wb, wc = 0;
	I ar, j, wr;
	...
	if(    ac) {
		RZ(*za = reaxis(ifb(ar, ab), a));
	}
	if(aw && wc) {
		RZ(*zw = reaxis(ifb(wr, wb), w));
	}
	R 1;
}    /* harmonize sparse elements and sparse axes for a and w */

static B jtiopart(J jt, A w, I r, I mm, I*zc, A*zi, A*zj, A*zx)
{
	A b, f, wx, x, wy, y;
	B*bv;
	I c = *zc, d, i, j, k, m, n, nd, p, q, wr, *v, *xv;
	P*wp;
	wr = AR(w);
	d = wr - r;
	wp = PAV(w);
	...
	if     (0 == d) {
		...
		j = -1;
		DO(m, bv[i] = j != *v; j = *v; v += n;);
	} else {
		GA(x, INT, d, 1, 0);
		xv = AV(x);
		*xv = -1;
		DO(m, bv[i] = 0; DO(d, if(xv[i] != v[i]) {
		bv[i] = 1;
			j = i;
			DO(d - j, xv[j] = v[j]; ++j;);
			break;
	...
	if(m) {
		RZ(f = cut(ds(CCOMMA), one));
		RZ(y = df2(b, dropr(d, wy), f));
		RZ(x = df2(b, wx, f));
	} else {
		y = mtm;
		RZ(x = reshape(v2(0L, prod(r, AS(w) + wr - r)), wx));
	}
	if(0 > c) {
		*zc = c = *(1 + AS(y));
	} else if(c != *(1 + AS(y))) {
		RZ(y = taker(c, y));
		RZ(x = taker((c / (n - d)) * aii(wx), x));
	}
	v = AV(y);
	k = 0;
	q = *AS(y);
...
A jtindexofss(J jt, I mode, A a, A w)
{
	...
	if(aw) {
		RZ(iopart(w, ar - 1, mm, &c, &wi, &wj, &wx));
	}
	...
	case 1:
		...
		break;
	case 2:
		x = stitch(aj, jt->ct ? iocol(mode, ax, ax) : ifdz(ax));
		break;
	case 3:
		x = stitch(aj, jt->ct ? iocol(mode, ax, ax) : ifdz(ax));
	...
	v = AV(ai);
	if(aw) {
		DO(AN(x), u[i] = m > u[i] ? v[u[i]] : n;
	})
	else {
		DO(AN(x), u[i] = v[u[i]];
	});
	if(!r) {
	R AN(x) ? sc(*u) : ioe(mode, a);
	}
	GA(z, SINT, 1, r, AS(w));
	zp = PAV(z);
	SPB(zp, a, IX(r));
...
F1(jtnubsievesp)
{
	...
	n = r ? *(AS(w) + wr - r) : 1;
	if(r < wr) {
		R irs2(IX(n), irs2(w, w, 0L, r, r, jtindexof), 0L, 1L, r ? 1L : 0L, jteq);
	}
	RZ(x = indexof(w, w));
	p = PAV(x);
	y = SPA(p, i);
