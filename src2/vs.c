...
static A jtselm(J jt, I t)
{
	R t&NUMERIC ? cvt(t, zero) : t&BOX ? ace : chr[' '];
}

A jtpaxis(J jt, I r, A a)
{
	A y, z;
	B*b;
	I j, *u, *v;
...
A jtdaxis(J jt, I r, A a)
{
	R less(IX(r), a);
}
/* dense axes relative to sparse axes a */

static A jtsparse1a(J jt, A s, A a, A e, A y, A x)
{
	A z;
	B*b;
	I an, *av, et, r, *sv, t, *v;
...
A jtsparseit(J jt, A w, A a, A e)
{
	...
	RZ(z = sparse1a(shape(w), a, e, mark, mark));
	p = PAV(z);
	RZ(ax = paxis(r, a));
	GA(y, INT, r, 1, 0);
	s = AV(y);
	...
	RE(m = prod(n, s));
	b = equ(a, IX(r));
	RZ(x = gah(1 + r - n, b ? w : cant2(ax, w)));
	v = AS(x);
	*v = m;
	if(r > n) {
		ICPY(1 + v, n + s, r - n);
	}
	b = b && SB01 & AT(z) && equ(e, zero);
	c = w;
	if(!b) {
		RZ(c = not(irs2(reshape(vec(INT, r - n, n + s), SPA(p, e)), x, 0L, RMAX, -1L, jtmatch)));
	}
	cn = AN(c);
	cv = BAV(c);
	cm = bsum(cn, cv);
...
F1(jtdenseit)
{
	...
	b = equ(a, IX(an));
	if(!an || !xn) {
		R reshape(shape(w), xn ? x : e);
	}
	if(b) {
		s = AS(w);
	} else {
		RZ(q = over(a, less(IX(r), a)));
		RZ(s1 = from(q, shape(w)));
		s = AV(s1);
	}
	RE(n = prod(r, s));
	GA(z, t, n, r, s);
...
F2(jtreaxis)
{
	...
	if(all1(eps(a1, a))) {
		...
		u[j] = k = 1;
		DO(c - d, --j; u[j] = k *= u[j];);
		RZ(q = pdt(take(v2(m, d - c), y), vec(INT, c - d, 1 + u)));
		iv = AV(q);
		RZ(p = over(less(a, a1), daxis(r, a)));
		v = AV(p);
		*u = n;
		j = 1;
		DO(AN(p), u[j++] = ws[*v++];);
		...
		*(1 + AS(y1)) = d;
		...
		DO(m, MC(t + g * iv[i], s, g); s += g; if(b[i]) {
		ICPY(v, u + i * c, d);
			v += d;
			t += h;
		});
		SPB(zp, i, y1);
		SPB(zp, x, cant2(increm(indexof(p, daxis(r, a1))), x1));
		R z;
	}
	R reaxis(a1, reaxis(over(a, less(a1, a)), w));
}    /* (2;a)$.w */

static A jtaxbytes1(J jt, I t, I an, I m, I xr, I*xs)
{
	I k, z;
	k = bp(t);
...
F2(jtsparse2)
{
	...
	case 0:
		ASSERT(!q, EVDOMAIN);
		R t&SPARSE ? denseit(w) : sparse1(w);
	case 1:
		ASSERT(!q, EVDOMAIN);
		R sparsep1(w);
	case -1:
