...
{
	RZ(a && w);
	R over(VERB & AT(a) ? arep(a) : a, VERB & AT(w) ? arep(w) : w);
}


static B jtatomic(J jt, C m, A w)
{
	A f, g;
	B ax, ay, vf, vg;
	C c, id;
	...
	v = VAV(w);
	id = v->id;
	if(strchr(atomic12, id) || strchr(1 == m ? atomic1 : atomic2, id)) {
		R 1;
	}
	f = v->f;
	vf = f && VERB & AT(f);
	...
	case CAT:
	case CATCO:
		R atomic(1, f) && atomic(m, g);
	case CUNDER:
	case CUNDCO:
		R atomic(m, f) && atomic(1, g);
	case CAMPCO:
		R atomic(m, f) && atomic(1, g);
	case CQQ:
		R ax || atomic(m, f);
	case CFORK:
		R (CCAP == ID(f) ? atomic(1, g) : atomic(m, f) && atomic(2, g)) && atomic(m, v->h);
	case CHOOK:
		R atomic(2, f) && atomic(1, g);
	case CTILDE:
		R NAME&AT(f) ? atomic(m, fix(f)) : atomic(2, f);
	case CFIT:
		R atomic(m, f);
	case CAMP:
		if(vf && vg) {
		...
		if(ax && atomic(2, g) || ay && atomic(2, f)) {
			R 1;
		}
		if(vg && 1 == AR(f)) {
			c = ID(g);
...
static A jtgjoin(J jt, C c, A a, A w)
{
	A f;
	RZ(a && w);
...
static DF1(jtcase1a)
{
	...
	if(atomic(1, g))RZ(k = df1(w, g))
			...
			RZ(k = df1(w, qq(g, zero)));
			ASSERT(AR(k) == AR(w) && AN(k) == AN(w), EVRANK);
		}
	if(B01 & AT(k)) {
		h = sv->h;
		ASSERT(2 <= AN(h), EVINDEX);
		hv = AAV(h);
		RZ(x = df1(t = repeat(not(k), w), hv[0]));
		if(!AR(x)) {
			RZ(x = reshape(tally(t), x));
		}
		RZ(y = df1(t = repeat(k,     w), hv[1]));
		if(!AR(y)) {
			RZ(y = reshape(tally(t), y));
		}
		RZ(z = !AN(x) ? y : !AN(y) ? x : from(grade1(grade1(k)), over(x, y)));
	} else {
		RZ(u = nub(k));
		RZ(y = df2(k, w, sldot(gjoin(CATCO, box(scc(CBOX)), from(u, sv->f)))));
		yv = AAV(y);
		b = 0;
		DO(AN(y), if(b = !AR(yv[i]))break;);
		if(b) {
			RZ(x = df2(k, w, sldot(ds(CPOUND))));
			xv = AV(x);
...
static DF2(jtgcl2)
{
	...
	d = fdep(hv[1]);
	FDEPINC(d);
	ff = df2(df2(a, w, hv[1]), gs, ds(sv->id));
	FDEPDEC(d);
...
static DF2(jtgcr2)
{
	...
	d = fdep(hv[1]);
	FDEPINC(d);
	ff = df2(fs, df2(a, w, hv[1]), ds(sv->id));
	FDEPDEC(d);
	R df2(df2(a, w, hv[0]), df2(a, w, hv[2]), ff);
}

A jtgconj(J jt, A a, A w, C id)
{
	A hs, y;
	B na;
	I n;
...
static DF2(jtgav2)
{
	...
	d = fdep(hv[1]);
	FDEPINC(d);
	ff = df1(df2(a, w, hv[1]), ds(sv->id));
	FDEPDEC(d);
	R df2(df2(a, w, hv[0]), df2(a, w, hv[2]), ff);
}

A jtgadv(J jt, A w, C id)
{
	A hs;
	I n;
	RZ(w);
