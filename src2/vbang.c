...
static Z jtzhorner(J jt, I n, D*c, Z v)
{
	Z s;
	D*d = n + c;
	s = zeroZ;
...
static Z jtzgps(J jt, Z z)
{
	R zdiv(z1, zhorner(terms, coeff, z));
}

static D jtdgamma(J jt, D x)
{
	B b;
	D t;
	t = 1.0;
...
static Z jtzgrecur(J jt, Z z)
{
	Z t;
	t = z1;
	if(0 <= z.re) while( 0.5 < z.re) {
			--z.re;
			t = ztymes(t, z);
			if(t.re == inf) {
				R t;
...
static Z jtzgauss(J jt, D n, Z z)
{
	D d = 1 / n;
	Z p, t;
	if(1 >= n) {
...
static I signf(D x)
{
	R 0 <= x || 1 <= x - 2 * jfloor(0.5 * x) ? 1 : -1;
}
/* sign of !x */

static D jtdbin(J jt, D x, D y)
{
	D c, d, e, h = 1.0, p, q, r;
	I k = 0;
	c = y;
...
static D jtbindd(J jt, D x, D y)
{
	...
	case 0: /* 0 0 0 */
	case 2: /* 0 1 0 */
		R ix && iy ? ibin(x, y) : dbin(x, y);
	case 3: /* 0 1 1 */
		R (MOD2(x) ? -1 : 1)*ibin(x, x - y - 1);
	case 6: /* 1 1 0 */
		R (MOD2(d) ? -1 : 1)*ibin(-1 - y, -1 - x);
	case 1: /* 0 0 1 */
	case 4: /* 1 0 0 */
	case 7: /* 1 1 1 */
		R 0;
...
static Z jtbinzz(J jt, Z x, Z y)
{
	B id, ix, iy;
	D rd, rx, ry;
	Z d;
	if(!x.im && !y.im) {
		R zrj0(bindd(x.re, y.re));
	}
	d = zminus(y, x);
	rd = d.re;
	id = rd == jfloor(rd) && 0 == d.im;
	rx = x.re;
	ix = rx == jfloor(rx) && 0 == x.im;
	ry = y.re;
	iy = ry == jfloor(ry) && 0 == y.im;
	...
	case 2: /* 0 1 0 */
		R zbin(x, y);
	case 3: /* 0 1 1 */
		R zrj0((MOD2(rx) ? -1 : 1)*ibin(rx, rx - ry - 1));
	case 6: /* 1 1 0 */
		R zrj0((MOD2(rd) ? -1 : 1)*ibin(-1 - ry, -1 - rx));
	case 1: /* 0 0 1 */
	case 4: /* 1 0 0 */
	case 7: /* 1 1 1 */
		R zeroZ;
