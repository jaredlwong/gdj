...
static F2(jttclosure)
{
	A z;
	B b;
	I an, *av, c, d, i, wn, wr, wt, *wv, *zu, *zv, *zz;
	RZ(a && w);
	wt = AT(w);
	wn = AN(w);
	wr = AR(w);
	...
	if(1 == wn) {
		*zv++ = c = *wv;
		...
		while(1) {
			...
			b = 1;
			DO(wn, d = c = *zu++; if(0 > c)c += an; ASSERT(0 <= c && c < an, EVINDEX); *zv++ = c = av[c]; if(c != d)b = 0;);
			if(b) {
				break;
			}
		}
...
static DF1(jtply1)
{
	...
	A b, hs, j, x, *xv, y, z;
	B*bv, q;
	I i, k, m, n, *nv, old, p = 0;
	hs = sv->h;
	m = AN(hs);
	RZ(x = ravel(hs));
	RZ(y = from(j = grade1(x), x));
	nv = AV(y);
	GA(x, BOX, m, 1, 0);
	xv = AAV(x);
	while(p < m && 0 > nv[p]) {
	...
	if(0 < p) {
		...
		n = nv[0];
		k = p - 1;
		RZ(b = eq(scf(-inf), from(j, ravel(gs))));
		bv = BAV(b);
		q = bv[k];
		old = jt->tbase + jt->ttop;
		for(i = -1; i >= n; --i) {
	...
	z = ope(reshape(shape(hs), from(grade1(j), x)));
	EPILOG(z);
}
...
static DF1(jtply1s)
{
	...
	A hs, j, y, y1, z;
	C*v, *zv;
	I c, e, i, *jv, k, m, n, *nv, r, *s, t, zn;
	RZ(w);
	hs = sv->h;
	m = AN(hs);
	nv = AV(hs);
	RZ(j = grade1(ravel(hs)));
	jv = AV(j);
	e = nv[*jv];
	if(!e && !nv[jv[m - 1]]) {
		R reshape(over(shape(hs), shape(w)), w);
	}
	RZ(y = y1 = CALL1(f1, w, fs));
	t = AT(y);
	r = AR(y);
	if(0 > e || t == BOX) {
		R ply1(w, self);
	...
	if(!e) {
		...
		if(!(t == AT(w) && AN(y) == AN(w) && (r == AR(w) || 1 >= r && 1 >= AR(w)))) {
			R ply1(w, self);
		}
	}
	k = AR(hs);
	RE(zn = mult(m, AN(y)));
	GA(z, AT(y), zn, k + AR(y), 0);
	zv = CAV(z);
	s = AS(z);
	ICPY(s, AS(hs), k);
	ICPY(k + s, AS(y), r);
	...
	for(i = 2; i <= n; ++i) {
		RZ(y = CALL1(f1, y, fs));
		if(t != AT(y) || r != AR(y) || ICMP(AS(y), s, r)) {
			R ply1(w, self);
		}
		DIST(i, y);
	}
...
F2(jtpowop)
{
	...
	case NV:
		ASSERT(0, EVDOMAIN);
	case NN:
		ASSERT(-1 == i0(w), EVDOMAIN);
		R vger2(CPOWOP, a, w);
	case VV:
		v = VAV(a);
		b = (v->id == CAT || v->id == CATCO) && ID(v->g) == CRIGHT;
	...
	case VN:
		if(BOX & AT(w)) {
			...
			if(!AR(w) && (x = *AAV(w), !AR(x) && NUMERIC & AT(x) || 1 == AR(x) && !AN(x))) {
				...
				if((!AN(x) || FL & AT(x) && inf == *DAV(x)) &&
						CAMP == v->id && (CFROM == ID(v->f) && (y = v->g, INT&AT(y) && 1 == AR(y)))) {
					f1 = jtindexseqlim1;
				}
				if(CTILDE == v->id && CFROM == ID(v->f)) {
					f2 = jtindexseqlim2;
				}
		...
		r = 0 < AR(hs);
		if(!r && -1 == n) {
			R CDERIV(CPOWOP, jtinv1, jtinv2, RMAX, RMAX, RMAX);
		}
		if(m && AT(w)&FL + CMPX) {
			RE(b = !all0(eps(w, over(ainf, scf(infm)))));
		}
