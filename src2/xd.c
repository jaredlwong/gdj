...
#if !SY_WINCE
char* toascbuf(char* s)
{
	return s;
}
char* tounibuf(char* s)
{
	return s;
}
#else
...
F1(jtjfperm1)
{
	...
	RE(f = stdf(w));
	if(f) {
		ASSERT(y = fname(sc((I)f)), EVFNUM) else {
			y = AAV0(w);
		}
	}
	RZ(fn = toutf16x(y));
...
F1(jtjfperm1)
{
	...
	GA(z, LIT, 3, 1, 0);
	s = CAV(z);
	if((attr = GetFileAttributes(tounibuf(p))) == 0xFFFFFFFF) {
		R jerrno();
	}
	s[0] = 'r';
...
static A jtdir1(J jt, LPWIN32_FIND_DATAW f, C* fn)
{
	A z, *zv;
	C rwx[3], *s, *t;
	I n, ts[6];
...
F2(jtjfatt2)
{
	...
	RZ(fn = toutf16x(y));
	if(SetFileAttributesW(USAV(fn), x)) {
		R one;
	}
	jsignal(EVFNAME);
	R 0;
...
{
	strcpy(jt->dirbase, name);
	if(stat(jt->dirnamebuf, &jt->dirstatbuf)) {
		R 0;
	}
	if('.' != *pat && ((!strcmp(name, ".")) || (!strcmp(name, "..")))) {
		R 0;
	}
	if(fnmatch(pat, name, 0)) {
		R 0;
	}
...
static A jtdir1(J jt, struct dirent*f)
{
	A z, *zv;
	C*s, att[16];
	I n, ts[6], i, m, sz;
...
F1(jtjdir)
{
	...
	if(NULL == (DP = opendir(dir))) {
		R reshape(v2(0L, 6L), ace);
	}
	/*
	 * SYSV and BSD have different return types for sprintf(),
	 * so we use less efficient but portable code.
	 */
	sprintf(jt->dirnamebuf, "%s/", dir);
	jt->dirbase = jt->dirnamebuf + strlen(jt->dirnamebuf);
	f = readdir(DP);
	GA(z, BOX, n, 1, 0);
	zv = AAV(z);
	while(f) {
...
F1(jtjfperm1)
{
	...
	RE(f = stdf(w));
	if(f) {
		ASSERT(y = fname(sc((I)f)), EVFNUM)
	} else {
		y = str0(AAV0(w));
	}
	if(0 != stat(CAV(y), &jt->dirstatbuf)) {
		R jerrno();
	}
