...
static void smmput1(A a, I**mfree, I n, C*v)
{
	I j, k;
	MS*x;
	k = 64;
...
static B jtsmmjoin(J jt, A a, I j)
{
	A y;
	I m, **mfree, n, *p, *q;
	RZ(y = smmblkf(a));
	...
	p = q = AV(y);
	m = 0;
	DO(n - 1, if(p[2] == p[0] + p[1])q[1] += p[3]; else {
		q += 2;
		q[0] = p[2];
			q[1] = p[3];
			++m;
...
static A jtsmmga(J jt, A a, I t, I n, I r, I*s)
{
	A z;
	I m, w;
	w = WP(t, n, r);
...
F2(jtsmmcar)
{
	...
	if(t & BOX) {
		wd = (I)w * ARELATIVE(w);
		DO(n, RZ(x = smmcar(a, WVR(i))); zv[i] = AREL(x, z););
	} else {
		MC(zv, wv, n * bp(t));
	}
	R z;
...
A jtcpa(J jt, B b, A w)
{
	A*wv, z, *zv;
	I wd;
	if(0 == b && AFNJA & AFLAG(w)) {
		R ra(w);
	}
	if(!(BOX & AT(w))) {
		R ca(w);
...
static B leafrel(A w)
{
	...
	if(BOX & AT(w)) {
		...
		if(AFLAG(w)&AFSMM + AFREL) {
			R 1;
		}
		v = AAV(w);
		DO(AN(w), if(leafrel(v[i]))R 1;);
	}
	R 0;
...
static I smmblkun(B b, A w)
{
	...
	if(BOX & AT(w)) {
		wv = A1AV(w);
		DO(AN(w), z += smmblkun(1,   (A)AABS(wv[i], w)););
	}
	R z;
}    /* # of lines in the result of smmblku */

static I* smmblku1(B b, I*zv, A w)
{
	A1*wv;
	MS*x;
	x = (MS*)w - 1;
	...
	if(BOX & AT(w)) {
		wv = A1AV(w);
		DO(AN(w), zv = smmblku1(1, zv, (A)AABS(wv[i], w)););
	}
	R zv;
}

static A jtsmmblku(J jt, A w)
{
	A z;
	I n;
	RZ(w);
...
F1(jtsmmblks)
{
	...
	*zv++ = IMIN;
	v = AV(x);
	DO(*AS(x), *zv++ = *v++; *zv++ = *v++; *zv++ = SMMCINUSE;);
	v = AV(y);
	DO(*AS(y), *zv++ = *v++; *zv++ = *v++; *zv++ = SMMCFREE; );
	RZ(z = grade2(z, z));
	zv = AV(z);
	*zv++ = (I)smmu(w);
	*zv++ = smmsize(w);
