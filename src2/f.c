...
I jtthv(J jt, A w, I n, C*s)
{
	A t;
	B ov = 0;
	C buf[WZ], *x, *y = s;
	I k, n4 = n - 4, p, wd, wn, wt;
	VF fmt;
	RZ(w && n);
	wn = AN(w);
	wt = AT(w);
...
static F1(jtthbit)
{
	...
	*(AS(z) + AR(z) - 1) = p;
	...
	q = c / BB;
	r = c % BB;
	r1 = c % BW ? (BW - c % BW) / BB : 0;
	for(i = 0; i < m; ++i) {
		DO(q - !r, memcpy(y, bitdisp + 2 * BB**x, 2 * BB  ); ++x; y += 2 * BB  ;);
		if(r)   {
			memcpy(y, bitdisp + 2 * BB**x, 2 * r - 1);
			++x;
			y += 2 * r - 1;
		} else    {
			memcpy(y, bitdisp + 2 * BB**x, 2 * BB - 1);
			++x;
			y += 2 * BB - 1;
		}
		x += r1;
...
static F1(jtthn)
{
	...
	if(1 >= r) {
		...
		*(AS(z) + AR(z) - 1) = p;
		...
		memset(zv, ' ', AN(z));
		y = tv;
		DO(m, DO(c, zv += dv[i]; p = strlen(y); MC(zv - p - (c > 1 + i), y, p); y += wd;););
	}
	R z;
}
...
static A jtthdx1(J jt, DX y)
{
	...
	GA(z, LIT, b + p1 + (1 < p1) + XBASEN * (n - 1) + p2, 1, 0);
	s = CAV(z);
	if(b) {
		*s++ = CSIGN;
	} *s++ = *s1;
...
static F1(jtthxqe)
{
	...
	case XNUM: {
		X*u = (X*) wv;
		DO(m, DO(c, RZ(*v++ = y = thx1(*u++));  dv[i] = MAX(dv[i], AN(y));));
	}
	break;
	case RAT:  {
		Q*u = (Q*) wv;
		DO(m, DO(c, RZ(*v++ = y = thq1(*u++));  dv[i] = MAX(dv[i], AN(y));));
	}
	break;
#ifdef UNDER_CE
	default:
		if (AT(w) == XD) {
			DX*u = (DX*)wv;
			DO(m, DO(c, RZ(*v++ = y = thdx1(*u++)); dv[i] = MAX(dv[i], AN(y));));
		} else          {
			ZX*u = (ZX*)wv;
			ASSERT(0, EVNONCE);
		}
	...
	*(AS(z) + AR(z) - 1) = p;
	...
	memset(zv, ' ', AN(z));
	v = tv;
	DO(m, DO(c, zv += dv[i]; y = *v++; p = AN(y); MC(zv - p - (c > 1 + i), AV(y), p);));
	R z;
}


static B jtrc(J jt, A w, A*px, A*py)
{
	A*v, x, y;
	I j, k, r, *s, xn, *xv, yn, *yv;
	RZ(w);
	r = AR(w);
	s = AS(w);
	v = AAV(w);
	xn = 1 < r ? s[r - 2] : 1;
	RZ(*px = x = apv(1 + xn, 0L, 0L));
	xv = AV(x);
	yn =  r ? s[r - 1] : 1;
	RZ(*py = y = apv(1 + yn, 0L, 0L));
	yv = AV(y);
...
static void jtfram(J jt, I k, I n, I*x, C*v)
{
	C a, b = 9 == k, d, l, r;
	l = jt->bx[k];
	a = b ? ' ' : jt->bx[10];
	...
	r = b ? l : jt->bx[2 + k];
	*v++ = l;
	DO(n, memset(v, a, x[i] - 1); v += x[i] - 1; *v++ = d;);
	*--v = r;
}

static void jtfminit(J jt, I m, I ht, I wd, A x, A y, C*zv)
{
	C*u, *v;
	I p, xn, *xv, yn, *yv;
	p = ht * wd;
	...
	fram(9L, yn, yv, zv);
	u = zv;
	DO(ht - 2, MC(u += wd, zv, wd););
	fram(3L, yn, yv, u = v = zv + wd**xv);
	DO(xn - 1, MC(u += wd * xv[1 + i], v, wd););
	fram(0L, yn, yv, zv);
...
static void jtfmfill(J jt, I p, I q, I wd, A w, A x, A y, C*zv)
{
	A e, *wv;
	C*u, *v;
	...
	xv = AV(x);
	j = 1;
	DO(1 + xn, k = xv[i]; xv[i] = j; j += k;);
	yn = AN(y) - 1;
	yv = AV(y);
	j = 1;
	DO(1 + yn, k = yv[i]; yv[i] = j; j += k;);
	for(i = 0; i < n; ++i) {
		j = i / yn % xn;
		k = i % yn;
		d = i / q * p + wd * xv[j] + yv[k];
		e = wv[i];
		s = AS(e);
		r = s[0];
		c = s[1];
		if(xp) {
			d += (xv[1 + j] - xv[j] - 1 - r) / (3 - xp) * wd;
		}
		if(yp) {
			d += (yv[1 + k] - yv[k] - 1 - c) / (3 - yp);
		}
		u = zv + d - wd;
		v = CAV(e) - c;
		DO(r, MC(u += wd, v += c, c););
...
static F1(jtenframe)
{
	A x, y, z;
	C*zv;
	I ht, m, n, p, q, wd, wr, xn, *xv, yn, *yv, zn;
	RE(rc(w, &x, &y));
	n = AN(w);
	wr = MAX(2, AR(w));
	xn = AN(x) - 1;
	xv = AV(x);
	ht = 1;
	DO(xn, ht += xv[i]; ASSERT(0 < ht, EVLIMIT););
	yn = AN(y) - 1;
	yv = AV(y);
	wd = 1;
	DO(yn, wd += yv[i]; ASSERT(0 < wd, EVLIMIT););
	RE(p = mult(ht, wd));
	q = MAX(1, xn * yn);
	m = n / q;
	RE(zn = mult(m, p));
...
F1(jtmat)
{
	...
	if(b) {
		...
		k = 2 < r ? 2 - r : 0;
		DO(r - 2, p = m; m *= s[i]; ASSERT(m >= p, EVLIMIT); k += m;);
	}
	RE(p = mult(m, q) + k * !!q);
	ASSERT(0 <= p, EVLIMIT);
	RE(zn = mult(p, c));
...
static F1(jtthbox)
{
	A z;
	UC*s;
	static C ctrl[] = " \001\002\003\004\005\006\007   \013\014 ";
...
static I scanbdc(C*v, I h, I nq, I c, I lb, I la)
{
	C*u, x;
	I m;
	u = v;
...
static I scaneol(C*v, I h, I nq, I c, I lb, I la)
{
	C e, *u, x;
	I m;
	u = v;
...
static C*dropl(I zn, C*zu, C*zv, I lb, I la, I m, C eo, C*eov)
{
	C*u, *v;
	I lc = 0, n, p, q;
	p = q = 0;
...
static A jtjprx(J jt, I ieol, I maxlen, I lb, I la, A w)
{
	A y, z;
	B ch;
	C e, eo, *eov, *v, x, *zu, *zv;
	D lba;
	...
	h = lba < IMAX ? lb + la : IMAX;
	h = MIN(nq, h);
	RE(zn = (3 + m) + (q ? p*m : 0) + mult(h, ch ? c + m + (3 + m) * (1 + c / maxlen) : c1 + m + 3 * (c1 < c)));
	if(ch && 1 < m             ) {
		zn +=    scaneol(v, h, nq, c, lb, la);
	}
	if(ch || AT(w)&BOX + SPARSE) {
		zn += nbx = scanbdc(v, h, nq, c, lb, la);
	}
	GA(z, LIT, zn, 1, 0);
	zu = zv = CAV(z);
	...
	if(lc > 1 + lba) {
		zv = dropl(zn, zu, zv, lb, la, m, eo, eov);
	}
	p = zv - zu;
