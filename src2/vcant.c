...
static A jtcants(J jt, A a, A w, A z)
{
	A a1, q, y;
	B*b, *c;
	I*u, wr, zr;
...
static F2(jtcanta)
{
	...
	B b;
	C*wv, *zv;
	I*av, c, d, j, k, *mv, r, *sv, *tv, wf, wr, *ws, zn, zr;
	RZ(a && w);
	av = AV(a);
	ws = AS(w);
	wr = AR(w);
	...
	if(wf = wr - r) {
		...
		tv = AV(a);
		DO(wf, tv[i] = i;);
		DO(r, tv[wf + i] = wf + av[i];);
		av = tv;
	}
	zr = 0;
	DO(wr, zr = MAX(zr, av[i]););
	zr += 0 < wr;
	GA(m, INT, zr, 1, 0);
	mv = AV(m);
	GA(s, INT, zr, 1, 0);
	...
	r = zr;
	j = wr;
	DO(wr, --j; if(j != av[j])break; d *= sv[j]; --r;);
	if(1 < d) {
		DO(r, mv[i] /= d;
	});
	zn = zn / d;
...
{
	A*av, p, t, y;
	I ad, j, k, m, n, *pv, q, r, *v;
	RZ(a && w);
	q = jt->rank ? jt->rank[0] : AR(a);
	r = jt->rank ? jt->rank[1] : AR(w);
	jt->rank = 0;
	if(1 < q || q < AR(a)) {
		R rank2ex(a, w, 0L, MIN(1, q), r, jtcant2);
	}
	if(BOX & AT(a)) {
		RZ(y = pfill(r, t = raze(a)));
		v = AV(y);
		GA(p, INT, AN(y), 1, 0);
		pv = AV(p);
		m = AN(a);
		...
		j = 0;
		DO(r - n, pv[*v++] = j++;);
		DO(m, k = AN(AVR(i)); DO(k, pv[*v++] = j;); if(k)++j;);
	} else {
		p = pinv(pfill(r, a));
	}
