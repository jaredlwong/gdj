...
#define LOCK 1
static B jtdolock(J jt, B lk, F f, I i, I n)
{
	ASSERT(0, EVNONCE);
}
#endif
...
F1(jtjlock)
{
	...
	if(jt->flkn == *AS(jt->flkd)) {
		RZ(jt->flkd = ext(1, jt->flkd));
	}
	RE(b = dolock(1, (F)v[0], v[1], v[2]));
	if(!b) {
		R zero;
	}
	ICPY(AV(jt->flkd) + LKC * jt->flkn, v, LKC);
	++jt->flkn;
	R one;
}    /* w is (number,index,length); lock the specified region */

static A jtunlj(J jt, I j)
{
	B b;
	I*u, *v;
	RE(j);
