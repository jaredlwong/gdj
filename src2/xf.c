...
static A jtrdns(J jt, F f)
{
	A za, z;
	I n;
	size_t r, tr = 0;
	GA(za, LIT, n = 1024, 1, 0);
	clearerr(f);
...
static B jtwa(J jt, F f, I j, A w)
{
	C*x;
	I n, p = 0;
	size_t q = 1;
...
F1(jtjfread)
{
	...
	RE(f = stdf(w));
	if(f) {
		R 1 == (I)f ? jgets("\001") : 3 == (I)f ? rdns(stdin) : rd(vfn(f), 0L, -1L);
	}
	RZ(f = jope(w, FREAD));
	z = rd(f, 0L, -1L);
	fclose(f);
...
F2(jtjfwrite)
{
	...
	if(4 == (I)f) {
		R (U)AN(a) != fwrite(CAV(a), sizeof(C), AN(a), stdout) ? jerrno() : a;
	}
	if(5 == (I)f) {
		R (U)AN(a) != fwrite(CAV(a), sizeof(C), AN(a), stderr) ? jerrno() : a;
	}
	if(b = !f) {
		RZ(f = jope(w, FWRITE))
	} else {
...
static F jtixf(J jt, A w)
{
	...
	case INT:
		f = (F) * AV(w);
		ASSERT(2 < (UI)f, EVFNUM);
	}
	R f ? vfn(f) : f;
}    /* process index file arg for file number; 0 if a file name */

static B jtixin(J jt, A w, I s, I*i, I*n)
{
	A in, *wv;
	I j, k, m, *u, wd;
	if(AT(w)&BOX) {
...
static B setparm(C*v, C*ms, HParamBlockRec mp)
{
	I n;
	n = strlen(v);
	ASSERT(n <= NPATH, EVLIMIT);
	*ms = n;
	MC(1 + ms, v, n);
