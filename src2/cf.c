...
static DF2(jtcorx2)
{
	...
	A z;
	if(cap(fs)) {
		RZ(z = df2(a, w, folk(ds(CCAP), gs, hs)))
	} else {
		FOLK2;
	}
	EPILOG(z);
...
static DF1(jtnvv1)
{
	DECLFGH;
	PROLOG;
	EPILOG(CALL2(g2, fs, CALL1(h1,  w, hs), gs));
}
static DF2(jtnvv2)
{
	DECLFGH;
	PROLOG;
	EPILOG(CALL2(g2, fs, CALL2(h2, a, w, hs), gs));
}

static DF2(jtfolkcomp)
{
	DECLFGH;
	PROLOG;
	A z;
...
static DF1(jtcharmapa)
{
	V*v = VAV(self);
	R charmap(w, VAV(v->h)->f, v->f);
}
static DF1(jtcharmapb)
{
	V*v = VAV(self);
	R charmap(w, VAV(v->f)->f, VAV(v->h)->f);
}

A jtfolk(J jt, A f, A g, A h)
{
	A p, q, x, y;
	AF f1 = jtfolk1, f2 = jtfolk2;
	B b;
	C c, fi, gi, hi;
	I flag = 0, j, m = -1;
	V*fv, *gv, *hv, *v;
	...
	if(NOUN & AT(f)) { /* y {~ x i. ] */
		...
		if(LIT & AT(f) && 1 == AR(f) && gi == CTILDE && CFROM == ID(gv->f) && hi == CFORK) {
			x = hv->f;
			if(LIT & AT(x) && 1 == AR(x) && CIOTA == ID(hv->g) && CRIGHT == ID(hv->h)) {
				f1 = jtcharmapa;
			}
		}
	...
	case CTILDE:
		...
		break; /* f  g h */
	case CSLASH:
		if(gi == CDIV && hi == CPOUND && CPLUS == ID(fv->f)) {
			f1 = jtmean;
		}
		break;  /* +/%# */
	case CAMP:   /* x&i.     { y"_ */
	...
	case CAT:    /* <"1@[ { ] */
		if(gi == CLBRACE && hi == CRIGHT) {
			p = fv->f;
			q = fv->g;
			if(CLEFT == ID(q) && CQQ == ID(p) && (v = VAV(p), x = v->f, CLT == ID(x) && equ(one, v->g))) {
				f2 = jtsfrom;
			}
		}
	}
	...
	case CSLASH:
		c = ID(gv->f);
		m = c == CPLUS ? 4 : c == CPLUSDOT ? 5 : c == CSTARDOT ? 6 : -1;
		if(fi == CCAP && vaid(gv->f) && vaid(h)) {
			f2 = jtfslashatg;
		}
		break;
	case CFCONS:
		if(hi == CFCONS) {
			x = hv->h;
			j = *BAV(x);
			m = B01 & AT(x) ? (gi == CIOTA ? j : gi == CICO ? 2 + j : -1) : -1;
		}
		break;
	case CRAZE:
		if(hi == CLBRACE) {
	...
	if(0 <= m) {
		v = 4 <= m ? hv : fv;
		b = CFIT == v->id && equ(zero, v->g);
		switch(b ? ID(v->f) : v->id) {
		case CEQ:
			f2 = b ? jtfolkcomp0 : jtfolkcomp;
			flag = 0 + 8 * m;
			break;
		case CNE:
			f2 = b ? jtfolkcomp0 : jtfolkcomp;
...
static DF1(taa)
{
	TDECL;
	A t = df1(w, fs);
	ASSERT(!t || AT(t)&NOUN + VERB, EVSYNTAX);
	R df1(t, gs);
}
static DF1(tvc)
{
...
{
	RZ(a && w);
	if(AT(a) == AT(w) && AT(a)&IS1BYTE && 1 == AN(a)) {
		A z;
...
static DF2(jthkfrom)
{
	R from(a, gah(1L, w));   /* special code for ({ ,) */
}
static DF2(jthktake)
{
	R take(a, gah(1L, w));   /* special code for ({.,) */
}
static DF2(jthkdrop)
{
	R drop(a, gah(1L, w));
...
F2(jthook)
{
	...
	case BD(VERB, VERB):
		...
		if(d == CCOMMA)switch(c) {
			...
			case CSLDOT:
				if(COMPOSE(d) && e == CIOTA && CPOUND == ID(v->g) && CBOX == ID(u->f)) {
					f1 = jtgroup;
				}
				break;
			case CPOUND:
				if(COMPOSE(d) && e == CIOTA && CPOUND == ID(v->g)) {
					f1 = jthkiota;
				}
				break;
			case CABASE:
				if(COMPOSE(d) && e == CIOTA && CSLASH == ID(v->g) && CSTAR == ID(VAV(v->g)->f)) {
					f1 = jthkodom;
				}
				break;
			case CIOTA:
			case CICO:
				if(d == CSLASH && (e == CMAX || e == CMIN)) {
					f1 = jthkindexofmaxmin;
				}
				break;
			case CFROM:
				if(d == CGRADE) {
					f2 = jtordstati;
				} else if(d == CTILDE && e == CGRADE) {
					f2 = jtordstat;
				}
			}
	...
	case BD(VERB, CONJ):
		f1 = tvc;
		id = ID(w);
		if(BOX & AT(a) && (id == CATDOT || id == CGRAVE || id == CGRCO)) {
			flag = VGERL;
		}
		break;
	case BD(CONJ, NOUN):
	case BD(CONJ, VERB):
		f1 = tcv;
		id = ID(a);
		if(BOX & AT(w) && (id == CGRAVE || id == CPOWOP && 1 < AN(w))) {
			flag = VGERR;
		}
	}
